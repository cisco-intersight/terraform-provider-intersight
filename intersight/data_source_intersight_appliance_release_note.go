...
func dataSourceApplianceReleaseNoteRead(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewApplianceReleaseNote()
	...
	result, _, err := conn.ApiClient.ApplianceApi.GetApplianceReleaseNoteList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewApplianceReleaseNote()
			...
			if err := d.Set("notes", flattenListOnpremUpgradeNote(s.Notes, d)); err != nil {
				return err
			}
