...
func dataSourceCapabilitySiocModuleDescriptor() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceCapabilitySiocModuleDescriptorRead,
...
func dataSourceCapabilitySiocModuleDescriptorRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.CapabilityApi.GetCapabilitySiocModuleDescriptorList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.CapabilitySiocModuleDescriptorList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to CapabilitySiocModuleDescriptor: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewCapabilitySiocModuleDescriptorWithDefaults()
