...
func dataSourceHyperflexServerFirmwareVersion() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceHyperflexServerFirmwareVersionRead,
...
func dataSourceHyperflexServerFirmwareVersionRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewHyperflexServerFirmwareVersionWithDefaults()
	...
	res, _, err := conn.ApiClient.HyperflexApi.GetHyperflexServerFirmwareVersionList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.HyperflexServerFirmwareVersionList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to HyperflexServerFirmwareVersion: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewHyperflexServerFirmwareVersionWithDefaults()
			...
			if err := d.Set("server_firmware_version_entries", flattenListHyperflexServerFirmwareVersionEntry(s.ServerFirmwareVersionEntries, d)); err != nil {
