...
func dataSourceFirmwarePsuDescriptor() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceFirmwarePsuDescriptorRead,
...
func dataSourceFirmwarePsuDescriptorRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.FirmwareApi.GetFirmwarePsuDescriptorList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.FirmwarePsuDescriptorList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to FirmwarePsuDescriptor: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewFirmwarePsuDescriptorWithDefaults()
