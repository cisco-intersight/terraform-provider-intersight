...
func dataSourceFabricUplinkRole() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceFabricUplinkRoleRead,
...
func dataSourceFabricUplinkRoleRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.FabricApi.GetFabricUplinkRoleList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.FabricUplinkRoleList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to FabricUplinkRole: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewFabricUplinkRoleWithDefaults()
