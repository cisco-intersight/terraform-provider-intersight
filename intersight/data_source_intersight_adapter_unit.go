...
func dataSourceAdapterUnitRead(d *schema.ResourceData, meta interface{}) error {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			...
			if err := d.Set("ext_eth_ifs", flattenListAdapterExtEthInterfaceRef(s.ExtEthIfs, d)); err != nil {
				return err
			}

			if err := d.Set("host_eth_ifs", flattenListAdapterHostEthInterfaceRef(s.HostEthIfs, d)); err != nil {
				return err
			}

			if err := d.Set("host_fc_ifs", flattenListAdapterHostFcInterfaceRef(s.HostFcIfs, d)); err != nil {
				return err
			}

			if err := d.Set("host_iscsi_ifs", flattenListAdapterHostIscsiInterfaceRef(s.HostIscsiIfs, d)); err != nil {
				return err
			}
