...
func resourceFirmwareGpuDescriptor() *schema.Resource {
	return &schema.Resource{
		Create: resourceFirmwareGpuDescriptorCreate,
		Read:   resourceFirmwareGpuDescriptorRead,
		Update: resourceFirmwareGpuDescriptorUpdate,
		Delete: resourceFirmwareGpuDescriptorDelete,
...
func resourceFirmwareGpuDescriptorCreate(d *schema.ResourceData, meta interface{}) error {
	...
	o.SetClassId("firmware.GpuDescriptor")
	...
	o.SetObjectType("firmware.GpuDescriptor")
	...
	r := conn.ApiClient.FirmwareApi.CreateFirmwareGpuDescriptor(conn.ctx).FirmwareGpuDescriptor(*o)
	...
	return resourceFirmwareGpuDescriptorRead(d, meta)
}

func resourceFirmwareGpuDescriptorRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)

	r := conn.ApiClient.FirmwareApi.GetFirmwareGpuDescriptorByMoid(conn.ctx, d.Id())
	s, _, err := r.Execute()
...
func resourceFirmwareGpuDescriptorUpdate(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewFirmwareGpuDescriptorWithDefaults()
	...
	r := conn.ApiClient.FirmwareApi.UpdateFirmwareGpuDescriptor(conn.ctx, d.Id()).FirmwareGpuDescriptor(*o)
	result, _, err := r.Execute()
	if err != nil {
...
func resourceFirmwareGpuDescriptorDelete(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	p := conn.ApiClient.FirmwareApi.DeleteFirmwareGpuDescriptor(conn.ctx, d.Id())
	_, err := p.Execute()
	if err != nil {
