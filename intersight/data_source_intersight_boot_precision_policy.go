...
func dataSourceBootPrecisionPolicy() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceBootPrecisionPolicyRead,
...
func dataSourceBootPrecisionPolicyRead(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("enforce_uefi_secure_boot"); ok {
		x := (v.(bool))
	...
	res, _, err := conn.ApiClient.BootApi.GetBootPrecisionPolicyList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.BootPrecisionPolicyList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to BootPrecisionPolicy: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewBootPrecisionPolicyWithDefaults()
