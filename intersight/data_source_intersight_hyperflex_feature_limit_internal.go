...
func dataSourceHyperflexFeatureLimitInternal() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceHyperflexFeatureLimitInternalRead,
...
func dataSourceHyperflexFeatureLimitInternalRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.HyperflexApi.GetHyperflexFeatureLimitInternalList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.HyperflexFeatureLimitInternalList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to HyperflexFeatureLimitInternal: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewHyperflexFeatureLimitInternalWithDefaults()
