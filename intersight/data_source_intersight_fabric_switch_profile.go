...
func dataSourceFabricSwitchProfile() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceFabricSwitchProfileRead,
...
func dataSourceFabricSwitchProfileRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.FabricApi.GetFabricSwitchProfileList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.FabricSwitchProfileList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to FabricSwitchProfile: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewFabricSwitchProfileWithDefaults()
