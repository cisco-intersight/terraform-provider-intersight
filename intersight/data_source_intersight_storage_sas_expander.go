...
func dataSourceStorageSasExpanderRead(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewStorageSasExpanderWithDefaults()
	...
	res, _, err := conn.ApiClient.StorageApi.GetStorageSasExpanderList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to StorageSasExpander: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewStorageSasExpanderWithDefaults()
			...
			if err := d.Set("expander_id", (s.ExpanderId)); err != nil {
				return fmt.Errorf("error occurred while setting property ExpanderId: %+v", err)
			}
			...
			if err := d.Set("sas_address", (s.SasAddress)); err != nil {
				return fmt.Errorf("error occurred while setting property SasAddress: %+v", err)
			}
