...
func dataSourceFcpoolPoolMemberRead(d *schema.ResourceData, meta interface{}) error {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			...
			if err := d.Set("block_head", flattenMapFcpoolFcBlockRelationship(s.GetBlockHead(), d)); err != nil {
			...
			if err := d.Set("peer", flattenMapFcpoolLeaseRelationship(s.GetPeer(), d)); err != nil {
