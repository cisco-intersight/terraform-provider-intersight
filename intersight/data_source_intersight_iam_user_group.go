...
func dataSourceIamUserGroup() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceIamUserGroupRead,
...
func dataSourceIamUserGroupRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.IamApi.GetIamUserGroupList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.IamUserGroupList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to IamUserGroup: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewIamUserGroupWithDefaults()
			...
			if err := d.Set("qualifier", flattenMapIamQualifierRelationship(s.GetQualifier(), d)); err != nil {
