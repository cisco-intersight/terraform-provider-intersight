...
func resourceIamResourceRoles() *schema.Resource {
	return &schema.Resource{
		Create: resourceIamResourceRolesCreate,
		Read:   resourceIamResourceRolesRead,
...
func resourceIamResourceRolesCreate(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewIamResourceRoles()
	...
	if v, ok := d.GetOk("resource"); ok {
		...
		l := (v.([]interface{})[0]).(map[string]interface{})
		{
			...
			p = append(p, o.AsMoBaseMoRelationship())
		}
		x := p[0]
		o.SetResource(x)
	}
	...
	r := conn.ApiClient.IamApi.CreateIamResourceRoles(conn.ctx).IamResourceRoles(*o)
	...
	return resourceIamResourceRolesRead(d, meta)
}

func resourceIamResourceRolesRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)

	r := conn.ApiClient.IamApi.GetIamResourceRolesByMoid(conn.ctx, d.Id())
	s, _, err := r.Execute()
	...
	if err := d.Set("resource", flattenMapMoBaseMoRelationship(s.Resource, d)); err != nil {
		return err
	}
...
func resourceIamResourceRolesUpdate(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewIamResourceRoles()
	...
	r := conn.ApiClient.IamApi.UpdateIamResourceRoles(conn.ctx, d.Id()).IamResourceRoles(*o)
	result, _, err := r.Execute()
	if err != nil {
...
func resourceIamResourceRolesDelete(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)

	r := conn.ApiClient.IamApi.DeleteIamResourceRoles(conn.ctx, d.Id())
	_, err := r.Execute()
	if err != nil {
