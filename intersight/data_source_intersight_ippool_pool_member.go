...
func dataSourceIppoolPoolMember() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceIppoolPoolMemberRead,
...
func dataSourceIppoolPoolMemberRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.IppoolApi.GetIppoolPoolMemberList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.IppoolPoolMemberList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to IppoolPoolMember: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			...
			if err := d.Set("ip_v4_block", flattenMapIppoolShadowBlockRelationship(s.IpV4Block, d)); err != nil {
			...
			if err := d.Set("peer", flattenMapIppoolIpLeaseRelationship(s.Peer, d)); err != nil {
				return fmt.Errorf("error occurred while setting property Peer: %+v", err)
			}

			if err := d.Set("pool", flattenMapIppoolShadowPoolRelationship(s.Pool, d)); err != nil {
