...
func dataSourceTerminalAuditLogRead(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewTerminalAuditLog()
	...
	result, _, err := conn.ApiClient.TerminalApi.GetTerminalAuditLogList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewTerminalAuditLog()
			...
			if err := d.Set("device_registration", flattenMapAssetDeviceConnectionRelationship(s.DeviceRegistration, d)); err != nil {
				return err
			}
