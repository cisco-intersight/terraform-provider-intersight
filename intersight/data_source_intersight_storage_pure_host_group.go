...
func dataSourceStoragePureHostGroup() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceStoragePureHostGroupRead,
		Schema: map[string]*schema.Schema{
			"description": {
				Description: "Short description about the host group.",
			...
			"host_names": {
				...
				Elem: &schema.Schema{
					Type: schema.TypeString}},
			"hosts": {
				Description: "A collection of references to the [storage.PureHost](mo://storage.PureHost) Managed Object.When this managed object is deleted, the referenced [storage.PureHost](mo://storage.PureHost) MOs unset their reference to this deleted MO.",
				Type:        schema.TypeList,
				Optional:    true,
				Computed:    true,
			...
			"name": {
				Description: "Name of the host group in storage array.",
				Type:        schema.TypeString,
				Optional:    true,
				Computed:    true,
			},
			...
			"protection_group": {
				Description: "A collection of references to the [storage.PureProtectionGroup](mo://storage.PureProtectionGroup) Managed Object.When this managed object is deleted, the referenced [storage.PureProtectionGroup](mo://storage.PureProtectionGroup) MO unsets its reference to this deleted MO.",
				Type:        schema.TypeList,
				MaxItems:    1,
				Optional:    true,
				Computed:    true,
...
func dataSourceStoragePureHostGroupRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)

	url := "storage/PureHostGroups"
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s models.StoragePureHostGroup
