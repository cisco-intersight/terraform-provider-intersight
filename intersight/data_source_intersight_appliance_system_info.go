...
func dataSourceApplianceSystemInfoRead(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewApplianceSystemInfo()
	...
	result, _, err := conn.ApiClient.ApplianceApi.GetApplianceSystemInfoList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewApplianceSystemInfo()
