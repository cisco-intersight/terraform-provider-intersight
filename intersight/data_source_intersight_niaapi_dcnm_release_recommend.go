...
func dataSourceNiaapiDcnmReleaseRecommendRead(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewNiaapiDcnmReleaseRecommendWithDefaults()
	...
	res, _, err := conn.ApiClient.NiaapiApi.GetNiaapiDcnmReleaseRecommendList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to NiaapiDcnmReleaseRecommend: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewNiaapiDcnmReleaseRecommendWithDefaults()
