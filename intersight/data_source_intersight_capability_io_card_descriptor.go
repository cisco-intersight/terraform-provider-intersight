...
func dataSourceCapabilityIoCardDescriptor() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceCapabilityIoCardDescriptorRead,
...
func dataSourceCapabilityIoCardDescriptorRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.CapabilityApi.GetCapabilityIoCardDescriptorList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.CapabilityIoCardDescriptorList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to CapabilityIoCardDescriptor: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewCapabilityIoCardDescriptorWithDefaults()
			...
			if err := d.Set("num_hif_ports", (s.NumHifPorts)); err != nil {
