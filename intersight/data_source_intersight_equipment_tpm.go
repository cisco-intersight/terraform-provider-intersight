...
func dataSourceEquipmentTpm() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceEquipmentTpmRead,
...
func dataSourceEquipmentTpmRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewEquipmentTpmWithDefaults()
	if v, ok := d.GetOk("activation_status"); ok {
		x := (v.(string))
		o.SetActivationStatus(x)
	}
	...
	res, _, err := conn.ApiClient.EquipmentApi.GetEquipmentTpmList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.EquipmentTpmList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to EquipmentTpm: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewEquipmentTpmWithDefaults()
			oo, _ := json.Marshal(r.Index(i).Interface())
			if err = json.Unmarshal(oo, s); err != nil {
				return fmt.Errorf("error occurred while unmarshalling result at index %+v: %+v", i, err)
			}
			if err := d.Set("activation_status", (s.ActivationStatus)); err != nil {
			...
			if err := d.Set("ownership", (s.Ownership)); err != nil {
				return fmt.Errorf("error occurred while setting property Ownership: %+v", err)
			}
			...
			if err := d.Set("tpm_id", (s.TpmId)); err != nil {
				return fmt.Errorf("error occurred while setting property TpmId: %+v", err)
			}
