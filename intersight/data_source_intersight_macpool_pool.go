...
func dataSourceMacpoolPool() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceMacpoolPoolRead,
...
func dataSourceMacpoolPoolRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.MacpoolApi.GetMacpoolPoolList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.MacpoolPoolList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to MacpoolPool: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewMacpoolPoolWithDefaults()
			...
			if err := d.Set("block_heads", flattenListMacpoolIdBlockRelationship(s.BlockHeads, d)); err != nil {
			...
			if err := d.Set("mac_blocks", flattenListMacpoolBlock(s.MacBlocks, d)); err != nil {
