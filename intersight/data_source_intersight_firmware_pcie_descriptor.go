...
func dataSourceFirmwarePcieDescriptor() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceFirmwarePcieDescriptorRead,
...
func dataSourceFirmwarePcieDescriptorRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.FirmwareApi.GetFirmwarePcieDescriptorList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.FirmwarePcieDescriptorList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to FirmwarePcieDescriptor: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewFirmwarePcieDescriptorWithDefaults()
