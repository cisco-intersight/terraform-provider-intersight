...
func dataSourceHyperflexCiscoHypervisorManager() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceHyperflexCiscoHypervisorManagerRead,
...
func dataSourceHyperflexCiscoHypervisorManagerRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.HyperflexApi.GetHyperflexCiscoHypervisorManagerList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.HyperflexCiscoHypervisorManagerList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to HyperflexCiscoHypervisorManager: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewHyperflexCiscoHypervisorManagerWithDefaults()
