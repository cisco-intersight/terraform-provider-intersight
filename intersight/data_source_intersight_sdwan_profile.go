...
func dataSourceSdwanProfileRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.SdwanApi.GetSdwanProfileList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.SdwanProfileList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to SdwanProfile: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewSdwanProfileWithDefaults()
			...
			if err := d.Set("router_nodes", flattenListSdwanRouterNodeRelationship(s.RouterNodes, d)); err != nil {
				return fmt.Errorf("error occurred while setting property RouterNodes: %+v", err)
			}

			if err := d.Set("router_policy", flattenMapSdwanRouterPolicyRelationship(s.RouterPolicy, d)); err != nil {
			...
			if err := d.Set("vmanage_account", flattenMapSdwanVmanageAccountPolicyRelationship(s.VmanageAccount, d)); err != nil {
