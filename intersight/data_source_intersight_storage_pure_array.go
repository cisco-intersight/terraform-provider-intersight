...
func dataSourceStoragePureArray() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceStoragePureArrayRead,
...
func dataSourceStoragePureArrayRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewStoragePureArrayWithDefaults()
	...
	res, _, err := conn.ApiClient.StorageApi.GetStoragePureArrayList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.StoragePureArrayList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to StoragePureArray: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewStoragePureArrayWithDefaults()
