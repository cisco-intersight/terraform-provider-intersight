...
func dataSourceTamAdvisoryInstance() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"advisory": {
				Description: "A reference to a tamAdvisory resource.\nWhen the $expand query parameter is specified, the referenced resource is returned inline.",
...
func dataSourceTamAdvisoryInstanceRead(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewTamAdvisoryInstance()
	...
	result, _, err := conn.ApiClient.TamApi.GetTamAdvisoryInstanceList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewTamAdvisoryInstance()
			oo, _ := json.Marshal(r.Index(i).Interface())
			if err = json.Unmarshal(oo, s); err != nil {
				return err
			}

			if err := d.Set("advisory", flattenMapTamAdvisoryRelationship(s.Advisory, d)); err != nil {
				return err
			}

			if err := d.Set("affected_object", flattenMapMoBaseMoRelationship(s.AffectedObject, d)); err != nil {
				return err
			}
