...
func dataSourceVirtualizationVmwareDatacenter() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"hypervisor_manager": {
				Description: "Hypervisor to which this datacenter belongs. All entities are owned by the hypervisor.",
...
func dataSourceVirtualizationVmwareDatacenterRead(d *schema.ResourceData, meta interface{}) error {
	...
	url := "virtualization/VmwareDatacenters"
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			...
			if err := d.Set("hypervisor_manager", flattenMapVirtualizationVmwareVcenterRef(s.HypervisorManager, d)); err != nil {
				return err
			}
