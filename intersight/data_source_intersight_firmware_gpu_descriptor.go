...
func dataSourceFirmwareGpuDescriptor() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceFirmwareGpuDescriptorRead,
...
func dataSourceFirmwareGpuDescriptorRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.FirmwareApi.GetFirmwareGpuDescriptorList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.FirmwareGpuDescriptorList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to FirmwareGpuDescriptor: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewFirmwareGpuDescriptorWithDefaults()
