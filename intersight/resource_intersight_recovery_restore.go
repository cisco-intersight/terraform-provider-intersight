...
func resourceRecoveryRestoreCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("backup_info"); ok {
		p := make([]models.RecoveryAbstractBackupInfoRelationship, 0, 1)
		s := v.([]interface{})
		for i := 0; i < len(s); i++ {
			...
			p = append(p, models.MoMoRefAsRecoveryAbstractBackupInfoRelationship(o))
		}
		if len(p) > 0 {
			x := p[0]
			o.SetBackupInfo(x)
	...
	o.SetObjectType("recovery.Restore")
