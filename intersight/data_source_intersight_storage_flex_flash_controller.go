...
func dataSourceStorageFlexFlashController() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"controller_state": {
				Description: "State of the Flex Flash Storage Controller.",
				Type:        schema.TypeString,
				Optional:    true,
				Computed:    true,
			},
			...
			"ff_controller_id": {
				Description: "Identifier for the Flex Flash Storage Controller.",
				Type:        schema.TypeString,
				Optional:    true,
				Computed:    true,
			},
			"flex_flash_controller_props": {
				Description: "An array of relationships to storageFlexFlashControllerProps resources.",
				Type:        schema.TypeList,
				Optional:    true,
				Computed:    true,
				Elem: &schema.Resource{
			...
			"flex_flash_physical_drives": {
				Description: "An array of relationships to storageFlexFlashPhysicalDrive resources.",
				Type:        schema.TypeList,
				Optional:    true,
				Computed:    true,
				Elem: &schema.Resource{
			...
			"flex_flash_virtual_drives": {
				Description: "An array of relationships to storageFlexFlashVirtualDrive resources.",
				Type:        schema.TypeList,
				Optional:    true,
				Computed:    true,
				Elem: &schema.Resource{
...
func dataSourceStorageFlexFlashControllerRead(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewStorageFlexFlashControllerWithDefaults()
	if v, ok := d.GetOk("class_id"); ok {
		x := (v.(string))
		o.SetClassId(x)
	}
	if v, ok := d.GetOk("controller_state"); ok {
		x := (v.(string))
		o.SetControllerState(x)
	}
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewStorageFlexFlashControllerWithDefaults()
			...
			if err := d.Set("controller_state", (s.ControllerState)); err != nil {
			...
			if err := d.Set("flex_flash_controller_props", flattenListStorageFlexFlashControllerPropsRelationship(s.FlexFlashControllerProps, d)); err != nil {
				return fmt.Errorf("error occurred while setting property FlexFlashControllerProps: %+v", err)
			}

			if err := d.Set("flex_flash_physical_drives", flattenListStorageFlexFlashPhysicalDriveRelationship(s.FlexFlashPhysicalDrives, d)); err != nil {
				return fmt.Errorf("error occurred while setting property FlexFlashPhysicalDrives: %+v", err)
			}

			if err := d.Set("flex_flash_virtual_drives", flattenListStorageFlexFlashVirtualDriveRelationship(s.FlexFlashVirtualDrives, d)); err != nil {
