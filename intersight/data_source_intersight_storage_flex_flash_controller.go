...
func dataSourceStorageFlexFlashControllerRead(d *schema.ResourceData, meta interface{}) error {
	...
	url := "storage/FlexFlashControllers"
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			...
			if err := d.Set("flex_flash_controller_props", flattenListStorageFlexFlashControllerPropsRef(s.FlexFlashControllerProps, d)); err != nil {
				return err
			}

			if err := d.Set("flex_flash_physical_drives", flattenListStorageFlexFlashPhysicalDriveRef(s.FlexFlashPhysicalDrives, d)); err != nil {
				return err
			}

			if err := d.Set("flex_flash_virtual_drives", flattenListStorageFlexFlashVirtualDriveRef(s.FlexFlashVirtualDrives, d)); err != nil {
				return err
			}
