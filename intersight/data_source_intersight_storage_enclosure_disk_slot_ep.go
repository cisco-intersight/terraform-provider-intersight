...
func dataSourceStorageEnclosureDiskSlotEp() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceStorageEnclosureDiskSlotEpRead,
...
func dataSourceStorageEnclosureDiskSlotEpRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewStorageEnclosureDiskSlotEpWithDefaults()
	...
	res, _, err := conn.ApiClient.StorageApi.GetStorageEnclosureDiskSlotEpList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.StorageEnclosureDiskSlotEpList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to StorageEnclosureDiskSlotEp: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewStorageEnclosureDiskSlotEpWithDefaults()
