...
func dataSourceIamEndPointUserPolicy() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceIamEndPointUserPolicyRead,
...
func dataSourceIamEndPointUserPolicyRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.IamApi.GetIamEndPointUserPolicyList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.IamEndPointUserPolicyList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to IamEndPointUserPolicy: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewIamEndPointUserPolicyWithDefaults()
