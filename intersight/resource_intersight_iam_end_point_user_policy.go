...
func resourceIamEndPointUserPolicyCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("password_properties"); ok {
		...
		if len(v.([]interface{})) > 0 {
			o := models.IamEndPointPasswordProperties{}
			l := (v.([]interface{})[0]).(map[string]interface{})
			if v, ok := l["additional_properties"]; ok {
				{
					...
					if err == nil && x1 != nil {
						o.IamEndPointPasswordPropertiesAO1P1.IamEndPointPasswordPropertiesAO1P1 = x1.(map[string]interface{})
					}
				}
			}
			...
			if v, ok := l["enable_password_expiry"]; ok {
				{
					x := (v.(bool))
					o.EnablePasswordExpiry = &x
				}
			}
			if v, ok := l["enforce_strong_password"]; ok {
				{
					x := (v.(bool))
					o.EnforceStrongPassword = &x
				}
			}
			if v, ok := l["force_send_password"]; ok {
				{
					x := (v.(bool))
					o.ForceSendPassword = &x
				}
			}
...
func detachIamEndPointUserPolicyProfiles(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("%v", meta)
	conn := meta.(*Config)
	url := "iam/EndPointUserPolicies" + "/" + d.Id()
	var o models.IamEndPointUserPolicy
...
func resourceIamEndPointUserPolicyRead(d *schema.ResourceData, meta interface{}) error {
	...
	var s models.IamEndPointUserPolicy
	err = s.UnmarshalJSON(body)
	if err != nil {
	...
	if err := d.Set("end_point_user_roles", flattenListIamEndPointUserRoleRef(s.EndPointUserRoles, d)); err != nil {
		return err
	}
...
func resourceIamEndPointUserPolicyDelete(d *schema.ResourceData, meta interface{}) error {
	...
	conn := meta.(*Config)
	url := "iam/EndPointUserPolicies" + "/" + d.Id()
	detachIamEndPointUserPolicyProfiles(d, meta)
