...
func resourceConfigExporter() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"export_candidates": {
				Description: "Reference to export candidates.",
			...
			"items": {
				Description: "List of MOs to be exported.",
				Type:        schema.TypeList,
				Optional:    true,
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
...
func resourceConfigExporterCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("export_candidates"); ok {
		x := make([]*models.ConfigExportCandidateRef, 0)
		switch reflect.TypeOf(v).Kind() {
		case reflect.Slice:
			s := reflect.ValueOf(v)
			for i := 0; i < s.Len(); i++ {
				o := models.ConfigExportCandidateRef{}
		...
		o.ExportCandidates = x

	}

	if v, ok := d.GetOk("exported_items"); ok {
		x := make([]*models.ConfigExportedItemRef, 0)
		switch reflect.TypeOf(v).Kind() {
		case reflect.Slice:
			s := reflect.ValueOf(v)
			for i := 0; i < s.Len(); i++ {
				o := models.ConfigExportedItemRef{}
				l := s.Index(i).Interface().(map[string]interface{})
				if v, ok := l["moid"]; ok {
					{
						x := (v.(string))
				...
				if v, ok := l["additional_properties"]; ok {
					{
						...
						if err == nil && x1 != nil {
							o.ConfigMoRefAO1P1.ConfigMoRefAO1P1 = x1.(map[string]interface{})
						}
					}
				}
...
func resourceConfigExporterRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err := d.Set("export_candidates", flattenListConfigExportCandidateRef(s.ExportCandidates, d)); err != nil {
		return err
	}

	if err := d.Set("exported_items", flattenListConfigExportedItemRef(s.ExportedItems, d)); err != nil {
		return err
	}
