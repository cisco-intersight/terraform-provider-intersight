...
func dataSourceFirmwareServerConfigurationUtilityDistributable() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceFirmwareServerConfigurationUtilityDistributableRead,
...
func dataSourceFirmwareServerConfigurationUtilityDistributableRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.FirmwareApi.GetFirmwareServerConfigurationUtilityDistributableList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to FirmwareServerConfigurationUtilityDistributable: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewFirmwareServerConfigurationUtilityDistributableWithDefaults()
