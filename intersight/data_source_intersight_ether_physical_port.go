...
func dataSourceEtherPhysicalPortRead(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewEtherPhysicalPort()
	...
	result, _, err := conn.ApiClient.EtherApi.GetEtherPhysicalPortList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewEtherPhysicalPort()
			...
			if err := d.Set("port_sub_group", flattenMapPortSubGroupRelationship(s.PortSubGroup, d)); err != nil {
				return err
			}
