...
func dataSourceHyperflexCapabilityInfo() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceHyperflexCapabilityInfoRead,
...
func dataSourceHyperflexCapabilityInfoRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.HyperflexApi.GetHyperflexCapabilityInfoList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.HyperflexCapabilityInfoList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to HyperflexCapabilityInfo: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewHyperflexCapabilityInfoWithDefaults()
