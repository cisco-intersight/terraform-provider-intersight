...
func dataSourceGraphicsController() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceGraphicsControllerRead,
		Schema: map[string]*schema.Schema{
			...
			"controller_id": {
				Description: "The id of the graphics controller.",
				Type:        schema.TypeInt,
				Optional:    true,
				Computed:    true,
			},
			...
			"pci_addr": {
				Description: "The PCI address of the graphics controller.",
				Type:        schema.TypeString,
				Optional:    true,
				Computed:    true,
			},
			"pci_slot": {
				Description: "The PCI slot information of the graphics controller.",
				Type:        schema.TypeString,
				Optional:    true,
				Computed:    true,
			},
...
func dataSourceGraphicsControllerRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewGraphicsControllerWithDefaults()
	...
	res, _, err := conn.ApiClient.GraphicsApi.GetGraphicsControllerList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.GraphicsControllerList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to GraphicsController: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewGraphicsControllerWithDefaults()
