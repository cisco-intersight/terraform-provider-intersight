...
func resourceIamCertificateRequest() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"certificate": {
				Description: "A collection of references to the [iam.Certificate](mo://iam.Certificate) Managed Object.When this managed object is deleted, the referenced [iam.Certificate](mo://iam.Certificate) MO on the other side of the relationship is deleted.",
				Type:        schema.TypeList,
				MaxItems:    1,
				Optional:    true,
				Elem: &schema.Resource{
			...
			"private_key_spec": {
				Description: "A collection of references to the [iam.PrivateKeySpec](mo://iam.PrivateKeySpec) Managed Object.When this managed object is deleted, the referenced [iam.PrivateKeySpec](mo://iam.PrivateKeySpec) MO on the other side of the relationship is deleted.",
				Type:        schema.TypeList,
				MaxItems:    1,
				Optional:    true,
				Elem: &schema.Resource{
...
func resourceIamCertificateRequestCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("certificate"); ok {
		p := models.IamCertificateRef{}
		if len(v.([]interface{})) > 0 {
			o := models.IamCertificateRef{}
			l := (v.([]interface{})[0]).(map[string]interface{})
			if v, ok := l["moid"]; ok {
				{
					x := (v.(string))
	...
	if v, ok := d.GetOk("private_key_spec"); ok {
		p := models.IamPrivateKeySpecRef{}
		if len(v.([]interface{})) > 0 {
			o := models.IamPrivateKeySpecRef{}
			l := (v.([]interface{})[0]).(map[string]interface{})
			if v, ok := l["moid"]; ok {
				{
					x := (v.(string))
			...
			if v, ok := l["additional_properties"]; ok {
				{
					...
					err := json.Unmarshal(x, &x1)
					if err == nil && x1 != nil {
						o.PkixSubjectAlternateNameAO1P1.PkixSubjectAlternateNameAO1P1 = x1.(map[string]interface{})
					}
				}
			}
...
func resourceIamCertificateRequestRead(d *schema.ResourceData, meta interface{}) error {
	...
	url := "iam/CertificateRequests" + "/" + d.Id()

	body, err := conn.SendGetRequest(url, []byte(""))
	if err != nil {
		return err
	}
	var s models.IamCertificateRequest
	...
	if err := d.Set("certificate", flattenMapIamCertificateRef(s.Certificate, d)); err != nil {
		return err
	}
	...
	if err := d.Set("private_key_spec", flattenMapIamPrivateKeySpecRef(s.PrivateKeySpec, d)); err != nil {
		return err
	}
