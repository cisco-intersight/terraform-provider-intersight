...
func dataSourceStoragePureVolumeSnapshot() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"protection_group_snapshot": {
				Description: "A reference to a storageProtectionGroupSnapshot resource.\nWhen the $expand query parameter is specified, the referenced resource is returned inline.",
				Type:        schema.TypeList,
				MaxItems:    1,
				Optional:    true,
				Computed:    true,
			...
			"volume": {
				Description: "A reference to a storageVolume resource.\nWhen the $expand query parameter is specified, the referenced resource is returned inline.",
				Type:        schema.TypeList,
				MaxItems:    1,
				Optional:    true,
				Computed:    true,
...
func dataSourceStoragePureVolumeSnapshotRead(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewStoragePureVolumeSnapshot()
	...
	result, _, err := conn.ApiClient.StorageApi.GetStoragePureVolumeSnapshotList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewStoragePureVolumeSnapshot()
			...
			if err := d.Set("protection_group_snapshot", flattenMapStorageProtectionGroupSnapshotRelationship(s.ProtectionGroupSnapshot, d)); err != nil {
				return err
			}
			...
			if err := d.Set("volume", flattenMapStorageVolumeRelationship(s.Volume, d)); err != nil {
				return err
			}
			d.SetId(s.GetMoid())
		}
