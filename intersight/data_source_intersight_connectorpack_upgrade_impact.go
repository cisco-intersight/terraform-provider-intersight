...
func dataSourceConnectorpackUpgradeImpact() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"ucsd_info": {
				Description: "Relationship to UCS Director for which connector packs are to be estimated.",
				Type:        schema.TypeList,
				MaxItems:    1,
				Optional:    true,
				Elem: &schema.Resource{
...
func dataSourceConnectorpackUpgradeImpactRead(d *schema.ResourceData, meta interface{}) error {
	...
	url := "connectorpack/UpgradeImpacts"
	...
	if v, ok := d.GetOk("is_eligible_for_upgrade"); ok {
		x := (v.(bool))
		o.IsEligibleForUpgrade = &x
	}
	if v, ok := d.GetOk("is_update_downloaded"); ok {
		x := (v.(bool))
		o.IsUpdateDownloaded = &x
	}
