...
func dataSourceFirmwareHbaDescriptor() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceFirmwareHbaDescriptorRead,
...
func dataSourceFirmwareHbaDescriptorRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.FirmwareApi.GetFirmwareHbaDescriptorList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.FirmwareHbaDescriptorList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to FirmwareHbaDescriptor: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewFirmwareHbaDescriptorWithDefaults()
