...
func resourceRecoveryOnDemandBackupCreate(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewRecoveryOnDemandBackup()
	...
	if v, ok := d.GetOk("config_result"); ok {
		...
		l := (v.([]interface{})[0]).(map[string]interface{})
		{
			...
			o.SetObjectType("recovery.ConfigResult")
			...
			p = append(p, o.AsRecoveryConfigResultRelationship())
		}
		x := p[0]
		o.SetConfigResult(x)
	}
...
func resourceRecoveryOnDemandBackupDelete(d *schema.ResourceData, meta interface{}) error {
	...
	r := conn.ApiClient.RecoveryApi.DeleteRecoveryOnDemandBackup(conn.ctx, d.Id())
	_, err := r.Execute()
	if err != nil {
