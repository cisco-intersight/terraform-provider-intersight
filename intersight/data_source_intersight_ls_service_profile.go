...
func dataSourceLsServiceProfile() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceLsServiceProfileRead,
...
func dataSourceLsServiceProfileRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewLsServiceProfileWithDefaults()
	if v, ok := d.GetOk("assign_state"); ok {
		x := (v.(string))
		o.SetAssignState(x)
	}
	if v, ok := d.GetOk("assoc_state"); ok {
		x := (v.(string))
		o.SetAssocState(x)
	}
	...
	res, _, err := conn.ApiClient.LsApi.GetLsServiceProfileList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.LsServiceProfileList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to LsServiceProfile: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewLsServiceProfileWithDefaults()
			...
			if err := d.Set("assign_state", (s.GetAssignState())); err != nil {
				return fmt.Errorf("error occurred while setting property AssignState: %+v", err)
			}
			if err := d.Set("assoc_state", (s.GetAssocState())); err != nil {
				return fmt.Errorf("error occurred while setting property AssocState: %+v", err)
			}
			if err := d.Set("associated_server", (s.GetAssociatedServer())); err != nil {
