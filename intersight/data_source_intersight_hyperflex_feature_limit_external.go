...
func dataSourceHyperflexFeatureLimitExternalRead(d *schema.ResourceData, meta interface{}) error {
	...
	result, _, err := conn.ApiClient.HyperflexApi.GetHyperflexFeatureLimitExternalList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewHyperflexFeatureLimitExternal()
