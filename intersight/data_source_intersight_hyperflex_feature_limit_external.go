...
func dataSourceHyperflexFeatureLimitExternal() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceHyperflexFeatureLimitExternalRead,
...
func dataSourceHyperflexFeatureLimitExternalRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.HyperflexApi.GetHyperflexFeatureLimitExternalList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.HyperflexFeatureLimitExternalList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to HyperflexFeatureLimitExternal: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewHyperflexFeatureLimitExternalWithDefaults()
