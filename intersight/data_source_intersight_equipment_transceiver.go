...
func dataSourceEquipmentTransceiver() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceEquipmentTransceiverRead,
...
func dataSourceEquipmentTransceiverRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewEquipmentTransceiverWithDefaults()
	...
	res, _, err := conn.ApiClient.EquipmentApi.GetEquipmentTransceiverList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.EquipmentTransceiverList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to EquipmentTransceiver: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewEquipmentTransceiverWithDefaults()
			...
			if err := d.Set("ether_physical_port", flattenMapEtherPhysicalPortRelationship(s.GetEtherPhysicalPort(), d)); err != nil {
				return fmt.Errorf("error occurred while setting property EtherPhysicalPort: %+v", err)
			}
