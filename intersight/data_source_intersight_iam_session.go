...
func dataSourceIamSessionRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewIamSessionWithDefaults()
	...
	res, _, err := conn.ApiClient.IamApi.GetIamSessionList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to IamSession: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewIamSessionWithDefaults()
			oo, _ := json.Marshal(r.Index(i).Interface())
			if err = json.Unmarshal(oo, s); err != nil {
				return fmt.Errorf("error occurred while unmarshalling result at index %+v: %+v", i, err)
			}

			if err := d.Set("account_permissions", flattenListIamAccountPermissions(s.AccountPermissions, d)); err != nil {
			...
			if err := d.Set("expiration", (s.Expiration).String()); err != nil {
				return fmt.Errorf("error occurred while setting property Expiration: %+v", err)
			}

			if err := d.Set("idle_time_expiration", (s.IdleTimeExpiration).String()); err != nil {
				return fmt.Errorf("error occurred while setting property IdleTimeExpiration: %+v", err)
			}
