...
func resourceResourceGroup() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"organizations": {
				Description: "A collection of references to the [organization.Organization](mo://organization.Organization) Managed Object.When this managed object is deleted, the referenced [organization.Organization](mo://organization.Organization) MOs unset their reference to this deleted MO.",
				Type:        schema.TypeList,
				Optional:    true,
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
...
func resourceResourceGroupCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("organizations"); ok {
		x := make([]*models.OrganizationOrganizationRef, 0)
		...
		case reflect.Slice:
			...
			for i := 0; i < s.Len(); i++ {
				...
				if v, ok := l["additional_properties"]; ok {
					{
						...
						if err == nil && x1 != nil {
							o.ResourcePerTypeCombinedSelectorAO1P1.ResourcePerTypeCombinedSelectorAO1P1 = x1.(map[string]interface{})
						}
					}
				}
				...
				if v, ok := l["selector_object_type"]; ok {
					{
						x := (v.(string))
						o.SelectorObjectType = x
					}
				}
				x = append(x, &o)
			}
		}
		o.PerTypeCombinedSelector = x
				...
				if v, ok := l["additional_properties"]; ok {
					{
						...
						err := json.Unmarshal(x, &x1)
						if err == nil && x1 != nil {
							o.ResourceSelectorAO1P1.ResourceSelectorAO1P1 = x1.(map[string]interface{})
						}
					}
				}
				if v, ok := l["object_type"]; ok {
...
func resourceResourceGroupRead(d *schema.ResourceData, meta interface{}) error {
	...
	url := "resource/Groups" + "/" + d.Id()

	body, err := conn.SendGetRequest(url, []byte(""))
	if err != nil {
		return err
	}
	var s models.ResourceGroup
	...
	if err := d.Set("organizations", flattenListOrganizationOrganizationRef(s.Organizations, d)); err != nil {
		return err
	}
