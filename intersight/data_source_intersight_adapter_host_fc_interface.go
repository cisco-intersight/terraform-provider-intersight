...
func dataSourceAdapterHostFcInterface() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceAdapterHostFcInterfaceRead,
...
func dataSourceAdapterHostFcInterfaceRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)

	url := "adapter/HostFcInterfaces"
	var o models.AdapterHostFcInterface
	...
	if v, ok := d.GetOk("host_fc_interface_id"); ok {
		x := int64(v.(int))
		o.HostFcInterfaceID = x
	}
	...
	if v, ok := d.GetOk("original_wwnn"); ok {
		x := (v.(string))
		o.OriginalWwnn = x
	}
	if v, ok := d.GetOk("original_wwpn"); ok {
		x := (v.(string))
		o.OriginalWwpn = x
	}
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s models.AdapterHostFcInterface
