...
func dataSourceFabricConfigResult() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceFabricConfigResultRead,
...
func dataSourceFabricConfigResultRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewFabricConfigResultWithDefaults()
	...
	res, _, err := conn.ApiClient.FabricApi.GetFabricConfigResultList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.FabricConfigResultList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to FabricConfigResult: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewFabricConfigResultWithDefaults()
			...
			if err := d.Set("profile", flattenMapFabricSwitchProfileRelationship(s.GetProfile(), d)); err != nil {
				return fmt.Errorf("error occurred while setting property Profile: %+v", err)
			}

			if err := d.Set("result_entries", flattenListFabricConfigResultEntryRelationship(s.GetResultEntries(), d)); err != nil {
