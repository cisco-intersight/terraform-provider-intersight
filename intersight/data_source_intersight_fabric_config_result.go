...
func dataSourceFabricConfigResultRead(d *schema.ResourceData, meta interface{}) error {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			...
			if err := d.Set("profile", flattenMapFabricSwitchProfileRelationship(s.Profile, d)); err != nil {
				return fmt.Errorf("error occurred while setting property Profile: %+v", err)
			}

			if err := d.Set("result_entries", flattenListFabricConfigResultEntryRelationship(s.ResultEntries, d)); err != nil {
