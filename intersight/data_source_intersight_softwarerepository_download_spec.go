...
func dataSourceSoftwarerepositoryDownloadSpec() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceSoftwarerepositoryDownloadSpecRead,
...
func dataSourceSoftwarerepositoryDownloadSpecRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewSoftwarerepositoryDownloadSpecWithDefaults()
	...
	res, _, err := conn.ApiClient.SoftwarerepositoryApi.GetSoftwarerepositoryDownloadSpecList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to SoftwarerepositoryDownloadSpec: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewSoftwarerepositoryDownloadSpecWithDefaults()
			oo, _ := json.Marshal(r.Index(i).Interface())
			if err = json.Unmarshal(oo, s); err != nil {
				return fmt.Errorf("error occurred while unmarshalling result at index %+v: %+v", i, err)
			}
			if err := d.Set("auth_token", (s.GetAuthToken())); err != nil {
				return fmt.Errorf("error occurred while setting property AuthToken: %+v", err)
			}
