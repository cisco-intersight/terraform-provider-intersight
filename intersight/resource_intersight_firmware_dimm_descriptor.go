...
func resourceFirmwareDimmDescriptor() *schema.Resource {
	return &schema.Resource{
		Create: resourceFirmwareDimmDescriptorCreate,
		Read:   resourceFirmwareDimmDescriptorRead,
		Update: resourceFirmwareDimmDescriptorUpdate,
		Delete: resourceFirmwareDimmDescriptorDelete,
...
func resourceFirmwareDimmDescriptorCreate(d *schema.ResourceData, meta interface{}) error {
	...
	o.SetClassId("firmware.DimmDescriptor")
	...
	o.SetObjectType("firmware.DimmDescriptor")
	...
	r := conn.ApiClient.FirmwareApi.CreateFirmwareDimmDescriptor(conn.ctx).FirmwareDimmDescriptor(*o)
	...
	return resourceFirmwareDimmDescriptorRead(d, meta)
}

func resourceFirmwareDimmDescriptorRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)

	r := conn.ApiClient.FirmwareApi.GetFirmwareDimmDescriptorByMoid(conn.ctx, d.Id())
	s, _, err := r.Execute()
...
func resourceFirmwareDimmDescriptorUpdate(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewFirmwareDimmDescriptorWithDefaults()
	...
	r := conn.ApiClient.FirmwareApi.UpdateFirmwareDimmDescriptor(conn.ctx, d.Id()).FirmwareDimmDescriptor(*o)
	result, _, err := r.Execute()
	if err != nil {
...
func resourceFirmwareDimmDescriptorDelete(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	p := conn.ApiClient.FirmwareApi.DeleteFirmwareDimmDescriptor(conn.ctx, d.Id())
	_, err := p.Execute()
	if err != nil {
