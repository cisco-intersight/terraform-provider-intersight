...
func dataSourceConfigImporter() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceConfigImporterRead,
...
func dataSourceConfigImporterRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.ConfigApi.GetConfigImporterList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.ConfigImporterList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to ConfigImporter: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewConfigImporterWithDefaults()
