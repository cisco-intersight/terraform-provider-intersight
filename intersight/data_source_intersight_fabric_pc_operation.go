...
func dataSourceFabricPcOperation() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceFabricPcOperationRead,
...
func dataSourceFabricPcOperationRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.FabricApi.GetFabricPcOperationList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.FabricPcOperationList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to FabricPcOperation: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewFabricPcOperationWithDefaults()
