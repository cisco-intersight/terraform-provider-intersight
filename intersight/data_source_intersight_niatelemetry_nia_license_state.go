...
func dataSourceNiatelemetryNiaLicenseStateRead(d *schema.ResourceData, meta interface{}) error {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			...
			if err := d.Set("device", flattenMapNiatelemetryNiaInventoryRelationship(s.GetDevice(), d)); err != nil {
				return fmt.Errorf("error occurred while setting property Device: %+v", err)
			}
			if err := d.Set("feature_activated", (s.GetFeatureActivated())); err != nil {
				return fmt.Errorf("error occurred while setting property FeatureActivated: %+v", err)
			}
			if err := d.Set("license_activated", (s.GetLicenseActivated())); err != nil {
