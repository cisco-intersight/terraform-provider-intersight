...
func dataSourceNiatelemetryNiaLicenseState() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"device": {
				Description: "A collection of references to the [niatelemetry.NiaInventory](mo://niatelemetry.NiaInventory) Managed Object.When this managed object is deleted, the referenced [niatelemetry.NiaInventory](mo://niatelemetry.NiaInventory) MO unsets its reference to this deleted MO.",
...
func dataSourceNiatelemetryNiaLicenseStateRead(d *schema.ResourceData, meta interface{}) error {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			...
			if err := d.Set("device", flattenMapNiatelemetryNiaInventoryRef(s.Device, d)); err != nil {
				return err
			}
