...
func dataSourceConnectorpackConnectorPackUpgrade() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceConnectorpackConnectorPackUpgradeRead,
...
func dataSourceConnectorpackConnectorPackUpgradeRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.ConnectorpackApi.GetConnectorpackConnectorPackUpgradeList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.ConnectorpackConnectorPackUpgradeList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to ConnectorpackConnectorPackUpgrade: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewConnectorpackConnectorPackUpgradeWithDefaults()
