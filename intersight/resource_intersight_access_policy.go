...
func resourceAccessPolicyRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err := d.Set("inband_ip_pool", flattenMapIppoolPoolRelationship(s.InbandIpPool, d)); err != nil {
	...
	if err := d.Set("inband_vrf", flattenMapVrfVrfRelationship(s.InbandVrf, d)); err != nil {
...
func resourceAccessPolicyUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("inband_ip_pool") {
		v := d.Get("inband_ip_pool")
		p := make([]models.IppoolPoolRelationship, 0, 1)
		s := v.([]interface{})
		for i := 0; i < len(s); i++ {
			l := s[i].(map[string]interface{})
	...
	if d.HasChange("inband_vlan") {
		v := d.Get("inband_vlan")
		x := int64(v.(int))
		o.SetInbandVlan(x)
	}

	if d.HasChange("inband_vrf") {
		v := d.Get("inband_vrf")
		p := make([]models.VrfVrfRelationship, 0, 1)
		s := v.([]interface{})
		for i := 0; i < len(s); i++ {
