...
func dataSourceServerConfigResult() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceServerConfigResultRead,
		Schema: map[string]*schema.Schema{
			...
			"result_entries": {
				Description: "An array of relationships to serverConfigResultEntry resources.",
				Type:        schema.TypeList,
				Optional:    true,
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
...
func dataSourceServerConfigResultRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewServerConfigResultWithDefaults()
	...
	res, _, err := conn.ApiClient.ServerApi.GetServerConfigResultList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.ServerConfigResultList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to ServerConfigResult: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewServerConfigResultWithDefaults()
			...
			if err := d.Set("result_entries", flattenListServerConfigResultEntryRelationship(s.GetResultEntries(), d)); err != nil {
