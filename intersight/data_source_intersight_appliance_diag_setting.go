...
func dataSourceApplianceDiagSetting() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceApplianceDiagSettingRead,
...
func dataSourceApplianceDiagSettingRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.ApplianceApi.GetApplianceDiagSettingList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.ApplianceDiagSettingList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to ApplianceDiagSetting: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewApplianceDiagSettingWithDefaults()
