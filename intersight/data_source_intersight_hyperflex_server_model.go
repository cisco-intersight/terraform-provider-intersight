...
func dataSourceHyperflexServerModel() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceHyperflexServerModelRead,
...
func dataSourceHyperflexServerModelRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewHyperflexServerModelWithDefaults()
	...
	res, _, err := conn.ApiClient.HyperflexApi.GetHyperflexServerModelList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.HyperflexServerModelList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to HyperflexServerModel: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewHyperflexServerModelWithDefaults()
			...
			if err := d.Set("server_model_entries", flattenListHyperflexServerModelEntry(s.GetServerModelEntries(), d)); err != nil {
				return fmt.Errorf("error occurred while setting property ServerModelEntries: %+v", err)
			}
