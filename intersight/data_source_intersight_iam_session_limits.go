...
func dataSourceIamSessionLimits() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceIamSessionLimitsRead,
...
func dataSourceIamSessionLimitsRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.IamApi.GetIamSessionLimitsList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.IamSessionLimitsList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to IamSessionLimits: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewIamSessionLimitsWithDefaults()
			...
			if err := d.Set("maximum_limit", (s.MaximumLimit)); err != nil {
			...
			if err := d.Set("session_time_out", (s.SessionTimeOut)); err != nil {
