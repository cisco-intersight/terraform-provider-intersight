...
func dataSourceAdapterConfigPolicy() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceAdapterConfigPolicyRead,
...
func dataSourceAdapterConfigPolicyRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.AdapterApi.GetAdapterConfigPolicyList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.AdapterConfigPolicyList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to AdapterConfigPolicy: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewAdapterConfigPolicyWithDefaults()
			...
			if err := d.Set("settings", flattenListAdapterAdapterConfig(s.GetSettings(), d)); err != nil {
				return fmt.Errorf("error occurred while setting property Settings: %+v", err)
			}
