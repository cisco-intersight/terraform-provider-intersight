...
func resourceSolPolicy() *schema.Resource {
	return &schema.Resource{
		Create: resourceSolPolicyCreate,
		Read:   resourceSolPolicyRead,
		Update: resourceSolPolicyUpdate,
		Delete: resourceSolPolicyDelete,
...
func resourceSolPolicyCreate(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewSolPolicy()
	if v, ok := d.GetOk("baud_rate"); ok {
		x := int32(v.(int))
		o.SetBaudRate(x)
	}

	o.SetClassId("sol.Policy")
	...
	return resourceSolPolicyRead(d, meta)
}
func detachSolPolicyProfiles(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewSolPolicy()
...
func resourceSolPolicyRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	...
	if err := d.Set("com_port", (s.ComPort)); err != nil {
		return err
	}
	...
	if err := d.Set("ssh_port", (s.SshPort)); err != nil {
		return err
	}
...
func resourceSolPolicyUpdate(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewSolPolicy()
...
func resourceSolPolicyDelete(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	e := detachSolPolicyProfiles(d, meta)
	if e != nil {
		return e
	}

	r := conn.ApiClient.SolApi.DeleteSolPolicy(conn.ctx, d.Id())
	_, err := r.Execute()
	if err != nil {
