...
func dataSourceHyperflexNodeProfile() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceHyperflexNodeProfileRead,
...
func dataSourceHyperflexNodeProfileRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.HyperflexApi.GetHyperflexNodeProfileList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to HyperflexNodeProfile: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewHyperflexNodeProfileWithDefaults()
