...
func dataSourceMacpoolPoolMemberRead(d *schema.ResourceData, meta interface{}) error {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			...
			if err := d.Set("block_head", flattenMapMacpoolIdBlockRelationship(s.BlockHead, d)); err != nil {
			...
			if err := d.Set("peer", flattenMapMacpoolLeaseRelationship(s.Peer, d)); err != nil {
