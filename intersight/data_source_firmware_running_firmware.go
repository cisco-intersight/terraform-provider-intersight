...
func dataSourceFirmwareRunningFirmware() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceFirmwareRunningFirmwareRead,
		Schema: map[string]*schema.Schema{
			"bios_unit": {
				Description: "A collection of references to the [bios.Unit](mo://bios.Unit) Managed Object.When this managed object is deleted, the referenced [bios.Unit](mo://bios.Unit) MO unsets its reference to this deleted MO.",
...
func dataSourceFirmwareRunningFirmwareRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)

	url := "firmware/RunningFirmwares"
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			...
			if err := d.Set("bios_unit", flattenMapBiosUnitRef(s.BiosUnit, d)); err != nil {
				return err
			}
			...
			if err := d.Set("network_elements", flattenListNetworkElementRef(s.NetworkElements, d)); err != nil {
				return err
			}
