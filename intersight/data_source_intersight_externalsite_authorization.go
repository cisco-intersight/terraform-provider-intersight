...
func dataSourceExternalsiteAuthorization() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceExternalsiteAuthorizationRead,
...
func dataSourceExternalsiteAuthorizationRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.ExternalsiteApi.GetExternalsiteAuthorizationList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.ExternalsiteAuthorizationList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to ExternalsiteAuthorization: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewExternalsiteAuthorizationWithDefaults()
