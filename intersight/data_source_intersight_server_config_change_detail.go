...
func dataSourceServerConfigChangeDetail() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceServerConfigChangeDetailRead,
...
func dataSourceServerConfigChangeDetailRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewServerConfigChangeDetailWithDefaults()
	...
	res, _, err := conn.ApiClient.ServerApi.GetServerConfigChangeDetailList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.ServerConfigChangeDetailList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to ServerConfigChangeDetail: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewServerConfigChangeDetailWithDefaults()
