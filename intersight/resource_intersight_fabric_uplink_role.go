...
func resourceFabricUplinkRole() *schema.Resource {
	return &schema.Resource{
		Create: resourceFabricUplinkRoleCreate,
		Read:   resourceFabricUplinkRoleRead,
...
func resourceFabricUplinkRoleCreate(d *schema.ResourceData, meta interface{}) error {
	...
	o.SetObjectType("fabric.UplinkRole")
	...
	r := conn.ApiClient.FabricApi.CreateFabricUplinkRole(conn.ctx).FabricUplinkRole(*o)
	...
	return resourceFabricUplinkRoleRead(d, meta)
}

func resourceFabricUplinkRoleRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)

	r := conn.ApiClient.FabricApi.GetFabricUplinkRoleByMoid(conn.ctx, d.Id())
	s, _, err := r.Execute()
	...
	if err := d.Set("fec", (s.Fec)); err != nil {
...
func resourceFabricUplinkRoleUpdate(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewFabricUplinkRoleWithDefaults()
	...
	r := conn.ApiClient.FabricApi.UpdateFabricUplinkRole(conn.ctx, d.Id()).FabricUplinkRole(*o)
	result, _, err := r.Execute()
	if err != nil {
...
func resourceFabricUplinkRoleDelete(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	p := conn.ApiClient.FabricApi.DeleteFabricUplinkRole(conn.ctx, d.Id())
	_, err := p.Execute()
	if err != nil {
