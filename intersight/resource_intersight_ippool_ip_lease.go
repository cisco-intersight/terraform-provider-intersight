...
func resourceIppoolIpLease() *schema.Resource {
	return &schema.Resource{
		Create: resourceIppoolIpLeaseCreate,
		Read:   resourceIppoolIpLeaseRead,
		Delete: resourceIppoolIpLeaseDelete,
		Schema: map[string]*schema.Schema{
			...
			"ip_v4_config": {
				Description: "Netmask, Gateway and DNS settings.",
				Type:        schema.TypeList,
				MaxItems:    1,
				Optional:    true,
				Computed:    true,
...
func resourceIppoolIpLeaseCreate(d *schema.ResourceData, meta interface{}) error {
	...
	o.SetObjectType("ippool.IpLease")
	...
	if v, ok := d.GetOk("pool_member"); ok {
		p := make([]models.IppoolPoolMemberRelationship, 0, 1)
		s := v.([]interface{})
		for i := 0; i < len(s); i++ {
			l := s[i].(map[string]interface{})
			o := models.NewMoMoRefWithDefaults()
			...
			p = append(p, models.MoMoRefAsIppoolPoolMemberRelationship(o))
		}
		if len(p) > 0 {
			x := p[0]
			o.SetPoolMember(x)
	...
	if v, ok := d.GetOk("universe"); ok {
		p := make([]models.IppoolUniverseRelationship, 0, 1)
		s := v.([]interface{})
		for i := 0; i < len(s); i++ {
			l := s[i].(map[string]interface{})
			o := models.NewMoMoRefWithDefaults()
			...
			p = append(p, models.MoMoRefAsIppoolUniverseRelationship(o))
		}
		if len(p) > 0 {
			x := p[0]
			o.SetUniverse(x)
	...
	r := conn.ApiClient.IppoolApi.CreateIppoolIpLease(conn.ctx).IppoolIpLease(*o)
	...
	return resourceIppoolIpLeaseRead(d, meta)
}

func resourceIppoolIpLeaseRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)

	r := conn.ApiClient.IppoolApi.GetIppoolIpLeaseByMoid(conn.ctx, d.Id())
	s, _, err := r.Execute()
...
func resourceIppoolIpLeaseDelete(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	p := conn.ApiClient.IppoolApi.DeleteIppoolIpLease(conn.ctx, d.Id())
	_, err := p.Execute()
	if err != nil {
