...
func resourceVnicEthQosPolicy() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"rate_limit": {
				Description: "The value in Mbps (0-40000) to use for limiting the data rate on the virtual interface. Setting this to zero will turn rate limiting off.",
				Type:        schema.TypeInt,
				Optional:    true,
			},
			"tags": {
...
func resourceVnicEthQosPolicyRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err := d.Set("mtu", (s.Mtu)); err != nil {
		return err
	}
	...
	if err := d.Set("trust_host_cos", (s.TrustHostCos)); err != nil {
		return err
	}
