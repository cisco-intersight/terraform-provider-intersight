...
func dataSourceHyperflexHxapHost() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceHyperflexHxapHostRead,
...
func dataSourceHyperflexHxapHostRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewHyperflexHxapHostWithDefaults()
	...
	res, _, err := conn.ApiClient.HyperflexApi.GetHyperflexHxapHostList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to HyperflexHxapHost: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewHyperflexHxapHostWithDefaults()
			oo, _ := json.Marshal(r.Index(i).Interface())
			if err = json.Unmarshal(oo, s); err != nil {
				return fmt.Errorf("error occurred while unmarshalling result at index %+v: %+v", i, err)
			}
			if err := d.Set("age", (s.GetAge())); err != nil {
				return fmt.Errorf("error occurred while setting property Age: %+v", err)
			}
			...
			if err := d.Set("internal_ip_address", (s.GetInternalIpAddress())); err != nil {
				return fmt.Errorf("error occurred while setting property InternalIpAddress: %+v", err)
			}
			...
			if err := d.Set("management_ip_address", (s.GetManagementIpAddress())); err != nil {
				return fmt.Errorf("error occurred while setting property ManagementIpAddress: %+v", err)
			}
			if err := d.Set("master_role", (s.GetMasterRole())); err != nil {
				return fmt.Errorf("error occurred while setting property MasterRole: %+v", err)
			}
