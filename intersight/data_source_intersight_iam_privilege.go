...
func dataSourceIamPrivilege() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceIamPrivilegeRead,
...
func dataSourceIamPrivilegeRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewIamPrivilegeWithDefaults()
	...
	res, _, err := conn.ApiClient.IamApi.GetIamPrivilegeList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.IamPrivilegeList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to IamPrivilege: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewIamPrivilegeWithDefaults()
			...
			if err := d.Set("hostname_prefix", (s.GetHostnamePrefix())); err != nil {
				return fmt.Errorf("error occurred while setting property HostnamePrefix: %+v", err)
			}
			if err := d.Set("method", (s.GetMethod())); err != nil {
				return fmt.Errorf("error occurred while setting property Method: %+v", err)
			}
			...
			if err := d.Set("url_prefix", (s.GetUrlPrefix())); err != nil {
				return fmt.Errorf("error occurred while setting property UrlPrefix: %+v", err)
			}
			d.SetId(s.GetMoid())
		}
	}
