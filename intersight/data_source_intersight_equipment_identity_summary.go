...
func dataSourceEquipmentIdentitySummary() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceEquipmentIdentitySummaryRead,
...
func dataSourceEquipmentIdentitySummaryRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewEquipmentIdentitySummaryWithDefaults()
	...
	res, _, err := conn.ApiClient.EquipmentApi.GetEquipmentIdentitySummaryList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to EquipmentIdentitySummary: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewEquipmentIdentitySummaryWithDefaults()
