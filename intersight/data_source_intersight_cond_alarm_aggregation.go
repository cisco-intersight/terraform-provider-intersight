...
func dataSourceCondAlarmAggregation() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceCondAlarmAggregationRead,
...
func dataSourceCondAlarmAggregationRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewCondAlarmAggregationWithDefaults()
	...
	res, _, err := conn.ApiClient.CondApi.GetCondAlarmAggregationList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to CondAlarmAggregation: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewCondAlarmAggregationWithDefaults()
			...
			if err := d.Set("alarm_aggregation_source", flattenMapMoBaseMoRelationship(s.GetAlarmAggregationSource(), d)); err != nil {
				return fmt.Errorf("error occurred while setting property AlarmAggregationSource: %+v", err)
			}
			...
			if err := d.Set("critical_alarms_count", (s.GetCriticalAlarmsCount())); err != nil {
				return fmt.Errorf("error occurred while setting property CriticalAlarmsCount: %+v", err)
			}
			...
			if err := d.Set("info_alarms_count", (s.GetInfoAlarmsCount())); err != nil {
				return fmt.Errorf("error occurred while setting property InfoAlarmsCount: %+v", err)
			}
			...
			if err := d.Set("warning_alarms_count", (s.GetWarningAlarmsCount())); err != nil {
				return fmt.Errorf("error occurred while setting property WarningAlarmsCount: %+v", err)
			}
			d.SetId(s.GetMoid())
		}
	}
