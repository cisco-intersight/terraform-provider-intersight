...
func resourceMacpoolPoolCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("block_heads"); ok {
		x := make([]models.MacpoolIdBlockRelationship, 0)
		s := v.([]interface{})
		for i := 0; i < len(s); i++ {
			...
			x = append(x, models.MoMoRefAsMacpoolIdBlockRelationship(o))
		}
		if len(x) > 0 {
			o.SetBlockHeads(x)
		}
	...
	if v, ok := d.GetOk("mac_blocks"); ok {
		x := make([]models.MacpoolBlock, 0)
		s := v.([]interface{})
		for i := 0; i < len(s); i++ {
			o := models.NewMacpoolBlockWithDefaults()
			l := s[i].(map[string]interface{})
