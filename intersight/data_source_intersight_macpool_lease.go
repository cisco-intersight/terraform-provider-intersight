...
func dataSourceMacpoolLease() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceMacpoolLeaseRead,
...
func dataSourceMacpoolLeaseRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewMacpoolLeaseWithDefaults()
	...
	res, _, err := conn.ApiClient.MacpoolApi.GetMacpoolLeaseList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to MacpoolLease: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewMacpoolLeaseWithDefaults()
			...
			if err := d.Set("pool_member", flattenMapMacpoolPoolMemberRelationship(s.PoolMember, d)); err != nil {
			...
			if err := d.Set("universe", flattenMapMacpoolUniverseRelationship(s.Universe, d)); err != nil {
