...
func dataSourceSoftwareHyperflexDistributableRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.SoftwareApi.GetSoftwareHyperflexDistributableList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.SoftwareHyperflexDistributableList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to SoftwareHyperflexDistributable: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewSoftwareHyperflexDistributableWithDefaults()
