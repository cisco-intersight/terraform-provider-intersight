...
func dataSourceStoragePureHostRead(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewStoragePureHostWithDefaults()
	...
	res, _, err := conn.ApiClient.StorageApi.GetStoragePureHostList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.StoragePureHostList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to StoragePureHost: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewStoragePureHostWithDefaults()
			...
			if err := d.Set("host_group", flattenMapStoragePureHostGroupRelationship(s.HostGroup, d)); err != nil {
			...
			if err := d.Set("initiators", flattenListStorageBaseInitiator(s.Initiators, d)); err != nil {
			...
			if err := d.Set("storage_utilization", flattenMapStoragePureHostUtilization(s.StorageUtilization, d)); err != nil {
