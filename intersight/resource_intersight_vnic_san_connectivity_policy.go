...
func resourceVnicSanConnectivityPolicy() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"fc_ifs": {
				Description: "A collection of references to the [vnic.FcIf](mo://vnic.FcIf) Managed Object.When this managed object is deleted, the referenced [vnic.FcIf](mo://vnic.FcIf) MOs on the other side of the relationship are deleted.",
...
func resourceVnicSanConnectivityPolicyCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("fc_ifs"); ok {
		x := make([]*models.VnicFcIfRef, 0)
		switch reflect.TypeOf(v).Kind() {
		case reflect.Slice:
			s := reflect.ValueOf(v)
			for i := 0; i < s.Len(); i++ {
				o := models.VnicFcIfRef{}
	...
	url := "vnic/SanConnectivityPolicies"
...
func detachVnicSanConnectivityPolicyProfiles(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("%v", meta)
	conn := meta.(*Config)
	url := "vnic/SanConnectivityPolicies" + "/" + d.Id()
	var o models.VnicSanConnectivityPolicy
...
func resourceVnicSanConnectivityPolicyRead(d *schema.ResourceData, meta interface{}) error {
	...
	var s models.VnicSanConnectivityPolicy
	err = s.UnmarshalJSON(body)
	if err != nil {
	...
	if err := d.Set("fc_ifs", flattenListVnicFcIfRef(s.FcIfs, d)); err != nil {
		return err
	}
...
func resourceVnicSanConnectivityPolicyDelete(d *schema.ResourceData, meta interface{}) error {
	...
	conn := meta.(*Config)
	url := "vnic/SanConnectivityPolicies" + "/" + d.Id()
	detachVnicSanConnectivityPolicyProfiles(d, meta)
