...
func dataSourceAaaAuditRecord() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"event": {
				Description: "The operation that was performed on this Managed Object.The event is a compound string that includes the CRUD operation such as Create, Modify, Delete, and a string representing the Managed Object type.",
				Type:        schema.TypeString,
				Optional:    true,
			},
			"inst_id": {
			...
			"trace_id": {
				Description: "The trace id of the request that was used to create, modify or delete a REST resource.A trace id is a unique identifier for one particular REST request. It may be used for troubleshooting purposeby the Intersight technical support team.",
				Type:        schema.TypeString,
				Optional:    true,
			},
			"user": {
...
func dataSourceAaaAuditRecordRead(d *schema.ResourceData, meta interface{}) error {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			...
			if err := d.Set("object_moid", (s.ObjectMoid)); err != nil {
				return err
			}
