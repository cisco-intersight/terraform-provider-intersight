...
func resourceHyperflexServerFirmwareVersionCreate(d *schema.ResourceData, meta interface{}) error {
	...
	o.SetClassId("hyperflex.ServerFirmwareVersion")
	...
	if v, ok := d.GetOk("server_firmware_version_entries"); ok {
		...
		for i := 0; i < len(s); i++ {
			...
			o.SetObjectType("hyperflex.ServerFirmwareVersionEntry")
			if v, ok := l["value"]; ok {
				{
					x := (v.(string))
					o.SetValue(x)
...
func resourceHyperflexServerFirmwareVersionDelete(d *schema.ResourceData, meta interface{}) error {
	...
	r := conn.ApiClient.HyperflexApi.DeleteHyperflexServerFirmwareVersion(conn.ctx, d.Id())
	_, err := r.Execute()
	if err != nil {
