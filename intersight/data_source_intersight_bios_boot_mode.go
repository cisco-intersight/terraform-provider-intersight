...
func dataSourceBiosBootMode() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceBiosBootModeRead,
		Schema: map[string]*schema.Schema{
			"actual_boot_mode": {
				Description: "The actual BIOS boot mode as reported by the platform BIOS.",
...
func dataSourceBiosBootModeRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewBiosBootModeWithDefaults()
	if v, ok := d.GetOk("actual_boot_mode"); ok {
		x := (v.(string))
		o.SetActualBootMode(x)
	}
	...
	res, _, err := conn.ApiClient.BiosApi.GetBiosBootModeList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.BiosBootModeList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to BiosBootMode: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewBiosBootModeWithDefaults()
			oo, _ := json.Marshal(r.Index(i).Interface())
			if err = json.Unmarshal(oo, s); err != nil {
				return fmt.Errorf("error occurred while unmarshalling result at index %+v: %+v", i, err)
			}
			if err := d.Set("actual_boot_mode", (s.ActualBootMode)); err != nil {
