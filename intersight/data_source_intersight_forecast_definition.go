...
func dataSourceForecastDefinition() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceForecastDefinitionRead,
		Schema: map[string]*schema.Schema{
			...
			"catalog": {
				Description: "A collection of references to the [forecast.Catalog](mo://forecast.Catalog) Managed Object.When this managed object is deleted, the referenced [forecast.Catalog](mo://forecast.Catalog) MO unsets its reference to this deleted MO.",
...
func dataSourceForecastDefinitionRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)

	url := "forecast/Definitions"
	var o models.ForecastDefinition
	if v, ok := d.GetOk("alert_threshold_in_percentage"); ok {
		x := int64(v.(int))
		o.AlertThresholdInPercentage = x
	}
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			...
			if err := d.Set("catalog", flattenMapForecastCatalogRef(s.Catalog, d)); err != nil {
				return err
			}
