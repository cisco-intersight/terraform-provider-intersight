...
func dataSourceForecastDefinitionRead(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewForecastDefinitionWithDefaults()
	...
	res, _, err := conn.ApiClient.ForecastApi.GetForecastDefinitionList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to ForecastDefinition: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewForecastDefinitionWithDefaults()
