...
func dataSourceIamIdpReference() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceIamIdpReferenceRead,
...
func dataSourceIamIdpReferenceRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewIamIdpReferenceWithDefaults()
	...
	res, _, err := conn.ApiClient.IamApi.GetIamIdpReferenceList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.IamIdpReferenceList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to IamIdpReference: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewIamIdpReferenceWithDefaults()
			...
			if err := d.Set("multi_factor_authentication", (s.GetMultiFactorAuthentication())); err != nil {
				return fmt.Errorf("error occurred while setting property MultiFactorAuthentication: %+v", err)
			}
