...
func resourceIamIdp() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"system": {
				Description: "A collection of references to the [iam.System](mo://iam.System) Managed Object.When this managed object is deleted, the referenced [iam.System](mo://iam.System) MO unsets its reference to this deleted MO.",
				Type:        schema.TypeList,
				MaxItems:    1,
				Optional:    true,
				Computed:    true,
...
func resourceIamIdpCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("system"); ok {
		p := models.IamSystemRef{}
		if len(v.([]interface{})) > 0 {
			o := models.IamSystemRef{}
			l := (v.([]interface{})[0]).(map[string]interface{})
			if v, ok := l["moid"]; ok {
				{
					x := (v.(string))
	...
	if v, ok := d.GetOk("user_login_time"); ok {
		x := make([]*models.IamUserLoginTimeRef, 0)
		switch reflect.TypeOf(v).Kind() {
		case reflect.Slice:
			s := reflect.ValueOf(v)
			for i := 0; i < s.Len(); i++ {
				o := models.IamUserLoginTimeRef{}
				l := s.Index(i).Interface().(map[string]interface{})
				if v, ok := l["moid"]; ok {
					{
						x := (v.(string))
	...
	if v, ok := d.GetOk("user_preferences"); ok {
		x := make([]*models.IamUserPreferenceRef, 0)
		switch reflect.TypeOf(v).Kind() {
		case reflect.Slice:
			s := reflect.ValueOf(v)
			for i := 0; i < s.Len(); i++ {
				o := models.IamUserPreferenceRef{}
				l := s.Index(i).Interface().(map[string]interface{})
				if v, ok := l["moid"]; ok {
					{
						x := (v.(string))
...
func resourceIamIdpRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err := d.Set("system", flattenMapIamSystemRef(s.System, d)); err != nil {
		return err
	}
	...
	if err := d.Set("user_login_time", flattenListIamUserLoginTimeRef(s.UserLoginTime, d)); err != nil {
		return err
	}

	if err := d.Set("user_preferences", flattenListIamUserPreferenceRef(s.UserPreferences, d)); err != nil {
		return err
	}

	if err := d.Set("usergroups", flattenListIamUserGroupRef(s.Usergroups, d)); err != nil {
		return err
	}
...
func resourceIamIdpUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("user_login_time") {
		v := d.Get("user_login_time")
		x := make([]*models.IamUserLoginTimeRef, 0)
		switch reflect.TypeOf(v).Kind() {
		case reflect.Slice:
			s := reflect.ValueOf(v)
