...
func dataSourceIaasMostRunTasksRead(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewIaasMostRunTasksWithDefaults()
	...
	res, _, err := conn.ApiClient.IaasApi.GetIaasMostRunTasksList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to IaasMostRunTasks: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewIaasMostRunTasksWithDefaults()
