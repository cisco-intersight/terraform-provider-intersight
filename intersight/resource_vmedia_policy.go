			...
			"mappings": {
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"remote_path": {
							Description: "Path to the location of the image on the remote server. Preferred format is /path/.",
							Type:        schema.TypeString,
							Optional:    true,
						},
						"username": {
				...
				if v, ok := l["additional_properties"]; ok {
					{
						...
						if err == nil && x1 != nil {
							o.VmediaMappingAO0P0.VmediaMappingAO0P0 = x1.(map[string]interface{})
						}
					}
				}
...
func resourceVmediaPolicyDelete(d *schema.ResourceData, meta interface{}) error {
	...
	conn := meta.(*Config)
	url := "vmedia/Policies" + "/" + d.Id()
	detachVmediaPolicyProfiles(d, meta)
