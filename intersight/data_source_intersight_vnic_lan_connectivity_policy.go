...
func dataSourceVnicLanConnectivityPolicyRead(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewVnicLanConnectivityPolicy()
	...
	result, _, err := conn.ApiClient.VnicApi.GetVnicLanConnectivityPolicyList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewVnicLanConnectivityPolicy()
			...
			if err := d.Set("eth_ifs", flattenListVnicEthIfRelationship(s.EthIfs, d)); err != nil {
				return err
			}
