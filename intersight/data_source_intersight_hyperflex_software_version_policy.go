...
func dataSourceHyperflexSoftwareVersionPolicyRead(d *schema.ResourceData, meta interface{}) error {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			...
			if err := d.Set("hxdp_version_info", flattenMapSoftwareHyperflexDistributableRef(s.HxdpVersionInfo, d)); err != nil {
				return err
			}
			...
			if err := d.Set("server_firmware_version_info", flattenMapFirmwareDistributableRef(s.ServerFirmwareVersionInfo, d)); err != nil {
				return err
			}
