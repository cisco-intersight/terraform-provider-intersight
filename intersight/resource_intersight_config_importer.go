...
func resourceConfigImporter() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"imported_items": {
				Description: "Reference to individual exported managed objects.",
...
func resourceConfigImporterCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("imported_items"); ok {
		x := make([]*models.ConfigImportedItemRef, 0)
		switch reflect.TypeOf(v).Kind() {
		case reflect.Slice:
			s := reflect.ValueOf(v)
			for i := 0; i < s.Len(); i++ {
				o := models.ConfigImportedItemRef{}
...
func resourceConfigImporterRead(d *schema.ResourceData, meta interface{}) error {
	...
	url := "config/Importers" + "/" + d.Id()

	body, err := conn.SendGetRequest(url, []byte(""))
	if err != nil {
		return err
	}
	var s models.ConfigImporter
	...
	if err := d.Set("imported_items", flattenListConfigImportedItemRef(s.ImportedItems, d)); err != nil {
		return err
	}
