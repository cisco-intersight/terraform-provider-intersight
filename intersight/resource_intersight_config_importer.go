...
func resourceConfigImporter() *schema.Resource {
	return &schema.Resource{
		Create: resourceConfigImporterCreate,
		Read:   resourceConfigImporterRead,
		Delete: resourceConfigImporterDelete,
		Schema: map[string]*schema.Schema{
			...
			"imported_items": {
				Description: "An array of relationships to configImportedItem resources.",
...
func resourceConfigImporterCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("imported_items"); ok {
		x := make([]models.ConfigImportedItemRelationship, 0)
		s := v.([]interface{})
		for i := 0; i < len(s); i++ {
			...
			x = append(x, models.MoMoRefAsConfigImportedItemRelationship(o))
		}
		if len(x) > 0 {
			o.SetImportedItems(x)
		}
	}
	...
	o.SetObjectType("config.Importer")
	...
	if v, ok := d.GetOkExists("skip_integrity_checks"); ok {
		x := v.(bool)
	...
	r := conn.ApiClient.ConfigApi.CreateConfigImporter(conn.ctx).ConfigImporter(*o)
	...
	return resourceConfigImporterRead(d, meta)
}

func resourceConfigImporterRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)

	r := conn.ApiClient.ConfigApi.GetConfigImporterByMoid(conn.ctx, d.Id())
	s, _, err := r.Execute()
...
func resourceConfigImporterDelete(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	p := conn.ApiClient.ConfigApi.DeleteConfigImporter(conn.ctx, d.Id())
	_, err := p.Execute()
	if err != nil {
