...
func dataSourceMacpoolIdBlock() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceMacpoolIdBlockRead,
...
func dataSourceMacpoolIdBlockRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewMacpoolIdBlockWithDefaults()
	...
	res, _, err := conn.ApiClient.MacpoolApi.GetMacpoolIdBlockList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.MacpoolIdBlockList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to MacpoolIdBlock: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewMacpoolIdBlockWithDefaults()
			...
			if err := d.Set("free_block_count", (s.FreeBlockCount)); err != nil {
				return fmt.Errorf("error occurred while setting property FreeBlockCount: %+v", err)
			}

			if err := d.Set("mac_block", flattenMapMacpoolBlock(s.MacBlock, d)); err != nil {
				return fmt.Errorf("error occurred while setting property MacBlock: %+v", err)
			}
			if err := d.Set("moid", (s.Moid)); err != nil {
				return fmt.Errorf("error occurred while setting property Moid: %+v", err)
			}
			if err := d.Set("next_id_allocator", (s.NextIdAllocator)); err != nil {
			...
			if err := d.Set("pool", flattenMapMacpoolPoolRelationship(s.Pool, d)); err != nil {
