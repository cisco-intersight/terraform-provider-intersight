...
func dataSourceHyperflexSysConfigPolicy() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceHyperflexSysConfigPolicyRead,
...
func dataSourceHyperflexSysConfigPolicyRead(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewHyperflexSysConfigPolicy()
	...
	result, _, err := conn.ApiClient.HyperflexApi.GetHyperflexSysConfigPolicyList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewHyperflexSysConfigPolicy()
