...
func resourceSdwanProfile() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"router_nodes": {
				Description: "A collection of references to the [sdwan.RouterNode](mo://sdwan.RouterNode) Managed Object.When this managed object is deleted, the referenced [sdwan.RouterNode](mo://sdwan.RouterNode) MOs unset their reference to this deleted MO.",
				Type:        schema.TypeList,
				Optional:    true,
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
			...
			"router_policy": {
				Description: "A collection of references to the [sdwan.RouterPolicy](mo://sdwan.RouterPolicy) Managed Object.When this managed object is deleted, the referenced [sdwan.RouterPolicy](mo://sdwan.RouterPolicy) MO unsets its reference to this deleted MO.",
				Type:        schema.TypeList,
				MaxItems:    1,
				Optional:    true,
				Elem: &schema.Resource{
			...
			"vmanage_account": {
				Description: "A collection of references to the [sdwan.VmanageAccountPolicy](mo://sdwan.VmanageAccountPolicy) Managed Object.When this managed object is deleted, the referenced [sdwan.VmanageAccountPolicy](mo://sdwan.VmanageAccountPolicy) MO unsets its reference to this deleted MO.",
				Type:        schema.TypeList,
				MaxItems:    1,
				Optional:    true,
				Elem: &schema.Resource{
...
func resourceSdwanProfileCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("router_nodes"); ok {
		x := make([]*models.SdwanRouterNodeRef, 0)
		switch reflect.TypeOf(v).Kind() {
		case reflect.Slice:
			s := reflect.ValueOf(v)
			for i := 0; i < s.Len(); i++ {
				o := models.SdwanRouterNodeRef{}
				l := s.Index(i).Interface().(map[string]interface{})
				if v, ok := l["moid"]; ok {
					{
						x := (v.(string))
	...
	if v, ok := d.GetOk("router_policy"); ok {
		p := models.SdwanRouterPolicyRef{}
		if len(v.([]interface{})) > 0 {
			o := models.SdwanRouterPolicyRef{}
			l := (v.([]interface{})[0]).(map[string]interface{})
			if v, ok := l["moid"]; ok {
				{
					x := (v.(string))
	...
	if v, ok := d.GetOk("vmanage_account"); ok {
		p := models.SdwanVmanageAccountPolicyRef{}
		if len(v.([]interface{})) > 0 {
			o := models.SdwanVmanageAccountPolicyRef{}
			l := (v.([]interface{})[0]).(map[string]interface{})
			if v, ok := l["moid"]; ok {
				{
					x := (v.(string))
...
func resourceSdwanProfileRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err := d.Set("router_nodes", flattenListSdwanRouterNodeRef(s.RouterNodes, d)); err != nil {
		return err
	}

	if err := d.Set("router_policy", flattenMapSdwanRouterPolicyRef(s.RouterPolicy, d)); err != nil {
		return err
	}
	...
	if err := d.Set("vmanage_account", flattenMapSdwanVmanageAccountPolicyRef(s.VmanageAccount, d)); err != nil {
		return err
	}
