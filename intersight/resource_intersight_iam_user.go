...
func resourceIamUser() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"oauth_tokens": {
				Description: "Collection of the available OAuthTokens.Each OAuthToken lives 30 days unless it is deleted manually by User.OAuthToken is created when Login performed via OAuth Client (AppRegistration).OAuthToken itself is not sensitive data since it doesn't contain salt, salt is stored in Vault.",
				Type:        schema.TypeList,
				Optional:    true,
				Computed:    true,
				Elem: &schema.Resource{
...
func resourceIamUserCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("api_keys"); ok {
		x := make([]*models.IamAPIKeyRef, 0)
		switch reflect.TypeOf(v).Kind() {
		case reflect.Slice:
			s := reflect.ValueOf(v)
			for i := 0; i < s.Len(); i++ {
				o := models.IamAPIKeyRef{}
	...
	if v, ok := d.GetOk("local_user_password"); ok {
		p := models.IamLocalUserPasswordRef{}
		if len(v.([]interface{})) > 0 {
			o := models.IamLocalUserPasswordRef{}
			l := (v.([]interface{})[0]).(map[string]interface{})
			if v, ok := l["moid"]; ok {
				{
					x := (v.(string))
	...
	if v, ok := d.GetOk("sessions"); ok {
		x := make([]*models.IamSessionRef, 0)
		switch reflect.TypeOf(v).Kind() {
		case reflect.Slice:
			s := reflect.ValueOf(v)
			for i := 0; i < s.Len(); i++ {
				o := models.IamSessionRef{}
				l := s.Index(i).Interface().(map[string]interface{})
				if v, ok := l["moid"]; ok {
					{
						x := (v.(string))
