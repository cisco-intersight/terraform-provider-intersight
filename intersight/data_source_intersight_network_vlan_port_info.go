...
func dataSourceNetworkVlanPortInfoRead(d *schema.ResourceData, meta interface{}) error {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			...
			if err := d.Set("access_vlan_port_count", (s.AccessVlanPortCount)); err != nil {
				return fmt.Errorf("error occurred while setting property AccessVlanPortCount: %+v", err)
			}
			if err := d.Set("border_vlan_port_count", (s.BorderVlanPortCount)); err != nil {
				return fmt.Errorf("error occurred while setting property BorderVlanPortCount: %+v", err)
			}
			if err := d.Set("class_id", (s.ClassId)); err != nil {
				return fmt.Errorf("error occurred while setting property ClassId: %+v", err)
			}
			if err := d.Set("compressed_vlan_port_count", (s.CompressedVlanPortCount)); err != nil {
			...
			if err := d.Set("uncompressed_vlan_port_count", (s.UncompressedVlanPortCount)); err != nil {
				return fmt.Errorf("error occurred while setting property UncompressedVlanPortCount: %+v", err)
			}
			if err := d.Set("vlan_port_limit", (s.VlanPortLimit)); err != nil {
