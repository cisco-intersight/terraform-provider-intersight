...
func dataSourceHyperflexExtIscsiStoragePolicy() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceHyperflexExtIscsiStoragePolicyRead,
...
func dataSourceHyperflexExtIscsiStoragePolicyRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.HyperflexApi.GetHyperflexExtIscsiStoragePolicyList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.HyperflexExtIscsiStoragePolicyList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to HyperflexExtIscsiStoragePolicy: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewHyperflexExtIscsiStoragePolicyWithDefaults()
