...
func dataSourceApplianceDataExportPolicy() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"parent_config": {
				Description: "A collection of references to the [appliance.DataExportPolicy](mo://appliance.DataExportPolicy) Managed Object.When this managed object is deleted, the referenced [appliance.DataExportPolicy](mo://appliance.DataExportPolicy) MO unsets its reference to this deleted MO.",
				Type:        schema.TypeList,
				MaxItems:    1,
				Optional:    true,
				Computed:    true,
...
func dataSourceApplianceDataExportPolicyRead(d *schema.ResourceData, meta interface{}) error {
	...
	url := "appliance/DataExportPolicies"
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			...
			if err := d.Set("enable", (s.Enable)); err != nil {
				return err
			}
			...
			if err := d.Set("parent_config", flattenMapApplianceDataExportPolicyRef(s.ParentConfig, d)); err != nil {
				return err
			}
			...
			if err := d.Set("sub_configs", flattenListApplianceDataExportPolicyRef(s.SubConfigs, d)); err != nil {
				return err
			}
