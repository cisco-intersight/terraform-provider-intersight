...
func dataSourceFirmwareDimmDescriptor() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceFirmwareDimmDescriptorRead,
...
func dataSourceFirmwareDimmDescriptorRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.FirmwareApi.GetFirmwareDimmDescriptorList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.FirmwareDimmDescriptorList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to FirmwareDimmDescriptor: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewFirmwareDimmDescriptorWithDefaults()
