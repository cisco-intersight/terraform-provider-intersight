...
func dataSourceStorageDiskGroupPolicy() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceStorageDiskGroupPolicyRead,
		Schema: map[string]*schema.Schema{
			...
			"storage_policies": {
				Description: "An array of relationships to storageStoragePolicy resources.",
				Type:        schema.TypeList,
				Optional:    true,
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
...
func dataSourceStorageDiskGroupPolicyRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.StorageApi.GetStorageDiskGroupPolicyList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.StorageDiskGroupPolicyList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to StorageDiskGroupPolicy: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewStorageDiskGroupPolicyWithDefaults()
