...
func dataSourceBiosUnit() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"init_seq": {
				Description: "The initSeq of the equipment.",
				Type:        schema.TypeString,
				Optional:    true,
				Computed:    true,
			},
			"init_ts": {
				Description: "The initTs of the equipment.",
				Type:        schema.TypeString,
				Optional:    true,
				Computed:    true,
			},
			...
			"system_boot_order": {
				Description: "A reference to a biosSystemBootOrder resource.\nWhen the $expand query parameter is specified, the referenced resource is returned inline.",
				Type:        schema.TypeList,
				MaxItems:    1,
				Optional:    true,
				Elem: &schema.Resource{
...
func dataSourceBiosUnitRead(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewBiosUnitWithDefaults()
	...
	res, _, err := conn.ApiClient.BiosApi.GetBiosUnitList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.BiosUnitList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to BiosUnit: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewBiosUnitWithDefaults()
			...
			if err := d.Set("init_seq", (s.GetInitSeq())); err != nil {
				return fmt.Errorf("error occurred while setting property InitSeq: %+v", err)
			}
			if err := d.Set("init_ts", (s.GetInitTs())); err != nil {
				return fmt.Errorf("error occurred while setting property InitTs: %+v", err)
			}
			...
			if err := d.Set("system_boot_order", flattenMapBiosSystemBootOrderRelationship(s.GetSystemBootOrder(), d)); err != nil {
				return fmt.Errorf("error occurred while setting property SystemBootOrder: %+v", err)
			}
