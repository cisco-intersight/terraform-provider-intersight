...
func dataSourceIamPrivateKeySpec() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceIamPrivateKeySpecRead,
...
func dataSourceIamPrivateKeySpecRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.IamApi.GetIamPrivateKeySpecList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.IamPrivateKeySpecList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to IamPrivateKeySpec: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewIamPrivateKeySpecWithDefaults()
			oo, _ := json.Marshal(r.Index(i).Interface())
			if err = json.Unmarshal(oo, s); err != nil {
				return fmt.Errorf("error occurred while unmarshalling result at index %+v: %+v", i, err)
			}

			if err := d.Set("algorithm", flattenMapPkixKeyGenerationSpec(s.Algorithm, d)); err != nil {
