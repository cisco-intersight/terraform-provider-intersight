...
func dataSourceComputeServerSettingRead(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewComputeServerSettingWithDefaults()
	if v, ok := d.GetOk("admin_locator_led_state"); ok {
		x := (v.(string))
		o.SetAdminLocatorLedState(x)
	}
	...
	res, _, err := conn.ApiClient.ComputeApi.GetComputeServerSettingList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.ComputeServerSettingList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to ComputeServerSetting: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewComputeServerSettingWithDefaults()
