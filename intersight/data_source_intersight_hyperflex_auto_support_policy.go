...
func dataSourceHyperflexAutoSupportPolicy() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceHyperflexAutoSupportPolicyRead,
...
func dataSourceHyperflexAutoSupportPolicyRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.HyperflexApi.GetHyperflexAutoSupportPolicyList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.HyperflexAutoSupportPolicyList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to HyperflexAutoSupportPolicy: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewHyperflexAutoSupportPolicyWithDefaults()
