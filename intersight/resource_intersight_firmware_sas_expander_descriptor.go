...
func resourceFirmwareSasExpanderDescriptor() *schema.Resource {
	return &schema.Resource{
		Create: resourceFirmwareSasExpanderDescriptorCreate,
		Read:   resourceFirmwareSasExpanderDescriptorRead,
		Update: resourceFirmwareSasExpanderDescriptorUpdate,
		Delete: resourceFirmwareSasExpanderDescriptorDelete,
...
func resourceFirmwareSasExpanderDescriptorCreate(d *schema.ResourceData, meta interface{}) error {
	...
	o.SetClassId("firmware.SasExpanderDescriptor")
	...
	o.SetObjectType("firmware.SasExpanderDescriptor")
	...
	r := conn.ApiClient.FirmwareApi.CreateFirmwareSasExpanderDescriptor(conn.ctx).FirmwareSasExpanderDescriptor(*o)
	...
	return resourceFirmwareSasExpanderDescriptorRead(d, meta)
}

func resourceFirmwareSasExpanderDescriptorRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)

	r := conn.ApiClient.FirmwareApi.GetFirmwareSasExpanderDescriptorByMoid(conn.ctx, d.Id())
	s, _, err := r.Execute()
...
func resourceFirmwareSasExpanderDescriptorUpdate(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewFirmwareSasExpanderDescriptorWithDefaults()
	...
	r := conn.ApiClient.FirmwareApi.UpdateFirmwareSasExpanderDescriptor(conn.ctx, d.Id()).FirmwareSasExpanderDescriptor(*o)
	result, _, err := r.Execute()
	if err != nil {
...
func resourceFirmwareSasExpanderDescriptorDelete(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	p := conn.ApiClient.FirmwareApi.DeleteFirmwareSasExpanderDescriptor(conn.ctx, d.Id())
	_, err := p.Execute()
	if err != nil {
