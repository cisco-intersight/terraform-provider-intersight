...
func dataSourceCapabilitySiocModuleManufacturingDef() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceCapabilitySiocModuleManufacturingDefRead,
...
func dataSourceCapabilitySiocModuleManufacturingDefRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.CapabilityApi.GetCapabilitySiocModuleManufacturingDefList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.CapabilitySiocModuleManufacturingDefList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to CapabilitySiocModuleManufacturingDef: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewCapabilitySiocModuleManufacturingDefWithDefaults()
