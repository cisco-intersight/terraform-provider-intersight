...
func dataSourceHyperflexExtFcStoragePolicy() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceHyperflexExtFcStoragePolicyRead,
...
func dataSourceHyperflexExtFcStoragePolicyRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.HyperflexApi.GetHyperflexExtFcStoragePolicyList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.HyperflexExtFcStoragePolicyList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to HyperflexExtFcStoragePolicy: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewHyperflexExtFcStoragePolicyWithDefaults()
