...
func dataSourceFabricUplinkPcRole() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceFabricUplinkPcRoleRead,
...
func dataSourceFabricUplinkPcRoleRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.FabricApi.GetFabricUplinkPcRoleList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.FabricUplinkPcRoleList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to FabricUplinkPcRole: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewFabricUplinkPcRoleWithDefaults()
