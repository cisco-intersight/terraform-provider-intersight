...
func resourceTamAdvisoryInstanceCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("advisory"); ok {
		p := make([]models.TamAdvisoryRelationship, 0, 1)
		l := (v.([]interface{})[0]).(map[string]interface{})
		{
			...
			p = append(p, o.AsTamAdvisoryRelationship())
		}
		x := p[0]
		o.SetAdvisory(x)
	}
...
func resourceTamAdvisoryInstanceDelete(d *schema.ResourceData, meta interface{}) error {
	...
	r := conn.ApiClient.TamApi.DeleteTamAdvisoryInstance(conn.ctx, d.Id())
	_, err := r.Execute()
	if err != nil {
