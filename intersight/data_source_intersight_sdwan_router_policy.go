...
func dataSourceSdwanRouterPolicyRead(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewSdwanRouterPolicy()
	...
	result, _, err := conn.ApiClient.SdwanApi.GetSdwanRouterPolicyList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewSdwanRouterPolicy()
