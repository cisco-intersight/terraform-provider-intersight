...
func dataSourceSdwanRouterPolicy() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceSdwanRouterPolicyRead,
		Schema: map[string]*schema.Schema{
			...
			"solution_image": {
				Description: "Relationship to the solution distributable object.",
				Type:        schema.TypeList,
				MaxItems:    1,
				Optional:    true,
				Elem: &schema.Resource{
...
func dataSourceSdwanRouterPolicyRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)

	url := "sdwan/RouterPolicies"
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s models.SdwanRouterPolicy
			...
			if err := d.Set("solution_image", flattenMapSoftwareSolutionDistributableRef(s.SolutionImage, d)); err != nil {
				return err
			}
