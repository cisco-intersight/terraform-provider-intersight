...
func dataSourceFabricConfigChangeDetail() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceFabricConfigChangeDetailRead,
...
func dataSourceFabricConfigChangeDetailRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewFabricConfigChangeDetailWithDefaults()
	...
	res, _, err := conn.ApiClient.FabricApi.GetFabricConfigChangeDetailList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.FabricConfigChangeDetailList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to FabricConfigChangeDetail: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewFabricConfigChangeDetailWithDefaults()
