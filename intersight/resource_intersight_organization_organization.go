...
func resourceOrganizationOrganization() *schema.Resource {
	return &schema.Resource{
		Create: resourceOrganizationOrganizationCreate,
		Read:   resourceOrganizationOrganizationRead,
		Update: resourceOrganizationOrganizationUpdate,
		Delete: resourceOrganizationOrganizationDelete,
...
func resourceOrganizationOrganizationCreate(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewOrganizationOrganization()
	...
	o.SetClassId("organization.Organization")
	...
	if v, ok := d.GetOk("resource_groups"); ok {
		...
		for i := 0; i < len(s); i++ {
			...
			x = append(x, o.AsResourceGroupRelationship())
		}
	...
	return resourceOrganizationOrganizationRead(d, meta)
}

func resourceOrganizationOrganizationRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	...
	if err := d.Set("resource_groups", flattenListResourceGroupRelationship(s.ResourceGroups, d)); err != nil {
		return err
	}
...
func resourceOrganizationOrganizationUpdate(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewOrganizationOrganization()
...
func resourceOrganizationOrganizationDelete(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)

	r := conn.ApiClient.OrganizationApi.DeleteOrganizationOrganization(conn.ctx, d.Id())
	_, err := r.Execute()
	if err != nil {
