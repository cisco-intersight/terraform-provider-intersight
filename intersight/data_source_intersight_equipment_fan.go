...
func dataSourceEquipmentFanRead(d *schema.ResourceData, meta interface{}) error {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			...
			if err := d.Set("equipment_fan_module", flattenMapEquipmentFanModuleRelationship(s.GetEquipmentFanModule(), d)); err != nil {
			...
			if err := d.Set("fan_id", (s.GetFanId())); err != nil {
				return fmt.Errorf("error occurred while setting property FanId: %+v", err)
			}
			if err := d.Set("fan_module_id", (s.GetFanModuleId())); err != nil {
