...
func dataSourceFabricEthNetworkControlPolicy() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceFabricEthNetworkControlPolicyRead,
...
func dataSourceFabricEthNetworkControlPolicyRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.FabricApi.GetFabricEthNetworkControlPolicyList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.FabricEthNetworkControlPolicyList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to FabricEthNetworkControlPolicy: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewFabricEthNetworkControlPolicyWithDefaults()
