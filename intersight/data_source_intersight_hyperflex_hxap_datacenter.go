...
func dataSourceHyperflexHxapDatacenter() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceHyperflexHxapDatacenterRead,
		Schema: map[string]*schema.Schema{
			...
			"hypervisor_manager": {
				Description: "A reference to a hyperflexCiscoHypervisorManager resource.\nWhen the $expand query parameter is specified, the referenced resource is returned inline.",
				Type:        schema.TypeList,
				MaxItems:    1,
				Optional:    true,
				Computed:    true,
...
func dataSourceHyperflexHxapDatacenterRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.HyperflexApi.GetHyperflexHxapDatacenterList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.HyperflexHxapDatacenterList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to HyperflexHxapDatacenter: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewHyperflexHxapDatacenterWithDefaults()
