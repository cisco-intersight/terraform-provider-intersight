...
func dataSourceFabricConfigResultEntry() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceFabricConfigResultEntryRead,
...
func dataSourceFabricConfigResultEntryRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewFabricConfigResultEntryWithDefaults()
	...
	res, _, err := conn.ApiClient.FabricApi.GetFabricConfigResultEntryList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.FabricConfigResultEntryList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to FabricConfigResultEntry: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewFabricConfigResultEntryWithDefaults()
