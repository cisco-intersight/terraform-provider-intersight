...
func resourceHyperflexHxapDatacenterCreate(d *schema.ResourceData, meta interface{}) error {
	...
	o.SetClassId("hyperflex.HxapDatacenter")

	if v, ok := d.GetOk("hypervisor_manager"); ok {
		p := make([]models.HyperflexCiscoHypervisorManagerRelationship, 0, 1)
		s := v.([]interface{})
		for i := 0; i < len(s); i++ {
			l := s[i].(map[string]interface{})
			o := models.NewMoMoRefWithDefaults()
			...
			p = append(p, models.MoMoRefAsHyperflexCiscoHypervisorManagerRelationship(o))
		}
		if len(p) > 0 {
			x := p[0]
	...
	o.SetObjectType("hyperflex.HxapDatacenter")
...
func resourceHyperflexHxapDatacenterUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("hypervisor_manager") {
