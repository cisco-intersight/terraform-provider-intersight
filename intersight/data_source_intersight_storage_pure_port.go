...
func dataSourceStoragePurePort() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"controller": {
				Description: "A reference to a storageArrayController resource.\nWhen the $expand query parameter is specified, the referenced resource is returned inline.",
...
func dataSourceStoragePurePortRead(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewStoragePurePort()
	...
	result, _, err := conn.ApiClient.StorageApi.GetStoragePurePortList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewStoragePurePort()
			...
			if err := d.Set("controller", flattenMapStorageArrayControllerRelationship(s.Controller, d)); err != nil {
				return err
			}
