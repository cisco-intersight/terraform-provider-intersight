...
func dataSourceVnicFcQosPolicy() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceVnicFcQosPolicyRead,
...
func dataSourceVnicFcQosPolicyRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.VnicApi.GetVnicFcQosPolicyList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.VnicFcQosPolicyList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to VnicFcQosPolicy: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewVnicFcQosPolicyWithDefaults()
			...
			if err := d.Set("cos", (s.Cos)); err != nil {
				return fmt.Errorf("error occurred while setting property Cos: %+v", err)
			}
			if err := d.Set("description", (s.Description)); err != nil {
				return fmt.Errorf("error occurred while setting property Description: %+v", err)
			}
			if err := d.Set("max_data_field_size", (s.MaxDataFieldSize)); err != nil {
			...
			if err := d.Set("rate_limit", (s.RateLimit)); err != nil {
