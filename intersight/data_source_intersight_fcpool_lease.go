...
func dataSourceFcpoolLeaseRead(d *schema.ResourceData, meta interface{}) error {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			...
			if err := d.Set("pool_member", flattenMapFcpoolPoolMemberRelationship(s.PoolMember, d)); err != nil {
				return fmt.Errorf("error occurred while setting property PoolMember: %+v", err)
			}
			if err := d.Set("pool_purpose", (s.PoolPurpose)); err != nil {
			...
			if err := d.Set("universe", flattenMapFcpoolUniverseRelationship(s.Universe, d)); err != nil {
