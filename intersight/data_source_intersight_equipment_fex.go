...
func dataSourceEquipmentFex() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceEquipmentFexRead,
...
func dataSourceEquipmentFexRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewEquipmentFexWithDefaults()
	...
	if v, ok := d.GetOk("discovery_state"); ok {
		x := (v.(string))
		o.SetDiscoveryState(x)
	}
	...
	res, _, err := conn.ApiClient.EquipmentApi.GetEquipmentFexList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.EquipmentFexList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to EquipmentFex: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewEquipmentFexWithDefaults()
			...
			if err := d.Set("discovery_state", (s.DiscoveryState)); err != nil {
				return fmt.Errorf("error occurred while setting property DiscoveryState: %+v", err)
			}
			if err := d.Set("dn", (s.Dn)); err != nil {
				return fmt.Errorf("error occurred while setting property Dn: %+v", err)
			}

			if err := d.Set("fans", flattenListEquipmentFanRelationship(s.Fans, d)); err != nil {
				return fmt.Errorf("error occurred while setting property Fans: %+v", err)
			}

			if err := d.Set("host_ports", flattenListEtherHostPortRelationship(s.HostPorts, d)); err != nil {
			...
			if err := d.Set("mgmt_controller", flattenMapManagementControllerRelationship(s.MgmtController, d)); err != nil {
			...
			if err := d.Set("network_ports", flattenListEtherNetworkPortRelationship(s.NetworkPorts, d)); err != nil {
