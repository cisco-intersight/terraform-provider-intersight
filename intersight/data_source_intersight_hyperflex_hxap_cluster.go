...
func dataSourceHyperflexHxapCluster() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceHyperflexHxapClusterRead,
...
func dataSourceHyperflexHxapClusterRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewHyperflexHxapClusterWithDefaults()
	...
	res, _, err := conn.ApiClient.HyperflexApi.GetHyperflexHxapClusterList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to HyperflexHxapCluster: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewHyperflexHxapClusterWithDefaults()
			...
			if err := d.Set("datacenter_name", (s.GetDatacenterName())); err != nil {
				return fmt.Errorf("error occurred while setting property DatacenterName: %+v", err)
			}
			...
			if err := d.Set("hx_cluster", flattenMapHyperflexClusterRelationship(s.GetHxCluster(), d)); err != nil {
				return fmt.Errorf("error occurred while setting property HxCluster: %+v", err)
			}
