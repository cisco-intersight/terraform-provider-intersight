...
func dataSourceRecoveryRestore() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceRecoveryRestoreRead,
...
func dataSourceRecoveryRestoreRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.RecoveryApi.GetRecoveryRestoreList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.RecoveryRestoreList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to RecoveryRestore: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewRecoveryRestoreWithDefaults()
			oo, _ := json.Marshal(r.Index(i).Interface())
			if err = json.Unmarshal(oo, s); err != nil {
				return fmt.Errorf("error occurred while unmarshalling result at index %+v: %+v", i, err)
			}

			if err := d.Set("backup_info", flattenMapRecoveryAbstractBackupInfoRelationship(s.BackupInfo, d)); err != nil {
