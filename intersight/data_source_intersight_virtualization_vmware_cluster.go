...
func dataSourceVirtualizationVmwareClusterRead(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewVirtualizationVmwareCluster()
	...
	result, _, err := conn.ApiClient.VirtualizationApi.GetVirtualizationVmwareClusterList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewVirtualizationVmwareCluster()
			...
			if err := d.Set("total_cores", (s.TotalCores)); err != nil {
				return err
			}
			d.SetId(s.GetMoid())
		}
