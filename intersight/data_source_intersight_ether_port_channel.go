...
func dataSourceEtherPortChannelRead(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewEtherPortChannelWithDefaults()
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewEtherPortChannelWithDefaults()
			oo, _ := json.Marshal(r.Index(i).Interface())
			if err = json.Unmarshal(oo, s); err != nil {
				return fmt.Errorf("error occurred while unmarshalling result at index %+v: %+v", i, err)
			}
			if err := d.Set("access_vlan", (s.AccessVlan)); err != nil {
			...
			if err := d.Set("allowed_vlans", (s.AllowedVlans)); err != nil {
			...
			if err := d.Set("native_vlan", (s.NativeVlan)); err != nil {
