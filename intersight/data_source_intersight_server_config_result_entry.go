...
func dataSourceServerConfigResultEntry() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceServerConfigResultEntryRead,
...
func dataSourceServerConfigResultEntryRead(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewServerConfigResultEntryWithDefaults()
	...
	res, _, err := conn.ApiClient.ServerApi.GetServerConfigResultEntryList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.ServerConfigResultEntryList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to ServerConfigResultEntry: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewServerConfigResultEntryWithDefaults()
