...
func dataSourceEquipmentSystemIoControllerRead(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewEquipmentSystemIoController()
	...
	result, _, err := conn.ApiClient.EquipmentApi.GetEquipmentSystemIoControllerList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewEquipmentSystemIoController()
			...
			if err := d.Set("managing_instance", (s.ManagingInstance)); err != nil {
				return err
			}
			...
			if err := d.Set("shared_io_module", flattenMapEquipmentSharedIoModuleRelationship(s.SharedIoModule, d)); err != nil {
				return err
			}
			if err := d.Set("system_io_controller_id", (s.SystemIoControllerId)); err != nil {
				return err
			}
