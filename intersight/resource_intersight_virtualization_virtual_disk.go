...
func resourceVirtualizationVirtualDisk() *schema.Resource {
	return &schema.Resource{
		Create: resourceVirtualizationVirtualDiskCreate,
		Read:   resourceVirtualizationVirtualDiskRead,
		Update: resourceVirtualizationVirtualDiskUpdate,
		Delete: resourceVirtualizationVirtualDiskDelete,
...
func resourceVirtualizationVirtualDiskCreate(d *schema.ResourceData, meta interface{}) error {
	...
	o.SetClassId("virtualization.VirtualDisk")
	...
	if v, ok := d.GetOk("inventory"); ok {
		...
		for i := 0; i < len(s); i++ {
			...
			p = append(p, models.MoMoRefAsVirtualizationBaseVirtualDiskRelationship(o))
		}
		if len(p) > 0 {
			x := p[0]
			o.SetInventory(x)
	...
	o.SetObjectType("virtualization.VirtualDisk")
	...
	return resourceVirtualizationVirtualDiskRead(d, meta)
}

func resourceVirtualizationVirtualDiskRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	...
	if err := d.Set("source_disk_to_clone", (s.GetSourceDiskToClone())); err != nil {
		return fmt.Errorf("error occurred while setting property SourceDiskToClone: %+v", err)
	}
...
func resourceVirtualizationVirtualDiskUpdate(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewVirtualizationVirtualDiskWithDefaults()
	...
	if d.HasChange("source_disk_to_clone") {
		v := d.Get("source_disk_to_clone")
		x := (v.(string))
		o.SetSourceDiskToClone(x)
	}

	if d.HasChange("source_file_path") {
...
func resourceVirtualizationVirtualDiskDelete(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
