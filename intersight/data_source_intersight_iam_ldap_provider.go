...
func dataSourceIamLdapProviderRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.IamApi.GetIamLdapProviderList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.IamLdapProviderList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to IamLdapProvider: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewIamLdapProviderWithDefaults()
