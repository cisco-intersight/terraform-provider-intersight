...
func dataSourceIaasLicenseInfo() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceIaasLicenseInfoRead,
...
func dataSourceIaasLicenseInfoRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewIaasLicenseInfoWithDefaults()
	...
	res, _, err := conn.ApiClient.IaasApi.GetIaasLicenseInfoList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.IaasLicenseInfoList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to IaasLicenseInfo: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewIaasLicenseInfoWithDefaults()
