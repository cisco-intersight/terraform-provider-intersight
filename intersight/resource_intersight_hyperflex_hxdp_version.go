...
func resourceHyperflexHxdpVersionCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("installer_image"); ok {
		p := models.HyperflexInstallerImageRef{}
		if len(v.([]interface{})) > 0 {
			o := models.HyperflexInstallerImageRef{}
			l := (v.([]interface{})[0]).(map[string]interface{})
			if v, ok := l["moid"]; ok {
				{
					x := (v.(string))
...
func resourceHyperflexHxdpVersionRead(d *schema.ResourceData, meta interface{}) error {
	...
	url := "hyperflex/HxdpVersions" + "/" + d.Id()
	...
	if err := d.Set("installer_image", flattenMapHyperflexInstallerImageRef(s.InstallerImage, d)); err != nil {
		return err
	}
...
func resourceHyperflexHxdpVersionUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("installer_image") {
		v := d.Get("installer_image")
		p := models.HyperflexInstallerImageRef{}
		if len(v.([]interface{})) > 0 {
			o := models.HyperflexInstallerImageRef{}
			l := (v.([]interface{})[0]).(map[string]interface{})
