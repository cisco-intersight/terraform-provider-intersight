...
func resourceFirmwarePcieDescriptor() *schema.Resource {
	return &schema.Resource{
		Create: resourceFirmwarePcieDescriptorCreate,
		Read:   resourceFirmwarePcieDescriptorRead,
		Update: resourceFirmwarePcieDescriptorUpdate,
		Delete: resourceFirmwarePcieDescriptorDelete,
...
func resourceFirmwarePcieDescriptorCreate(d *schema.ResourceData, meta interface{}) error {
	...
	o.SetClassId("firmware.PcieDescriptor")
	...
	o.SetObjectType("firmware.PcieDescriptor")
	...
	r := conn.ApiClient.FirmwareApi.CreateFirmwarePcieDescriptor(conn.ctx).FirmwarePcieDescriptor(*o)
	...
	return resourceFirmwarePcieDescriptorRead(d, meta)
}

func resourceFirmwarePcieDescriptorRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
...
func resourceFirmwarePcieDescriptorUpdate(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewFirmwarePcieDescriptorWithDefaults()
	...
	r := conn.ApiClient.FirmwareApi.UpdateFirmwarePcieDescriptor(conn.ctx, d.Id()).FirmwarePcieDescriptor(*o)
	result, _, err := r.Execute()
	if err != nil {
...
func resourceFirmwarePcieDescriptorDelete(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
