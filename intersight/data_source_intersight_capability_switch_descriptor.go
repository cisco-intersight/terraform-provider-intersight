...
func dataSourceCapabilitySwitchDescriptor() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceCapabilitySwitchDescriptorRead,
...
func dataSourceCapabilitySwitchDescriptorRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.CapabilityApi.GetCapabilitySwitchDescriptorList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.CapabilitySwitchDescriptorList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to CapabilitySwitchDescriptor: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewCapabilitySwitchDescriptorWithDefaults()
			...
			if err := d.Set("expected_memory", (s.ExpectedMemory)); err != nil {
