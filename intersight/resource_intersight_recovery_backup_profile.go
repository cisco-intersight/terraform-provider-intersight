...
func resourceRecoveryBackupProfile() *schema.Resource {
	return &schema.Resource{
		Create: resourceRecoveryBackupProfileCreate,
		Read:   resourceRecoveryBackupProfileRead,
		Update: resourceRecoveryBackupProfileUpdate,
		Delete: resourceRecoveryBackupProfileDelete,
...
func resourceRecoveryBackupProfileCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("backup_config"); ok {
		p := models.RecoveryBackupConfigPolicyRef{}
		if len(v.([]interface{})) > 0 {
			o := models.RecoveryBackupConfigPolicyRef{}
	...
	if v, ok := d.GetOk("config_result"); ok {
		p := models.RecoveryConfigResultRef{}
		if len(v.([]interface{})) > 0 {
			o := models.RecoveryConfigResultRef{}
			l := (v.([]interface{})[0]).(map[string]interface{})
			if v, ok := l["moid"]; ok {
				{
					x := (v.(string))
	...
	if v, ok := d.GetOk("schedule_config"); ok {
		p := models.RecoveryScheduleConfigPolicyRef{}
		if len(v.([]interface{})) > 0 {
			o := models.RecoveryScheduleConfigPolicyRef{}
			l := (v.([]interface{})[0]).(map[string]interface{})
			if v, ok := l["moid"]; ok {
				{
					x := (v.(string))
	...
	d.SetId(o.Moid)
	return resourceRecoveryBackupProfileRead(d, meta)
}

func resourceRecoveryBackupProfileRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)

	url := "recovery/BackupProfiles" + "/" + d.Id()
	...
	if err := d.Set("backup_config", flattenMapRecoveryBackupConfigPolicyRef(s.BackupConfig, d)); err != nil {
		return err
	}
	...
	if err := d.Set("config_result", flattenMapRecoveryConfigResultRef(s.ConfigResult, d)); err != nil {
		return err
	}
	...
	if err := d.Set("device_id", flattenMapAssetDeviceRegistrationRef(s.DeviceID, d)); err != nil {
		return err
	}
	...
	if err := d.Set("schedule_config", flattenMapRecoveryScheduleConfigPolicyRef(s.ScheduleConfig, d)); err != nil {
		return err
	}
...
func resourceRecoveryBackupProfileUpdate(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o models.RecoveryBackupProfile
...
func resourceRecoveryBackupProfileDelete(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	url := "recovery/BackupProfiles" + "/" + d.Id()
