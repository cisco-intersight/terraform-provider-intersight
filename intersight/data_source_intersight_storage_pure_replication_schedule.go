...
func dataSourceStoragePureReplicationScheduleRead(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewStoragePureReplicationScheduleWithDefaults()
	...
	res, _, err := conn.ApiClient.StorageApi.GetStoragePureReplicationScheduleList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to StoragePureReplicationSchedule: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewStoragePureReplicationScheduleWithDefaults()
