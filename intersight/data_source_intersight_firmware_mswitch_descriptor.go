...
func dataSourceFirmwareMswitchDescriptor() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceFirmwareMswitchDescriptorRead,
...
func dataSourceFirmwareMswitchDescriptorRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.FirmwareApi.GetFirmwareMswitchDescriptorList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.FirmwareMswitchDescriptorList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to FirmwareMswitchDescriptor: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewFirmwareMswitchDescriptorWithDefaults()
