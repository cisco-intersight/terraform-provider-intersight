...
func dataSourceEquipmentFanModuleRead(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewEquipmentFanModuleWithDefaults()
	...
	res, _, err := conn.ApiClient.EquipmentApi.GetEquipmentFanModuleList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.EquipmentFanModuleList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to EquipmentFanModule: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewEquipmentFanModuleWithDefaults()
			...
			if err := d.Set("equipment_rack_enclosure", flattenMapEquipmentRackEnclosureRelationship(s.EquipmentRackEnclosure, d)); err != nil {
