...
func dataSourceCapabilityAdapterUnitDescriptor() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceCapabilityAdapterUnitDescriptorRead,
...
func dataSourceCapabilityAdapterUnitDescriptorRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.CapabilityApi.GetCapabilityAdapterUnitDescriptorList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to CapabilityAdapterUnitDescriptor: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewCapabilityAdapterUnitDescriptorWithDefaults()
