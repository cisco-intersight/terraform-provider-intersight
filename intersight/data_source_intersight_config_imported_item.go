...
func dataSourceConfigImportedItemRead(d *schema.ResourceData, meta interface{}) error {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s models.ConfigImportedItem
			...
			if err := d.Set("importer", flattenMapConfigImporterRef(s.Importer, d)); err != nil {
				return err
			}
