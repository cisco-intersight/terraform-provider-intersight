...
func resourceVnicLanConnectivityPolicy() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"eth_ifs": {
				Description: "A collection of references to the [vnic.EthIf](mo://vnic.EthIf) Managed Object.When this managed object is deleted, the referenced [vnic.EthIf](mo://vnic.EthIf) MOs on the other side of the relationship are deleted.",
...
func resourceVnicLanConnectivityPolicyCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("eth_ifs"); ok {
		x := make([]*models.VnicEthIfRef, 0)
		switch reflect.TypeOf(v).Kind() {
		case reflect.Slice:
			s := reflect.ValueOf(v)
			for i := 0; i < s.Len(); i++ {
				o := models.VnicEthIfRef{}
...
func detachVnicLanConnectivityPolicyProfiles(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("%v", meta)
	conn := meta.(*Config)
	url := "vnic/LanConnectivityPolicies" + "/" + d.Id()
	var o models.VnicLanConnectivityPolicy
...
func resourceVnicLanConnectivityPolicyDelete(d *schema.ResourceData, meta interface{}) error {
	...
	conn := meta.(*Config)
	url := "vnic/LanConnectivityPolicies" + "/" + d.Id()
	detachVnicLanConnectivityPolicyProfiles(d, meta)
