...
func dataSourcePortGroupRead(d *schema.ResourceData, meta interface{}) error {
	...
	result, _, err := conn.ApiClient.PortApi.GetPortGroupList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			...
			if err := d.Set("sub_groups", flattenListPortSubGroupRelationship(s.SubGroups, d)); err != nil {
				return err
			}
