...
func dataSourceInventoryDeviceInfoRead(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewInventoryDeviceInfo()
	...
	result, _, err := conn.ApiClient.InventoryApi.GetInventoryDeviceInfoList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewInventoryDeviceInfo()
			...
			if err := d.Set("control_action", (s.ControlAction)); err != nil {
				return err
			}
			if err := d.Set("error_state", (s.ErrorState)); err != nil {
				return err
			}

			if err := d.Set("job_info", flattenListPolicyinventoryJobInfo(s.JobInfo, d)); err != nil {
				return err
			}
			...
			if err := d.Set("profile_mo_id", (s.ProfileMoId)); err != nil {
				return err
			}
