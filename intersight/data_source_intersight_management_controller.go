...
func dataSourceManagementController() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceManagementControllerRead,
		Schema: map[string]*schema.Schema{
			...
			"management_interfaces": {
				Description: "An array of relationships to managementInterface resources.",
				Type:        schema.TypeList,
				Optional:    true,
				Computed:    true,
				Elem: &schema.Resource{
			...
			"storage_sas_expander": {
				Description: "A reference to a storageSasExpander resource.\nWhen the $expand query parameter is specified, the referenced resource is returned inline.",
				Type:        schema.TypeList,
				MaxItems:    1,
				Optional:    true,
				Computed:    true,
...
func dataSourceManagementControllerRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewManagementControllerWithDefaults()
	...
	res, _, err := conn.ApiClient.ManagementApi.GetManagementControllerList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.ManagementControllerList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to ManagementController: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewManagementControllerWithDefaults()
			...
			if err := d.Set("equipment_shared_io_module", flattenMapEquipmentSharedIoModuleRelationship(s.EquipmentSharedIoModule, d)); err != nil {
				return fmt.Errorf("error occurred while setting property EquipmentSharedIoModule: %+v", err)
			}

			if err := d.Set("equipment_system_io_controller", flattenMapEquipmentSystemIoControllerRelationship(s.EquipmentSystemIoController, d)); err != nil {
			...
			if err := d.Set("management_interfaces", flattenListManagementInterfaceRelationship(s.ManagementInterfaces, d)); err != nil {
				return fmt.Errorf("error occurred while setting property ManagementInterfaces: %+v", err)
			}
			...
			if err := d.Set("storage_sas_expander", flattenMapStorageSasExpanderRelationship(s.StorageSasExpander, d)); err != nil {
				return fmt.Errorf("error occurred while setting property StorageSasExpander: %+v", err)
			}
