...
func dataSourceVirtualizationVmwareVcenter() *schema.Resource {
...
func dataSourceVirtualizationVmwareVcenterRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewVirtualizationVmwareVcenterWithDefaults()
	...
	res, _, err := conn.ApiClient.VirtualizationApi.GetVirtualizationVmwareVcenterList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.VirtualizationVmwareVcenterList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to VirtualizationVmwareVcenter: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewVirtualizationVmwareVcenterWithDefaults()
