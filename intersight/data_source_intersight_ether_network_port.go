...
func dataSourceEtherNetworkPort() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceEtherNetworkPortRead,
		Schema: map[string]*schema.Schema{
			...
			"speed": {
				Description: "Network Port Speed of IO card or fabric extender.",
				Type:        schema.TypeString,
				Optional:    true,
				Computed:    true,
			},
...
func dataSourceEtherNetworkPortRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewEtherNetworkPortWithDefaults()
	...
	res, _, err := conn.ApiClient.EtherApi.GetEtherNetworkPortList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.EtherNetworkPortList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to EtherNetworkPort: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewEtherNetworkPortWithDefaults()
