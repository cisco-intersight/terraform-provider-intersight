...
func dataSourceVirtualizationVirtualDisk() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceVirtualizationVirtualDiskRead,
...
func dataSourceVirtualizationVirtualDiskRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.VirtualizationApi.GetVirtualizationVirtualDiskList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.VirtualizationVirtualDiskList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to VirtualizationVirtualDisk: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewVirtualizationVirtualDiskWithDefaults()
