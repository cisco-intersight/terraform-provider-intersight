...
func dataSourceComputeBoardRead(d *schema.ResourceData, meta interface{}) error {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			...
			if err := d.Set("equipment_tpms", flattenListEquipmentTpmRef(s.EquipmentTpms, d)); err != nil {
				return err
			}

			if err := d.Set("graphics_cards", flattenListGraphicsCardRef(s.GraphicsCards, d)); err != nil {
				return err
			}

			if err := d.Set("memory_arrays", flattenListMemoryArrayRef(s.MemoryArrays, d)); err != nil {
				return err
			}
			...
			if err := d.Set("pci_coprocessor_cards", flattenListPciCoprocessorCardRef(s.PciCoprocessorCards, d)); err != nil {
				return err
			}

			if err := d.Set("pci_switch", flattenListPciSwitchRef(s.PciSwitch, d)); err != nil {
				return err
			}
			...
			if err := d.Set("persistent_memory_configuration", flattenMapMemoryPersistentMemoryConfigurationRef(s.PersistentMemoryConfiguration, d)); err != nil {
				return err
			}
			...
			if err := d.Set("processors", flattenListProcessorUnitRef(s.Processors, d)); err != nil {
				return err
			}
			...
			if err := d.Set("security_units", flattenListSecurityUnitRef(s.SecurityUnits, d)); err != nil {
				return err
			}
			...
			if err := d.Set("storage_controllers", flattenListStorageControllerRef(s.StorageControllers, d)); err != nil {
				return err
			}

			if err := d.Set("storage_flex_flash_controllers", flattenListStorageFlexFlashControllerRef(s.StorageFlexFlashControllers, d)); err != nil {
				return err
			}

			if err := d.Set("storage_flex_util_controllers", flattenListStorageFlexUtilControllerRef(s.StorageFlexUtilControllers, d)); err != nil {
				return err
			}
