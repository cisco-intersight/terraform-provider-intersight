...
func (dst *FabricServerRoleResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "fabric.ServerRole.List" {
		...
		if err == nil {
			jsonFabricServerRoleList, _ := json.Marshal(dst.FabricServerRoleList)
			if string(jsonFabricServerRoleList) == "{}" { // empty struct
	...
	// try to unmarshal data into FabricServerRoleList
	err = json.Unmarshal(data, &dst.FabricServerRoleList)
	if err == nil {
		jsonFabricServerRoleList, _ := json.Marshal(dst.FabricServerRoleList)
		if string(jsonFabricServerRoleList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(FabricServerRoleResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(FabricServerRoleResponse)")
	}
}
