...
func (dst *IamAppRegistrationRelationship) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into IamAppRegistration
	err = json.Unmarshal(data, &dst.IamAppRegistration)
	if err == nil {
		jsonIamAppRegistration, _ := json.Marshal(dst.IamAppRegistration)
		if string(jsonIamAppRegistration) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IamAppRegistrationRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IamAppRegistrationRelationship)")
	}
}
