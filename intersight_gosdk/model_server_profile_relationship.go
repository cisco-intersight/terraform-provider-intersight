...
func (dst *ServerProfileRelationship) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into ServerProfile
	err = json.Unmarshal(data, &dst.ServerProfile)
	if err == nil {
		jsonServerProfile, _ := json.Marshal(dst.ServerProfile)
		if string(jsonServerProfile) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(ServerProfileRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(ServerProfileRelationship)")
	}
}
