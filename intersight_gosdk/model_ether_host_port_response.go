...
func (dst *EtherHostPortResponse) UnmarshalJSON(data []byte) error {
	var err error
	match := 0
	// try to unmarshal data into EtherHostPortList
	err = json.Unmarshal(data, &dst.EtherHostPortList)
	if err == nil {
		jsonEtherHostPortList, _ := json.Marshal(dst.EtherHostPortList)
		if string(jsonEtherHostPortList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(EtherHostPortResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(EtherHostPortResponse)")
	}
}
