...
func (dst *StorageVirtualDriveExtensionRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "storage.VirtualDriveExtension" {
		...
		if err == nil {
			jsonStorageVirtualDriveExtension, _ := json.Marshal(dst.StorageVirtualDriveExtension)
			if string(jsonStorageVirtualDriveExtension) == "{}" { // empty struct
	...
	// try to unmarshal data into StorageVirtualDriveExtension
	err = json.Unmarshal(data, &dst.StorageVirtualDriveExtension)
	if err == nil {
		jsonStorageVirtualDriveExtension, _ := json.Marshal(dst.StorageVirtualDriveExtension)
		if string(jsonStorageVirtualDriveExtension) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(StorageVirtualDriveExtensionRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(StorageVirtualDriveExtensionRelationship)")
	}
}
