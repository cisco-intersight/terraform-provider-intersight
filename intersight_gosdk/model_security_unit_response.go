...
func (dst *SecurityUnitResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "security.Unit.List" {
		...
		if err == nil {
			jsonSecurityUnitList, _ := json.Marshal(dst.SecurityUnitList)
			if string(jsonSecurityUnitList) == "{}" { // empty struct
	...
	// try to unmarshal data into SecurityUnitList
	err = json.Unmarshal(data, &dst.SecurityUnitList)
	if err == nil {
		jsonSecurityUnitList, _ := json.Marshal(dst.SecurityUnitList)
		if string(jsonSecurityUnitList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(SecurityUnitResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(SecurityUnitResponse)")
	}
}
