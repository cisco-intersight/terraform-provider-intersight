...
func (dst *VirtualizationVirtualDiskResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "virtualization.VirtualDisk.List" {
		...
		if err == nil {
			jsonVirtualizationVirtualDiskList, _ := json.Marshal(dst.VirtualizationVirtualDiskList)
			if string(jsonVirtualizationVirtualDiskList) == "{}" { // empty struct
	...
	// try to unmarshal data into VirtualizationVirtualDiskList
	err = json.Unmarshal(data, &dst.VirtualizationVirtualDiskList)
	if err == nil {
		jsonVirtualizationVirtualDiskList, _ := json.Marshal(dst.VirtualizationVirtualDiskList)
		if string(jsonVirtualizationVirtualDiskList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(VirtualizationVirtualDiskResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(VirtualizationVirtualDiskResponse)")
	}
}
