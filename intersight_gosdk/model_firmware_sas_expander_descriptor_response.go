...
func (dst *FirmwareSasExpanderDescriptorResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "firmware.SasExpanderDescriptor.List" {
		...
		if err == nil {
			jsonFirmwareSasExpanderDescriptorList, _ := json.Marshal(dst.FirmwareSasExpanderDescriptorList)
			if string(jsonFirmwareSasExpanderDescriptorList) == "{}" { // empty struct
	...
	// try to unmarshal data into FirmwareSasExpanderDescriptorList
	err = json.Unmarshal(data, &dst.FirmwareSasExpanderDescriptorList)
	if err == nil {
		jsonFirmwareSasExpanderDescriptorList, _ := json.Marshal(dst.FirmwareSasExpanderDescriptorList)
		if string(jsonFirmwareSasExpanderDescriptorList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(FirmwareSasExpanderDescriptorResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(FirmwareSasExpanderDescriptorResponse)")
	}
}
