...
func (dst *RecoveryBackupProfileResponse) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into RecoveryBackupProfileList
	err = json.Unmarshal(data, &dst.RecoveryBackupProfileList)
	if err == nil {
		jsonRecoveryBackupProfileList, _ := json.Marshal(dst.RecoveryBackupProfileList)
		if string(jsonRecoveryBackupProfileList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(RecoveryBackupProfileResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(RecoveryBackupProfileResponse)")
	}
}
