...
func (dst *StoragePhysicalDiskRelationship) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into StoragePhysicalDisk
	err = json.Unmarshal(data, &dst.StoragePhysicalDisk)
	if err == nil {
		jsonStoragePhysicalDisk, _ := json.Marshal(dst.StoragePhysicalDisk)
		if string(jsonStoragePhysicalDisk) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(StoragePhysicalDiskRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(StoragePhysicalDiskRelationship)")
	}
}
