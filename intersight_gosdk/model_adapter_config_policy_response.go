...
func (dst *AdapterConfigPolicyResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "adapter.ConfigPolicy.List" {
		...
		if err == nil {
			jsonAdapterConfigPolicyList, _ := json.Marshal(dst.AdapterConfigPolicyList)
			if string(jsonAdapterConfigPolicyList) == "{}" { // empty struct
	...
	// try to unmarshal data into AdapterConfigPolicyList
	err = json.Unmarshal(data, &dst.AdapterConfigPolicyList)
	if err == nil {
		jsonAdapterConfigPolicyList, _ := json.Marshal(dst.AdapterConfigPolicyList)
		if string(jsonAdapterConfigPolicyList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(AdapterConfigPolicyResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(AdapterConfigPolicyResponse)")
	}
}
