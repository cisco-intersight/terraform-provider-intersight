...
func (dst *ApplianceBackupResponse) UnmarshalJSON(data []byte) error {
	var err error
	match := 0
	// try to unmarshal data into ApplianceBackupList
	err = json.Unmarshal(data, &dst.ApplianceBackupList)
	if err == nil {
		jsonApplianceBackupList, _ := json.Marshal(dst.ApplianceBackupList)
		if string(jsonApplianceBackupList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(ApplianceBackupResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(ApplianceBackupResponse)")
	}
}
