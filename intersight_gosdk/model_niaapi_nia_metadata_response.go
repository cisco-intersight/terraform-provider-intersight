...
func (dst *NiaapiNiaMetadataResponse) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into NiaapiNiaMetadataList
	err = json.Unmarshal(data, &dst.NiaapiNiaMetadataList)
	if err == nil {
		jsonNiaapiNiaMetadataList, _ := json.Marshal(dst.NiaapiNiaMetadataList)
		if string(jsonNiaapiNiaMetadataList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(NiaapiNiaMetadataResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(NiaapiNiaMetadataResponse)")
	}
}
