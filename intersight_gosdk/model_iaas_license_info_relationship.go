...
func (dst *IaasLicenseInfoRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "iaas.LicenseInfo" {
		...
		if err == nil {
			jsonIaasLicenseInfo, _ := json.Marshal(dst.IaasLicenseInfo)
			if string(jsonIaasLicenseInfo) == "{}" { // empty struct
	...
	// try to unmarshal data into IaasLicenseInfo
	err = json.Unmarshal(data, &dst.IaasLicenseInfo)
	if err == nil {
		jsonIaasLicenseInfo, _ := json.Marshal(dst.IaasLicenseInfo)
		if string(jsonIaasLicenseInfo) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IaasLicenseInfoRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IaasLicenseInfoRelationship)")
	}
}
