...
func (dst *EquipmentSharedIoModuleResponse) UnmarshalJSON(data []byte) error {
	var err error
	match := 0
	// try to unmarshal data into EquipmentSharedIoModuleList
	err = json.Unmarshal(data, &dst.EquipmentSharedIoModuleList)
	if err == nil {
		jsonEquipmentSharedIoModuleList, _ := json.Marshal(dst.EquipmentSharedIoModuleList)
		if string(jsonEquipmentSharedIoModuleList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(EquipmentSharedIoModuleResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(EquipmentSharedIoModuleResponse)")
	}
}
