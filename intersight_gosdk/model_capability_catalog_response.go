...
func (dst *CapabilityCatalogResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "capability.Catalog.List" {
		...
		if err == nil {
			jsonCapabilityCatalogList, _ := json.Marshal(dst.CapabilityCatalogList)
			if string(jsonCapabilityCatalogList) == "{}" { // empty struct
	...
	// try to unmarshal data into CapabilityCatalogList
	err = json.Unmarshal(data, &dst.CapabilityCatalogList)
	if err == nil {
		jsonCapabilityCatalogList, _ := json.Marshal(dst.CapabilityCatalogList)
		if string(jsonCapabilityCatalogList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(CapabilityCatalogResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(CapabilityCatalogResponse)")
	}
}
