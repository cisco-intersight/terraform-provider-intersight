...
func (dst *HyperflexClusterProfileResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "hyperflex.ClusterProfile.List" {
		...
		if err == nil {
			jsonHyperflexClusterProfileList, _ := json.Marshal(dst.HyperflexClusterProfileList)
			if string(jsonHyperflexClusterProfileList) == "{}" { // empty struct
	...
	// try to unmarshal data into HyperflexClusterProfileList
	err = json.Unmarshal(data, &dst.HyperflexClusterProfileList)
	if err == nil {
		jsonHyperflexClusterProfileList, _ := json.Marshal(dst.HyperflexClusterProfileList)
		if string(jsonHyperflexClusterProfileList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(HyperflexClusterProfileResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(HyperflexClusterProfileResponse)")
	}
}
