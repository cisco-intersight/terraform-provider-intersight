...
func (dst *AdapterHostFcInterfaceRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "adapter.HostFcInterface" {
		...
		if err == nil {
			jsonAdapterHostFcInterface, _ := json.Marshal(dst.AdapterHostFcInterface)
			if string(jsonAdapterHostFcInterface) == "{}" { // empty struct
	...
	// try to unmarshal data into AdapterHostFcInterface
	err = json.Unmarshal(data, &dst.AdapterHostFcInterface)
	if err == nil {
		jsonAdapterHostFcInterface, _ := json.Marshal(dst.AdapterHostFcInterface)
		if string(jsonAdapterHostFcInterface) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(AdapterHostFcInterfaceRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(AdapterHostFcInterfaceRelationship)")
	}
}
