...
func (dst *StorageStoragePolicyRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "storage.StoragePolicy" {
		...
		if err == nil {
			jsonStorageStoragePolicy, _ := json.Marshal(dst.StorageStoragePolicy)
			if string(jsonStorageStoragePolicy) == "{}" { // empty struct
	...
	// try to unmarshal data into StorageStoragePolicy
	err = json.Unmarshal(data, &dst.StorageStoragePolicy)
	if err == nil {
		jsonStorageStoragePolicy, _ := json.Marshal(dst.StorageStoragePolicy)
		if string(jsonStorageStoragePolicy) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(StorageStoragePolicyRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(StorageStoragePolicyRelationship)")
	}
}
