...
func (dst *InventoryGenericInventoryRelationship) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into InventoryGenericInventory
	err = json.Unmarshal(data, &dst.InventoryGenericInventory)
	if err == nil {
		jsonInventoryGenericInventory, _ := json.Marshal(dst.InventoryGenericInventory)
		if string(jsonInventoryGenericInventory) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(InventoryGenericInventoryRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(InventoryGenericInventoryRelationship)")
	}
}
