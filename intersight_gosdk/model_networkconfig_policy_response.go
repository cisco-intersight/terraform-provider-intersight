...
func (dst *NetworkconfigPolicyResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "networkconfig.Policy.List" {
		...
		if err == nil {
			jsonNetworkconfigPolicyList, _ := json.Marshal(dst.NetworkconfigPolicyList)
			if string(jsonNetworkconfigPolicyList) == "{}" { // empty struct
	...
	// try to unmarshal data into NetworkconfigPolicyList
	err = json.Unmarshal(data, &dst.NetworkconfigPolicyList)
	if err == nil {
		jsonNetworkconfigPolicyList, _ := json.Marshal(dst.NetworkconfigPolicyList)
		if string(jsonNetworkconfigPolicyList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(NetworkconfigPolicyResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(NetworkconfigPolicyResponse)")
	}
}
