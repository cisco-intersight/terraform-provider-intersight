...
func (dst *IamLdapGroupRelationship) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into IamLdapGroup
	err = json.Unmarshal(data, &dst.IamLdapGroup)
	if err == nil {
		jsonIamLdapGroup, _ := json.Marshal(dst.IamLdapGroup)
		if string(jsonIamLdapGroup) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IamLdapGroupRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IamLdapGroupRelationship)")
	}
}
