...
func (dst *ConnectorpackConnectorPackUpgradeResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "connectorpack.ConnectorPackUpgrade.List" {
		...
		if err == nil {
			jsonConnectorpackConnectorPackUpgradeList, _ := json.Marshal(dst.ConnectorpackConnectorPackUpgradeList)
			if string(jsonConnectorpackConnectorPackUpgradeList) == "{}" { // empty struct
	...
	// try to unmarshal data into ConnectorpackConnectorPackUpgradeList
	err = json.Unmarshal(data, &dst.ConnectorpackConnectorPackUpgradeList)
	if err == nil {
		jsonConnectorpackConnectorPackUpgradeList, _ := json.Marshal(dst.ConnectorpackConnectorPackUpgradeList)
		if string(jsonConnectorpackConnectorPackUpgradeList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(ConnectorpackConnectorPackUpgradeResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(ConnectorpackConnectorPackUpgradeResponse)")
	}
}
