...
func (dst *IamEndPointPrivilegeResponse) UnmarshalJSON(data []byte) error {
	var err error
	match := 0
	// try to unmarshal data into IamEndPointPrivilegeList
	err = json.Unmarshal(data, &dst.IamEndPointPrivilegeList)
	if err == nil {
		jsonIamEndPointPrivilegeList, _ := json.Marshal(dst.IamEndPointPrivilegeList)
		if string(jsonIamEndPointPrivilegeList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IamEndPointPrivilegeResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IamEndPointPrivilegeResponse)")
	}
}
