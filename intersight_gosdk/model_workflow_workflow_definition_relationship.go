...
func (dst *WorkflowWorkflowDefinitionRelationship) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into WorkflowWorkflowDefinition
	err = json.Unmarshal(data, &dst.WorkflowWorkflowDefinition)
	if err == nil {
		jsonWorkflowWorkflowDefinition, _ := json.Marshal(dst.WorkflowWorkflowDefinition)
		if string(jsonWorkflowWorkflowDefinition) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(WorkflowWorkflowDefinitionRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(WorkflowWorkflowDefinitionRelationship)")
	}
}
