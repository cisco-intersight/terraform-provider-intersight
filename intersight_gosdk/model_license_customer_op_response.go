...
func (dst *LicenseCustomerOpResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "license.CustomerOp.List" {
		...
		if err == nil {
			jsonLicenseCustomerOpList, _ := json.Marshal(dst.LicenseCustomerOpList)
			if string(jsonLicenseCustomerOpList) == "{}" { // empty struct
	...
	// try to unmarshal data into LicenseCustomerOpList
	err = json.Unmarshal(data, &dst.LicenseCustomerOpList)
	if err == nil {
		jsonLicenseCustomerOpList, _ := json.Marshal(dst.LicenseCustomerOpList)
		if string(jsonLicenseCustomerOpList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(LicenseCustomerOpResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(LicenseCustomerOpResponse)")
	}
}
