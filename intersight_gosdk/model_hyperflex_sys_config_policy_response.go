...
func (dst *HyperflexSysConfigPolicyResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "hyperflex.SysConfigPolicy.List" {
		...
		if err == nil {
			jsonHyperflexSysConfigPolicyList, _ := json.Marshal(dst.HyperflexSysConfigPolicyList)
			if string(jsonHyperflexSysConfigPolicyList) == "{}" { // empty struct
	...
	// try to unmarshal data into HyperflexSysConfigPolicyList
	err = json.Unmarshal(data, &dst.HyperflexSysConfigPolicyList)
	if err == nil {
		jsonHyperflexSysConfigPolicyList, _ := json.Marshal(dst.HyperflexSysConfigPolicyList)
		if string(jsonHyperflexSysConfigPolicyList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(HyperflexSysConfigPolicyResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(HyperflexSysConfigPolicyResponse)")
	}
}
