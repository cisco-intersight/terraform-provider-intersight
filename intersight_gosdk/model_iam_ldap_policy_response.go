...
func (dst *IamLdapPolicyResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "iam.LdapPolicy.List" {
		...
		if err == nil {
			jsonIamLdapPolicyList, _ := json.Marshal(dst.IamLdapPolicyList)
			if string(jsonIamLdapPolicyList) == "{}" { // empty struct
	...
	// try to unmarshal data into IamLdapPolicyList
	err = json.Unmarshal(data, &dst.IamLdapPolicyList)
	if err == nil {
		jsonIamLdapPolicyList, _ := json.Marshal(dst.IamLdapPolicyList)
		if string(jsonIamLdapPolicyList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IamLdapPolicyResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IamLdapPolicyResponse)")
	}
}
