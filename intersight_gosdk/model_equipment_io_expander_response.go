...
func (dst *EquipmentIoExpanderResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "equipment.IoExpander.List" {
		...
		if err == nil {
			jsonEquipmentIoExpanderList, _ := json.Marshal(dst.EquipmentIoExpanderList)
			if string(jsonEquipmentIoExpanderList) == "{}" { // empty struct
	...
	// try to unmarshal data into EquipmentIoExpanderList
	err = json.Unmarshal(data, &dst.EquipmentIoExpanderList)
	if err == nil {
		jsonEquipmentIoExpanderList, _ := json.Marshal(dst.EquipmentIoExpanderList)
		if string(jsonEquipmentIoExpanderList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(EquipmentIoExpanderResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(EquipmentIoExpanderResponse)")
	}
}
