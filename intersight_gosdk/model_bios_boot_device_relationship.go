...
func (dst *BiosBootDeviceRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "bios.BootDevice" {
		...
		if err == nil {
			jsonBiosBootDevice, _ := json.Marshal(dst.BiosBootDevice)
			if string(jsonBiosBootDevice) == "{}" { // empty struct
	...
	// try to unmarshal data into BiosBootDevice
	err = json.Unmarshal(data, &dst.BiosBootDevice)
	if err == nil {
		jsonBiosBootDevice, _ := json.Marshal(dst.BiosBootDevice)
		if string(jsonBiosBootDevice) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(BiosBootDeviceRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(BiosBootDeviceRelationship)")
	}
}
