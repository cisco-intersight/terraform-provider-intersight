...
func (dst *StorageFlexFlashControllerPropsRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "storage.FlexFlashControllerProps" {
		...
		if err == nil {
			jsonStorageFlexFlashControllerProps, _ := json.Marshal(dst.StorageFlexFlashControllerProps)
			if string(jsonStorageFlexFlashControllerProps) == "{}" { // empty struct
	...
	// try to unmarshal data into StorageFlexFlashControllerProps
	err = json.Unmarshal(data, &dst.StorageFlexFlashControllerProps)
	if err == nil {
		jsonStorageFlexFlashControllerProps, _ := json.Marshal(dst.StorageFlexFlashControllerProps)
		if string(jsonStorageFlexFlashControllerProps) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(StorageFlexFlashControllerPropsRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(StorageFlexFlashControllerPropsRelationship)")
	}
}
