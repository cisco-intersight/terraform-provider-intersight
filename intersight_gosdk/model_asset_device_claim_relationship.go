...
func (dst *AssetDeviceClaimRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "asset.DeviceClaim" {
		...
		if err == nil {
			jsonAssetDeviceClaim, _ := json.Marshal(dst.AssetDeviceClaim)
			if string(jsonAssetDeviceClaim) == "{}" { // empty struct
	...
	// try to unmarshal data into AssetDeviceClaim
	err = json.Unmarshal(data, &dst.AssetDeviceClaim)
	if err == nil {
		jsonAssetDeviceClaim, _ := json.Marshal(dst.AssetDeviceClaim)
		if string(jsonAssetDeviceClaim) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(AssetDeviceClaimRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(AssetDeviceClaimRelationship)")
	}
}
