...
func (dst *VirtualizationBaseClusterRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "virtualization.BaseCluster" {
		...
		if err == nil {
			jsonVirtualizationBaseCluster, _ := json.Marshal(dst.VirtualizationBaseCluster)
			if string(jsonVirtualizationBaseCluster) == "{}" { // empty struct
	...
	// try to unmarshal data into VirtualizationBaseCluster
	err = json.Unmarshal(data, &dst.VirtualizationBaseCluster)
	if err == nil {
		jsonVirtualizationBaseCluster, _ := json.Marshal(dst.VirtualizationBaseCluster)
		if string(jsonVirtualizationBaseCluster) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(VirtualizationBaseClusterRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(VirtualizationBaseClusterRelationship)")
	}
}
