...
func (dst *AdapterUnitResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "adapter.Unit.List" {
		...
		if err == nil {
			jsonAdapterUnitList, _ := json.Marshal(dst.AdapterUnitList)
			if string(jsonAdapterUnitList) == "{}" { // empty struct
	...
	// try to unmarshal data into AdapterUnitList
	err = json.Unmarshal(data, &dst.AdapterUnitList)
	if err == nil {
		jsonAdapterUnitList, _ := json.Marshal(dst.AdapterUnitList)
		if string(jsonAdapterUnitList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(AdapterUnitResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(AdapterUnitResponse)")
	}
}
