...
func (dst *EquipmentLocatorLedRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "equipment.LocatorLed" {
		...
		if err == nil {
			jsonEquipmentLocatorLed, _ := json.Marshal(dst.EquipmentLocatorLed)
			if string(jsonEquipmentLocatorLed) == "{}" { // empty struct
	...
	// try to unmarshal data into EquipmentLocatorLed
	err = json.Unmarshal(data, &dst.EquipmentLocatorLed)
	if err == nil {
		jsonEquipmentLocatorLed, _ := json.Marshal(dst.EquipmentLocatorLed)
		if string(jsonEquipmentLocatorLed) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(EquipmentLocatorLedRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(EquipmentLocatorLedRelationship)")
	}
}
