...
func (dst *NiaapiApicHweolResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "niaapi.ApicHweol.List" {
		...
		if err == nil {
			jsonNiaapiApicHweolList, _ := json.Marshal(dst.NiaapiApicHweolList)
			if string(jsonNiaapiApicHweolList) == "{}" { // empty struct
	...
	// try to unmarshal data into NiaapiApicHweolList
	err = json.Unmarshal(data, &dst.NiaapiApicHweolList)
	if err == nil {
		jsonNiaapiApicHweolList, _ := json.Marshal(dst.NiaapiApicHweolList)
		if string(jsonNiaapiApicHweolList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(NiaapiApicHweolResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(NiaapiApicHweolResponse)")
	}
}
