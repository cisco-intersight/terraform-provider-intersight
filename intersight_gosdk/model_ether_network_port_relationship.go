...
func (dst *EtherNetworkPortRelationship) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into EtherNetworkPort
	err = json.Unmarshal(data, &dst.EtherNetworkPort)
	if err == nil {
		jsonEtherNetworkPort, _ := json.Marshal(dst.EtherNetworkPort)
		if string(jsonEtherNetworkPort) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(EtherNetworkPortRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(EtherNetworkPortRelationship)")
	}
}
