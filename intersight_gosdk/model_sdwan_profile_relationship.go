...
func (dst *SdwanProfileRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "sdwan.Profile" {
		...
		if err == nil {
			jsonSdwanProfile, _ := json.Marshal(dst.SdwanProfile)
			if string(jsonSdwanProfile) == "{}" { // empty struct
	...
	// try to unmarshal data into SdwanProfile
	err = json.Unmarshal(data, &dst.SdwanProfile)
	if err == nil {
		jsonSdwanProfile, _ := json.Marshal(dst.SdwanProfile)
		if string(jsonSdwanProfile) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(SdwanProfileRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(SdwanProfileRelationship)")
	}
}
