...
func (dst *EquipmentPhysicalIdentityRelationship) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into EquipmentPhysicalIdentity
	err = json.Unmarshal(data, &dst.EquipmentPhysicalIdentity)
	if err == nil {
		jsonEquipmentPhysicalIdentity, _ := json.Marshal(dst.EquipmentPhysicalIdentity)
		if string(jsonEquipmentPhysicalIdentity) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(EquipmentPhysicalIdentityRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(EquipmentPhysicalIdentityRelationship)")
	}
}
