...
func (dst *FabricEthNetworkPolicyResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "fabric.EthNetworkPolicy.List" {
		...
		if err == nil {
			jsonFabricEthNetworkPolicyList, _ := json.Marshal(dst.FabricEthNetworkPolicyList)
			if string(jsonFabricEthNetworkPolicyList) == "{}" { // empty struct
	...
	// try to unmarshal data into FabricEthNetworkPolicyList
	err = json.Unmarshal(data, &dst.FabricEthNetworkPolicyList)
	if err == nil {
		jsonFabricEthNetworkPolicyList, _ := json.Marshal(dst.FabricEthNetworkPolicyList)
		if string(jsonFabricEthNetworkPolicyList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(FabricEthNetworkPolicyResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(FabricEthNetworkPolicyResponse)")
	}
}
