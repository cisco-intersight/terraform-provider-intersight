...
func (dst *EquipmentChassisIdentityResponse) UnmarshalJSON(data []byte) error {
	var err error
	match := 0
	// try to unmarshal data into EquipmentChassisIdentityList
	err = json.Unmarshal(data, &dst.EquipmentChassisIdentityList)
	if err == nil {
		jsonEquipmentChassisIdentityList, _ := json.Marshal(dst.EquipmentChassisIdentityList)
		if string(jsonEquipmentChassisIdentityList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(EquipmentChassisIdentityResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(EquipmentChassisIdentityResponse)")
	}
}
