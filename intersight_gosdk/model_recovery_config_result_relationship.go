...
func MoMoRefAsRecoveryConfigResultRelationship(v *MoMoRef) RecoveryConfigResultRelationship {
	return RecoveryConfigResultRelationship{MoMoRef: v}
}
...
func RecoveryConfigResultAsRecoveryConfigResultRelationship(v *RecoveryConfigResult) RecoveryConfigResultRelationship {
	return RecoveryConfigResultRelationship{RecoveryConfigResult: v}
}
...
func (dst *RecoveryConfigResultRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "recovery.ConfigResult" {
		...
		if err == nil {
			jsonRecoveryConfigResult, _ := json.Marshal(dst.RecoveryConfigResult)
			if string(jsonRecoveryConfigResult) == "{}" { // empty struct
	...
	// try to unmarshal data into RecoveryConfigResult
	err = json.Unmarshal(data, &dst.RecoveryConfigResult)
	if err == nil {
		jsonRecoveryConfigResult, _ := json.Marshal(dst.RecoveryConfigResult)
		if string(jsonRecoveryConfigResult) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(RecoveryConfigResultRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(RecoveryConfigResultRelationship)")
	}
}
