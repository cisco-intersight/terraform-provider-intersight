...
func (dst *ApplianceBackupPolicyResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "appliance.BackupPolicy.List" {
		...
		if err == nil {
			jsonApplianceBackupPolicyList, _ := json.Marshal(dst.ApplianceBackupPolicyList)
			if string(jsonApplianceBackupPolicyList) == "{}" { // empty struct
	...
	// try to unmarshal data into ApplianceBackupPolicyList
	err = json.Unmarshal(data, &dst.ApplianceBackupPolicyList)
	if err == nil {
		jsonApplianceBackupPolicyList, _ := json.Marshal(dst.ApplianceBackupPolicyList)
		if string(jsonApplianceBackupPolicyList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(ApplianceBackupPolicyResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(ApplianceBackupPolicyResponse)")
	}
}
