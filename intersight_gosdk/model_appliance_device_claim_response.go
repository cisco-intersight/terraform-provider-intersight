...
func (dst *ApplianceDeviceClaimResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "appliance.DeviceClaim.List" {
		...
		if err == nil {
			jsonApplianceDeviceClaimList, _ := json.Marshal(dst.ApplianceDeviceClaimList)
			if string(jsonApplianceDeviceClaimList) == "{}" { // empty struct
	...
	// try to unmarshal data into ApplianceDeviceClaimList
	err = json.Unmarshal(data, &dst.ApplianceDeviceClaimList)
	if err == nil {
		jsonApplianceDeviceClaimList, _ := json.Marshal(dst.ApplianceDeviceClaimList)
		if string(jsonApplianceDeviceClaimList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(ApplianceDeviceClaimResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(ApplianceDeviceClaimResponse)")
	}
}
