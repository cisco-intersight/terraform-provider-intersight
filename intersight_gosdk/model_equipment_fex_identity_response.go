...
func (dst *EquipmentFexIdentityResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "equipment.FexIdentity.List" {
		...
		if err == nil {
			jsonEquipmentFexIdentityList, _ := json.Marshal(dst.EquipmentFexIdentityList)
			if string(jsonEquipmentFexIdentityList) == "{}" { // empty struct
	...
	// try to unmarshal data into EquipmentFexIdentityList
	err = json.Unmarshal(data, &dst.EquipmentFexIdentityList)
	if err == nil {
		jsonEquipmentFexIdentityList, _ := json.Marshal(dst.EquipmentFexIdentityList)
		if string(jsonEquipmentFexIdentityList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(EquipmentFexIdentityResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(EquipmentFexIdentityResponse)")
	}
}
