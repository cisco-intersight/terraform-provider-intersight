...
func (dst *ComputeServerSettingResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "compute.ServerSetting.List" {
		...
		if err == nil {
			jsonComputeServerSettingList, _ := json.Marshal(dst.ComputeServerSettingList)
			if string(jsonComputeServerSettingList) == "{}" { // empty struct
	...
	// try to unmarshal data into ComputeServerSettingList
	err = json.Unmarshal(data, &dst.ComputeServerSettingList)
	if err == nil {
		jsonComputeServerSettingList, _ := json.Marshal(dst.ComputeServerSettingList)
		if string(jsonComputeServerSettingList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(ComputeServerSettingResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(ComputeServerSettingResponse)")
	}
}
