...
func (dst *StoragePhysicalDiskExtensionRelationship) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into StoragePhysicalDiskExtension
	err = json.Unmarshal(data, &dst.StoragePhysicalDiskExtension)
	if err == nil {
		jsonStoragePhysicalDiskExtension, _ := json.Marshal(dst.StoragePhysicalDiskExtension)
		if string(jsonStoragePhysicalDiskExtension) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(StoragePhysicalDiskExtensionRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(StoragePhysicalDiskExtensionRelationship)")
	}
}
