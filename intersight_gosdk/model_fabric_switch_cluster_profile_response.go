...
func (dst *FabricSwitchClusterProfileResponse) UnmarshalJSON(data []byte) error {
	var err error
	match := 0
	// try to unmarshal data into FabricSwitchClusterProfileList
	err = json.Unmarshal(data, &dst.FabricSwitchClusterProfileList)
	if err == nil {
		jsonFabricSwitchClusterProfileList, _ := json.Marshal(dst.FabricSwitchClusterProfileList)
		if string(jsonFabricSwitchClusterProfileList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(FabricSwitchClusterProfileResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(FabricSwitchClusterProfileResponse)")
	}
}
