...
func (dst *PolicyAbstractConfigProfileRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "policy.AbstractConfigProfile" {
		...
		if err == nil {
			jsonPolicyAbstractConfigProfile, _ := json.Marshal(dst.PolicyAbstractConfigProfile)
			if string(jsonPolicyAbstractConfigProfile) == "{}" { // empty struct
	...
	// try to unmarshal data into PolicyAbstractConfigProfile
	err = json.Unmarshal(data, &dst.PolicyAbstractConfigProfile)
	if err == nil {
		jsonPolicyAbstractConfigProfile, _ := json.Marshal(dst.PolicyAbstractConfigProfile)
		if string(jsonPolicyAbstractConfigProfile) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(PolicyAbstractConfigProfileRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(PolicyAbstractConfigProfileRelationship)")
	}
}
