...
func (dst *CapabilityPortGroupAggregationDefResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "capability.PortGroupAggregationDef.List" {
		...
		if err == nil {
			jsonCapabilityPortGroupAggregationDefList, _ := json.Marshal(dst.CapabilityPortGroupAggregationDefList)
			if string(jsonCapabilityPortGroupAggregationDefList) == "{}" { // empty struct
	...
	// try to unmarshal data into CapabilityPortGroupAggregationDefList
	err = json.Unmarshal(data, &dst.CapabilityPortGroupAggregationDefList)
	if err == nil {
		jsonCapabilityPortGroupAggregationDefList, _ := json.Marshal(dst.CapabilityPortGroupAggregationDefList)
		if string(jsonCapabilityPortGroupAggregationDefList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(CapabilityPortGroupAggregationDefResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(CapabilityPortGroupAggregationDefResponse)")
	}
}
