...
func (dst *ApplianceRestoreResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "appliance.Restore.List" {
		...
		if err == nil {
			jsonApplianceRestoreList, _ := json.Marshal(dst.ApplianceRestoreList)
			if string(jsonApplianceRestoreList) == "{}" { // empty struct
	...
	// try to unmarshal data into ApplianceRestoreList
	err = json.Unmarshal(data, &dst.ApplianceRestoreList)
	if err == nil {
		jsonApplianceRestoreList, _ := json.Marshal(dst.ApplianceRestoreList)
		if string(jsonApplianceRestoreList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(ApplianceRestoreResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(ApplianceRestoreResponse)")
	}
}
