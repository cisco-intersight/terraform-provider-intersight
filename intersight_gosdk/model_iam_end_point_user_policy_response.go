...
func (dst *IamEndPointUserPolicyResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "iam.EndPointUserPolicy.List" {
		...
		if err == nil {
			jsonIamEndPointUserPolicyList, _ := json.Marshal(dst.IamEndPointUserPolicyList)
			if string(jsonIamEndPointUserPolicyList) == "{}" { // empty struct
	...
	// try to unmarshal data into IamEndPointUserPolicyList
	err = json.Unmarshal(data, &dst.IamEndPointUserPolicyList)
	if err == nil {
		jsonIamEndPointUserPolicyList, _ := json.Marshal(dst.IamEndPointUserPolicyList)
		if string(jsonIamEndPointUserPolicyList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IamEndPointUserPolicyResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IamEndPointUserPolicyResponse)")
	}
}
