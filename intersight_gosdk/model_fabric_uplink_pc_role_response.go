...
func (dst *FabricUplinkPcRoleResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "fabric.UplinkPcRole.List" {
		...
		if err == nil {
			jsonFabricUplinkPcRoleList, _ := json.Marshal(dst.FabricUplinkPcRoleList)
			if string(jsonFabricUplinkPcRoleList) == "{}" { // empty struct
	...
	// try to unmarshal data into FabricUplinkPcRoleList
	err = json.Unmarshal(data, &dst.FabricUplinkPcRoleList)
	if err == nil {
		jsonFabricUplinkPcRoleList, _ := json.Marshal(dst.FabricUplinkPcRoleList)
		if string(jsonFabricUplinkPcRoleList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(FabricUplinkPcRoleResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(FabricUplinkPcRoleResponse)")
	}
}
