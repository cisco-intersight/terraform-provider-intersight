...
func (dst *StorageEnclosureDiskSlotEpRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "storage.EnclosureDiskSlotEp" {
		...
		if err == nil {
			jsonStorageEnclosureDiskSlotEp, _ := json.Marshal(dst.StorageEnclosureDiskSlotEp)
			if string(jsonStorageEnclosureDiskSlotEp) == "{}" { // empty struct
	...
	// try to unmarshal data into StorageEnclosureDiskSlotEp
	err = json.Unmarshal(data, &dst.StorageEnclosureDiskSlotEp)
	if err == nil {
		jsonStorageEnclosureDiskSlotEp, _ := json.Marshal(dst.StorageEnclosureDiskSlotEp)
		if string(jsonStorageEnclosureDiskSlotEp) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(StorageEnclosureDiskSlotEpRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(StorageEnclosureDiskSlotEpRelationship)")
	}
}
