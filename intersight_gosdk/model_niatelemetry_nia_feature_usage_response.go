...
func (dst *NiatelemetryNiaFeatureUsageResponse) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into NiatelemetryNiaFeatureUsageList
	err = json.Unmarshal(data, &dst.NiatelemetryNiaFeatureUsageList)
	if err == nil {
		jsonNiatelemetryNiaFeatureUsageList, _ := json.Marshal(dst.NiatelemetryNiaFeatureUsageList)
		if string(jsonNiatelemetryNiaFeatureUsageList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(NiatelemetryNiaFeatureUsageResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(NiatelemetryNiaFeatureUsageResponse)")
	}
}
