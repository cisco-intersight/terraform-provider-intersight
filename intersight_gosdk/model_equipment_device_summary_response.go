...
func (dst *EquipmentDeviceSummaryResponse) UnmarshalJSON(data []byte) error {
	var err error
	match := 0
	// try to unmarshal data into EquipmentDeviceSummaryList
	err = json.Unmarshal(data, &dst.EquipmentDeviceSummaryList)
	if err == nil {
		jsonEquipmentDeviceSummaryList, _ := json.Marshal(dst.EquipmentDeviceSummaryList)
		if string(jsonEquipmentDeviceSummaryList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(EquipmentDeviceSummaryResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(EquipmentDeviceSummaryResponse)")
	}
}
