...
func (dst *StoragePhysicalDiskResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "storage.PhysicalDisk.List" {
		...
		if err == nil {
			jsonStoragePhysicalDiskList, _ := json.Marshal(dst.StoragePhysicalDiskList)
			if string(jsonStoragePhysicalDiskList) == "{}" { // empty struct
	...
	// try to unmarshal data into StoragePhysicalDiskList
	err = json.Unmarshal(data, &dst.StoragePhysicalDiskList)
	if err == nil {
		jsonStoragePhysicalDiskList, _ := json.Marshal(dst.StoragePhysicalDiskList)
		if string(jsonStoragePhysicalDiskList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(StoragePhysicalDiskResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(StoragePhysicalDiskResponse)")
	}
}
