...
func (dst *RecoveryOnDemandBackupRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "recovery.OnDemandBackup" {
		...
		if err == nil {
			jsonRecoveryOnDemandBackup, _ := json.Marshal(dst.RecoveryOnDemandBackup)
			if string(jsonRecoveryOnDemandBackup) == "{}" { // empty struct
	...
	// try to unmarshal data into RecoveryOnDemandBackup
	err = json.Unmarshal(data, &dst.RecoveryOnDemandBackup)
	if err == nil {
		jsonRecoveryOnDemandBackup, _ := json.Marshal(dst.RecoveryOnDemandBackup)
		if string(jsonRecoveryOnDemandBackup) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(RecoveryOnDemandBackupRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(RecoveryOnDemandBackupRelationship)")
	}
}
