...
func (dst *CondHclStatusResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "cond.HclStatus.List" {
		...
		if err == nil {
			jsonCondHclStatusList, _ := json.Marshal(dst.CondHclStatusList)
			if string(jsonCondHclStatusList) == "{}" { // empty struct
	...
	// try to unmarshal data into CondHclStatusList
	err = json.Unmarshal(data, &dst.CondHclStatusList)
	if err == nil {
		jsonCondHclStatusList, _ := json.Marshal(dst.CondHclStatusList)
		if string(jsonCondHclStatusList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(CondHclStatusResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(CondHclStatusResponse)")
	}
}
