...
func (dst *ApplianceDataExportPolicyRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "appliance.DataExportPolicy" {
		...
		if err == nil {
			jsonApplianceDataExportPolicy, _ := json.Marshal(dst.ApplianceDataExportPolicy)
			if string(jsonApplianceDataExportPolicy) == "{}" { // empty struct
	...
	// try to unmarshal data into ApplianceDataExportPolicy
	err = json.Unmarshal(data, &dst.ApplianceDataExportPolicy)
	if err == nil {
		jsonApplianceDataExportPolicy, _ := json.Marshal(dst.ApplianceDataExportPolicy)
		if string(jsonApplianceDataExportPolicy) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(ApplianceDataExportPolicyRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(ApplianceDataExportPolicyRelationship)")
	}
}
