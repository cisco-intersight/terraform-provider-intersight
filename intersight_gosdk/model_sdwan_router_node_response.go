...
func (dst *SdwanRouterNodeResponse) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into SdwanRouterNodeList
	err = json.Unmarshal(data, &dst.SdwanRouterNodeList)
	if err == nil {
		jsonSdwanRouterNodeList, _ := json.Marshal(dst.SdwanRouterNodeList)
		if string(jsonSdwanRouterNodeList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(SdwanRouterNodeResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(SdwanRouterNodeResponse)")
	}
}
