...
func (dst *PciLinkRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "pci.Link" {
		...
		if err == nil {
			jsonPciLink, _ := json.Marshal(dst.PciLink)
			if string(jsonPciLink) == "{}" { // empty struct
				dst.PciLink = nil
			} else {
	...
	// try to unmarshal data into PciLink
	err = json.Unmarshal(data, &dst.PciLink)
	if err == nil {
		jsonPciLink, _ := json.Marshal(dst.PciLink)
		if string(jsonPciLink) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(PciLinkRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(PciLinkRelationship)")
	}
}
