...
func (dst *FirmwareIomDescriptorResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "firmware.IomDescriptor.List" {
		...
		if err == nil {
			jsonFirmwareIomDescriptorList, _ := json.Marshal(dst.FirmwareIomDescriptorList)
			if string(jsonFirmwareIomDescriptorList) == "{}" { // empty struct
	...
	// try to unmarshal data into FirmwareIomDescriptorList
	err = json.Unmarshal(data, &dst.FirmwareIomDescriptorList)
	if err == nil {
		jsonFirmwareIomDescriptorList, _ := json.Marshal(dst.FirmwareIomDescriptorList)
		if string(jsonFirmwareIomDescriptorList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(FirmwareIomDescriptorResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(FirmwareIomDescriptorResponse)")
	}
}
