...
func (dst *MemoryPersistentMemoryPolicyResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "memory.PersistentMemoryPolicy.List" {
		...
		if err == nil {
			jsonMemoryPersistentMemoryPolicyList, _ := json.Marshal(dst.MemoryPersistentMemoryPolicyList)
			if string(jsonMemoryPersistentMemoryPolicyList) == "{}" { // empty struct
	...
	// try to unmarshal data into MemoryPersistentMemoryPolicyList
	err = json.Unmarshal(data, &dst.MemoryPersistentMemoryPolicyList)
	if err == nil {
		jsonMemoryPersistentMemoryPolicyList, _ := json.Marshal(dst.MemoryPersistentMemoryPolicyList)
		if string(jsonMemoryPersistentMemoryPolicyList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(MemoryPersistentMemoryPolicyResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(MemoryPersistentMemoryPolicyResponse)")
	}
}
