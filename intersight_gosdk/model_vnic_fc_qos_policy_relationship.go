...
func (dst *VnicFcQosPolicyRelationship) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into VnicFcQosPolicy
	err = json.Unmarshal(data, &dst.VnicFcQosPolicy)
	if err == nil {
		jsonVnicFcQosPolicy, _ := json.Marshal(dst.VnicFcQosPolicy)
		if string(jsonVnicFcQosPolicy) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(VnicFcQosPolicyRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(VnicFcQosPolicyRelationship)")
	}
}
