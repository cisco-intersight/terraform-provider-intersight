...
func (dst *FirmwareDistributableResponse) UnmarshalJSON(data []byte) error {
	var err error
	match := 0
	// try to unmarshal data into FirmwareDistributableList
	err = json.Unmarshal(data, &dst.FirmwareDistributableList)
	if err == nil {
		jsonFirmwareDistributableList, _ := json.Marshal(dst.FirmwareDistributableList)
		if string(jsonFirmwareDistributableList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(FirmwareDistributableResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(FirmwareDistributableResponse)")
	}
}
