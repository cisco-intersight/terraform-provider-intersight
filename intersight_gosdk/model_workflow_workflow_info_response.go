...
func (dst *WorkflowWorkflowInfoResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "workflow.WorkflowInfo.List" {
		...
		if err == nil {
			jsonWorkflowWorkflowInfoList, _ := json.Marshal(dst.WorkflowWorkflowInfoList)
			if string(jsonWorkflowWorkflowInfoList) == "{}" { // empty struct
	...
	// try to unmarshal data into WorkflowWorkflowInfoList
	err = json.Unmarshal(data, &dst.WorkflowWorkflowInfoList)
	if err == nil {
		jsonWorkflowWorkflowInfoList, _ := json.Marshal(dst.WorkflowWorkflowInfoList)
		if string(jsonWorkflowWorkflowInfoList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(WorkflowWorkflowInfoResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(WorkflowWorkflowInfoResponse)")
	}
}
