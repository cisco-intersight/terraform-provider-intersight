...
func (dst *IaasUcsdManagedInfraRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "iaas.UcsdManagedInfra" {
		...
		if err == nil {
			jsonIaasUcsdManagedInfra, _ := json.Marshal(dst.IaasUcsdManagedInfra)
			if string(jsonIaasUcsdManagedInfra) == "{}" { // empty struct
	...
	// try to unmarshal data into IaasUcsdManagedInfra
	err = json.Unmarshal(data, &dst.IaasUcsdManagedInfra)
	if err == nil {
		jsonIaasUcsdManagedInfra, _ := json.Marshal(dst.IaasUcsdManagedInfra)
		if string(jsonIaasUcsdManagedInfra) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IaasUcsdManagedInfraRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IaasUcsdManagedInfraRelationship)")
	}
}
