...
func (dst *SecurityUnitRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "security.Unit" {
		...
		if err == nil {
			jsonSecurityUnit, _ := json.Marshal(dst.SecurityUnit)
			if string(jsonSecurityUnit) == "{}" { // empty struct
	...
	// try to unmarshal data into SecurityUnit
	err = json.Unmarshal(data, &dst.SecurityUnit)
	if err == nil {
		jsonSecurityUnit, _ := json.Marshal(dst.SecurityUnit)
		if string(jsonSecurityUnit) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(SecurityUnitRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(SecurityUnitRelationship)")
	}
}
