...
func (dst *FabricVlanResponse) UnmarshalJSON(data []byte) error {
	var err error
	match := 0
	// try to unmarshal data into FabricVlanList
	err = json.Unmarshal(data, &dst.FabricVlanList)
	if err == nil {
		jsonFabricVlanList, _ := json.Marshal(dst.FabricVlanList)
		if string(jsonFabricVlanList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(FabricVlanResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(FabricVlanResponse)")
	}
}
