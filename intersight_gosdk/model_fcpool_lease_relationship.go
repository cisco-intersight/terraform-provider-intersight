...
func (dst *FcpoolLeaseRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "fcpool.Lease" {
		...
		if err == nil {
			jsonFcpoolLease, _ := json.Marshal(dst.FcpoolLease)
			if string(jsonFcpoolLease) == "{}" { // empty struct
				dst.FcpoolLease = nil
			} else {
	...
	// try to unmarshal data into FcpoolLease
	err = json.Unmarshal(data, &dst.FcpoolLease)
	if err == nil {
		jsonFcpoolLease, _ := json.Marshal(dst.FcpoolLease)
		if string(jsonFcpoolLease) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(FcpoolLeaseRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(FcpoolLeaseRelationship)")
	}
}
