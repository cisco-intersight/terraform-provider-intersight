...
func (dst *PortMacBindingResponse) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into PortMacBindingList
	err = json.Unmarshal(data, &dst.PortMacBindingList)
	if err == nil {
		jsonPortMacBindingList, _ := json.Marshal(dst.PortMacBindingList)
		if string(jsonPortMacBindingList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(PortMacBindingResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(PortMacBindingResponse)")
	}
}
