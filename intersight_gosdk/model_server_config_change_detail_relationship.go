...
func (dst *ServerConfigChangeDetailRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "server.ConfigChangeDetail" {
		...
		if err == nil {
			jsonServerConfigChangeDetail, _ := json.Marshal(dst.ServerConfigChangeDetail)
			if string(jsonServerConfigChangeDetail) == "{}" { // empty struct
	...
	// try to unmarshal data into ServerConfigChangeDetail
	err = json.Unmarshal(data, &dst.ServerConfigChangeDetail)
	if err == nil {
		jsonServerConfigChangeDetail, _ := json.Marshal(dst.ServerConfigChangeDetail)
		if string(jsonServerConfigChangeDetail) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(ServerConfigChangeDetailRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(ServerConfigChangeDetailRelationship)")
	}
}
