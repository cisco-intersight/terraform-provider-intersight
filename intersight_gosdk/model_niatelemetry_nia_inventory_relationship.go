...
func (dst *NiatelemetryNiaInventoryRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "niatelemetry.NiaInventory" {
		...
		if err == nil {
			jsonNiatelemetryNiaInventory, _ := json.Marshal(dst.NiatelemetryNiaInventory)
			if string(jsonNiatelemetryNiaInventory) == "{}" { // empty struct
	...
	// try to unmarshal data into NiatelemetryNiaInventory
	err = json.Unmarshal(data, &dst.NiatelemetryNiaInventory)
	if err == nil {
		jsonNiatelemetryNiaInventory, _ := json.Marshal(dst.NiatelemetryNiaInventory)
		if string(jsonNiatelemetryNiaInventory) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(NiatelemetryNiaInventoryRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(NiatelemetryNiaInventoryRelationship)")
	}
}
