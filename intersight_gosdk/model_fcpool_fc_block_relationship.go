...
func (dst *FcpoolFcBlockRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "fcpool.FcBlock" {
		...
		if err == nil {
			jsonFcpoolFcBlock, _ := json.Marshal(dst.FcpoolFcBlock)
			if string(jsonFcpoolFcBlock) == "{}" { // empty struct
	...
	// try to unmarshal data into FcpoolFcBlock
	err = json.Unmarshal(data, &dst.FcpoolFcBlock)
	if err == nil {
		jsonFcpoolFcBlock, _ := json.Marshal(dst.FcpoolFcBlock)
		if string(jsonFcpoolFcBlock) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(FcpoolFcBlockRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(FcpoolFcBlockRelationship)")
	}
}
