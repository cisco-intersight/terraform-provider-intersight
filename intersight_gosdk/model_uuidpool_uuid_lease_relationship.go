...
func (dst *UuidpoolUuidLeaseRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "uuidpool.UuidLease" {
		...
		if err == nil {
			jsonUuidpoolUuidLease, _ := json.Marshal(dst.UuidpoolUuidLease)
			if string(jsonUuidpoolUuidLease) == "{}" { // empty struct
	...
	// try to unmarshal data into UuidpoolUuidLease
	err = json.Unmarshal(data, &dst.UuidpoolUuidLease)
	if err == nil {
		jsonUuidpoolUuidLease, _ := json.Marshal(dst.UuidpoolUuidLease)
		if string(jsonUuidpoolUuidLease) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(UuidpoolUuidLeaseRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(UuidpoolUuidLeaseRelationship)")
	}
}
