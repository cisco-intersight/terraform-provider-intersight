...
func (dst *BiosBootDeviceResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "bios.BootDevice.List" {
		...
		if err == nil {
			jsonBiosBootDeviceList, _ := json.Marshal(dst.BiosBootDeviceList)
			if string(jsonBiosBootDeviceList) == "{}" { // empty struct
	...
	// try to unmarshal data into BiosBootDeviceList
	err = json.Unmarshal(data, &dst.BiosBootDeviceList)
	if err == nil {
		jsonBiosBootDeviceList, _ := json.Marshal(dst.BiosBootDeviceList)
		if string(jsonBiosBootDeviceList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(BiosBootDeviceResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(BiosBootDeviceResponse)")
	}
}
