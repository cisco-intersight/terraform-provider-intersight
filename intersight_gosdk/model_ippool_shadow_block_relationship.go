...
func (dst *IppoolShadowBlockRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "ippool.ShadowBlock" {
		...
		if err == nil {
			jsonIppoolShadowBlock, _ := json.Marshal(dst.IppoolShadowBlock)
			if string(jsonIppoolShadowBlock) == "{}" { // empty struct
	...
	// try to unmarshal data into IppoolShadowBlock
	err = json.Unmarshal(data, &dst.IppoolShadowBlock)
	if err == nil {
		jsonIppoolShadowBlock, _ := json.Marshal(dst.IppoolShadowBlock)
		if string(jsonIppoolShadowBlock) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IppoolShadowBlockRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IppoolShadowBlockRelationship)")
	}
}
