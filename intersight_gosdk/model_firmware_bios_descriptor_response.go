...
func (dst *FirmwareBiosDescriptorResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "firmware.BiosDescriptor.List" {
		...
		if err == nil {
			jsonFirmwareBiosDescriptorList, _ := json.Marshal(dst.FirmwareBiosDescriptorList)
			if string(jsonFirmwareBiosDescriptorList) == "{}" { // empty struct
	...
	// try to unmarshal data into FirmwareBiosDescriptorList
	err = json.Unmarshal(data, &dst.FirmwareBiosDescriptorList)
	if err == nil {
		jsonFirmwareBiosDescriptorList, _ := json.Marshal(dst.FirmwareBiosDescriptorList)
		if string(jsonFirmwareBiosDescriptorList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(FirmwareBiosDescriptorResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(FirmwareBiosDescriptorResponse)")
	}
}
