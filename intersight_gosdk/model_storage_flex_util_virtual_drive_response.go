...
func (dst *StorageFlexUtilVirtualDriveResponse) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into StorageFlexUtilVirtualDriveList
	err = json.Unmarshal(data, &dst.StorageFlexUtilVirtualDriveList)
	if err == nil {
		jsonStorageFlexUtilVirtualDriveList, _ := json.Marshal(dst.StorageFlexUtilVirtualDriveList)
		if string(jsonStorageFlexUtilVirtualDriveList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(StorageFlexUtilVirtualDriveResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(StorageFlexUtilVirtualDriveResponse)")
	}
}
