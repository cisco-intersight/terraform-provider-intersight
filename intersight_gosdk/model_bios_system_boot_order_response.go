...
func (dst *BiosSystemBootOrderResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "bios.SystemBootOrder.List" {
		...
		if err == nil {
			jsonBiosSystemBootOrderList, _ := json.Marshal(dst.BiosSystemBootOrderList)
			if string(jsonBiosSystemBootOrderList) == "{}" { // empty struct
	...
	// try to unmarshal data into BiosSystemBootOrderList
	err = json.Unmarshal(data, &dst.BiosSystemBootOrderList)
	if err == nil {
		jsonBiosSystemBootOrderList, _ := json.Marshal(dst.BiosSystemBootOrderList)
		if string(jsonBiosSystemBootOrderList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(BiosSystemBootOrderResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(BiosSystemBootOrderResponse)")
	}
}
