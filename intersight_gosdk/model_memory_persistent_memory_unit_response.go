...
func (dst *MemoryPersistentMemoryUnitResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "memory.PersistentMemoryUnit.List" {
		...
		if err == nil {
			jsonMemoryPersistentMemoryUnitList, _ := json.Marshal(dst.MemoryPersistentMemoryUnitList)
			if string(jsonMemoryPersistentMemoryUnitList) == "{}" { // empty struct
	...
	// try to unmarshal data into MemoryPersistentMemoryUnitList
	err = json.Unmarshal(data, &dst.MemoryPersistentMemoryUnitList)
	if err == nil {
		jsonMemoryPersistentMemoryUnitList, _ := json.Marshal(dst.MemoryPersistentMemoryUnitList)
		if string(jsonMemoryPersistentMemoryUnitList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(MemoryPersistentMemoryUnitResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(MemoryPersistentMemoryUnitResponse)")
	}
}
