...
func (dst *IamPermissionRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "iam.Permission" {
		...
		if err == nil {
			jsonIamPermission, _ := json.Marshal(dst.IamPermission)
			if string(jsonIamPermission) == "{}" { // empty struct
	...
	// try to unmarshal data into IamPermission
	err = json.Unmarshal(data, &dst.IamPermission)
	if err == nil {
		jsonIamPermission, _ := json.Marshal(dst.IamPermission)
		if string(jsonIamPermission) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IamPermissionRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IamPermissionRelationship)")
	}
}
