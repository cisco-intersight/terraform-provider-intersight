...
func (dst *PortSubGroupRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "port.SubGroup" {
		...
		if err == nil {
			jsonPortSubGroup, _ := json.Marshal(dst.PortSubGroup)
			if string(jsonPortSubGroup) == "{}" { // empty struct
	...
	// try to unmarshal data into PortSubGroup
	err = json.Unmarshal(data, &dst.PortSubGroup)
	if err == nil {
		jsonPortSubGroup, _ := json.Marshal(dst.PortSubGroup)
		if string(jsonPortSubGroup) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(PortSubGroupRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(PortSubGroupRelationship)")
	}
}
