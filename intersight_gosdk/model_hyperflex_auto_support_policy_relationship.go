...
func (dst *HyperflexAutoSupportPolicyRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "hyperflex.AutoSupportPolicy" {
		...
		if err == nil {
			jsonHyperflexAutoSupportPolicy, _ := json.Marshal(dst.HyperflexAutoSupportPolicy)
			if string(jsonHyperflexAutoSupportPolicy) == "{}" { // empty struct
	...
	// try to unmarshal data into HyperflexAutoSupportPolicy
	err = json.Unmarshal(data, &dst.HyperflexAutoSupportPolicy)
	if err == nil {
		jsonHyperflexAutoSupportPolicy, _ := json.Marshal(dst.HyperflexAutoSupportPolicy)
		if string(jsonHyperflexAutoSupportPolicy) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(HyperflexAutoSupportPolicyRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(HyperflexAutoSupportPolicyRelationship)")
	}
}
