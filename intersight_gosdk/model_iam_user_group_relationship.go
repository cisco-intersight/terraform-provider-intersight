...
func (dst *IamUserGroupRelationship) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into IamUserGroup
	err = json.Unmarshal(data, &dst.IamUserGroup)
	if err == nil {
		jsonIamUserGroup, _ := json.Marshal(dst.IamUserGroup)
		if string(jsonIamUserGroup) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IamUserGroupRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IamUserGroupRelationship)")
	}
}
