...
func (dst *NiaapiApicLatestMaintainedReleaseResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "niaapi.ApicLatestMaintainedRelease.List" {
		...
		if err == nil {
			jsonNiaapiApicLatestMaintainedReleaseList, _ := json.Marshal(dst.NiaapiApicLatestMaintainedReleaseList)
			if string(jsonNiaapiApicLatestMaintainedReleaseList) == "{}" { // empty struct
	...
	// try to unmarshal data into NiaapiApicLatestMaintainedReleaseList
	err = json.Unmarshal(data, &dst.NiaapiApicLatestMaintainedReleaseList)
	if err == nil {
		jsonNiaapiApicLatestMaintainedReleaseList, _ := json.Marshal(dst.NiaapiApicLatestMaintainedReleaseList)
		if string(jsonNiaapiApicLatestMaintainedReleaseList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(NiaapiApicLatestMaintainedReleaseResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(NiaapiApicLatestMaintainedReleaseResponse)")
	}
}
