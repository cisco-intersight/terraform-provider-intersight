...
func (dst *IppoolShadowPoolResponse) UnmarshalJSON(data []byte) error {
	var err error
	match := 0
	// try to unmarshal data into IppoolShadowPoolList
	err = json.Unmarshal(data, &dst.IppoolShadowPoolList)
	if err == nil {
		jsonIppoolShadowPoolList, _ := json.Marshal(dst.IppoolShadowPoolList)
		if string(jsonIppoolShadowPoolList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IppoolShadowPoolResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IppoolShadowPoolResponse)")
	}
}
