...
func (dst *PciLinkResponse) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into PciLinkList
	err = json.Unmarshal(data, &dst.PciLinkList)
	if err == nil {
		jsonPciLinkList, _ := json.Marshal(dst.PciLinkList)
		if string(jsonPciLinkList) == "{}" { // empty struct
			dst.PciLinkList = nil
		} else {
			match++
		}
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(PciLinkResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(PciLinkResponse)")
	}
}
