...
func (dst *HclServiceStatusResponse) UnmarshalJSON(data []byte) error {
	var err error
	match := 0
	// try to unmarshal data into HclServiceStatusList
	err = json.Unmarshal(data, &dst.HclServiceStatusList)
	if err == nil {
		jsonHclServiceStatusList, _ := json.Marshal(dst.HclServiceStatusList)
		if string(jsonHclServiceStatusList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(HclServiceStatusResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(HclServiceStatusResponse)")
	}
}
