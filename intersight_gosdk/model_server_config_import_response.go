...
func (dst *ServerConfigImportResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "server.ConfigImport.List" {
		...
		if err == nil {
			jsonServerConfigImportList, _ := json.Marshal(dst.ServerConfigImportList)
			if string(jsonServerConfigImportList) == "{}" { // empty struct
	...
	// try to unmarshal data into ServerConfigImportList
	err = json.Unmarshal(data, &dst.ServerConfigImportList)
	if err == nil {
		jsonServerConfigImportList, _ := json.Marshal(dst.ServerConfigImportList)
		if string(jsonServerConfigImportList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(ServerConfigImportResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(ServerConfigImportResponse)")
	}
}
