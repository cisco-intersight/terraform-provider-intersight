...
func (dst *HyperflexClusterNetworkPolicyResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "hyperflex.ClusterNetworkPolicy.List" {
		...
		if err == nil {
			jsonHyperflexClusterNetworkPolicyList, _ := json.Marshal(dst.HyperflexClusterNetworkPolicyList)
			if string(jsonHyperflexClusterNetworkPolicyList) == "{}" { // empty struct
	...
	// try to unmarshal data into HyperflexClusterNetworkPolicyList
	err = json.Unmarshal(data, &dst.HyperflexClusterNetworkPolicyList)
	if err == nil {
		jsonHyperflexClusterNetworkPolicyList, _ := json.Marshal(dst.HyperflexClusterNetworkPolicyList)
		if string(jsonHyperflexClusterNetworkPolicyList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(HyperflexClusterNetworkPolicyResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(HyperflexClusterNetworkPolicyResponse)")
	}
}
