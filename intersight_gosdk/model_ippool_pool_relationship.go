...
func (dst *IppoolPoolRelationship) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into IppoolPool
	err = json.Unmarshal(data, &dst.IppoolPool)
	if err == nil {
		jsonIppoolPool, _ := json.Marshal(dst.IppoolPool)
		if string(jsonIppoolPool) == "{}" { // empty struct
			dst.IppoolPool = nil
		} else {
			match++
		}
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IppoolPoolRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IppoolPoolRelationship)")
	}
}
