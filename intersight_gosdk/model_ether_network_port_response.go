...
func (dst *EtherNetworkPortResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "ether.NetworkPort.List" {
		...
		if err == nil {
			jsonEtherNetworkPortList, _ := json.Marshal(dst.EtherNetworkPortList)
			if string(jsonEtherNetworkPortList) == "{}" { // empty struct
	...
	// try to unmarshal data into EtherNetworkPortList
	err = json.Unmarshal(data, &dst.EtherNetworkPortList)
	if err == nil {
		jsonEtherNetworkPortList, _ := json.Marshal(dst.EtherNetworkPortList)
		if string(jsonEtherNetworkPortList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(EtherNetworkPortResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(EtherNetworkPortResponse)")
	}
}
