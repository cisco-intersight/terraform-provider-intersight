...
func (dst *SoftwareSolutionDistributableRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "software.SolutionDistributable" {
		...
		if err == nil {
			jsonSoftwareSolutionDistributable, _ := json.Marshal(dst.SoftwareSolutionDistributable)
			if string(jsonSoftwareSolutionDistributable) == "{}" { // empty struct
	...
	// try to unmarshal data into SoftwareSolutionDistributable
	err = json.Unmarshal(data, &dst.SoftwareSolutionDistributable)
	if err == nil {
		jsonSoftwareSolutionDistributable, _ := json.Marshal(dst.SoftwareSolutionDistributable)
		if string(jsonSoftwareSolutionDistributable) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(SoftwareSolutionDistributableRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(SoftwareSolutionDistributableRelationship)")
	}
}
