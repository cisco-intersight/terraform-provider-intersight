...
func (dst *StorageDiskGroupRelationship) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into StorageDiskGroup
	err = json.Unmarshal(data, &dst.StorageDiskGroup)
	if err == nil {
		jsonStorageDiskGroup, _ := json.Marshal(dst.StorageDiskGroup)
		if string(jsonStorageDiskGroup) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(StorageDiskGroupRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(StorageDiskGroupRelationship)")
	}
}
