...
func (dst *BiosSystemBootOrderRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "bios.SystemBootOrder" {
		...
		if err == nil {
			jsonBiosSystemBootOrder, _ := json.Marshal(dst.BiosSystemBootOrder)
			if string(jsonBiosSystemBootOrder) == "{}" { // empty struct
	...
	// try to unmarshal data into BiosSystemBootOrder
	err = json.Unmarshal(data, &dst.BiosSystemBootOrder)
	if err == nil {
		jsonBiosSystemBootOrder, _ := json.Marshal(dst.BiosSystemBootOrder)
		if string(jsonBiosSystemBootOrder) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(BiosSystemBootOrderRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(BiosSystemBootOrderRelationship)")
	}
}
