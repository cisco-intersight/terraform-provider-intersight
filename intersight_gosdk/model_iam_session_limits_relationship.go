...
func (dst *IamSessionLimitsRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "iam.SessionLimits" {
		...
		if err == nil {
			jsonIamSessionLimits, _ := json.Marshal(dst.IamSessionLimits)
			if string(jsonIamSessionLimits) == "{}" { // empty struct
	...
	// try to unmarshal data into IamSessionLimits
	err = json.Unmarshal(data, &dst.IamSessionLimits)
	if err == nil {
		jsonIamSessionLimits, _ := json.Marshal(dst.IamSessionLimits)
		if string(jsonIamSessionLimits) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IamSessionLimitsRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IamSessionLimitsRelationship)")
	}
}
