...
func (dst *CapabilityEquipmentPhysicalDefResponse) UnmarshalJSON(data []byte) error {
	var err error
	match := 0
	// try to unmarshal data into CapabilityEquipmentPhysicalDefList
	err = json.Unmarshal(data, &dst.CapabilityEquipmentPhysicalDefList)
	if err == nil {
		jsonCapabilityEquipmentPhysicalDefList, _ := json.Marshal(dst.CapabilityEquipmentPhysicalDefList)
		if string(jsonCapabilityEquipmentPhysicalDefList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(CapabilityEquipmentPhysicalDefResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(CapabilityEquipmentPhysicalDefResponse)")
	}
}
