...
func (dst *LsServiceProfileResponse) UnmarshalJSON(data []byte) error {
	var err error
	match := 0
	// try to unmarshal data into LsServiceProfileList
	err = json.Unmarshal(data, &dst.LsServiceProfileList)
	if err == nil {
		jsonLsServiceProfileList, _ := json.Marshal(dst.LsServiceProfileList)
		if string(jsonLsServiceProfileList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(LsServiceProfileResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(LsServiceProfileResponse)")
	}
}
