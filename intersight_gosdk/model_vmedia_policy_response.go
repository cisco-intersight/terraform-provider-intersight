...
func (dst *VmediaPolicyResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "vmedia.Policy.List" {
		...
		if err == nil {
			jsonVmediaPolicyList, _ := json.Marshal(dst.VmediaPolicyList)
			if string(jsonVmediaPolicyList) == "{}" { // empty struct
	...
	// try to unmarshal data into VmediaPolicyList
	err = json.Unmarshal(data, &dst.VmediaPolicyList)
	if err == nil {
		jsonVmediaPolicyList, _ := json.Marshal(dst.VmediaPolicyList)
		if string(jsonVmediaPolicyList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(VmediaPolicyResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(VmediaPolicyResponse)")
	}
}
