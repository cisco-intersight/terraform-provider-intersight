...
func (dst *ManagementControllerRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "management.Controller" {
		...
		if err == nil {
			jsonManagementController, _ := json.Marshal(dst.ManagementController)
			if string(jsonManagementController) == "{}" { // empty struct
	...
	// try to unmarshal data into ManagementController
	err = json.Unmarshal(data, &dst.ManagementController)
	if err == nil {
		jsonManagementController, _ := json.Marshal(dst.ManagementController)
		if string(jsonManagementController) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(ManagementControllerRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(ManagementControllerRelationship)")
	}
}
