...
func (dst *WorkflowBuildTaskMetaResponse) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into WorkflowBuildTaskMetaList
	err = json.Unmarshal(data, &dst.WorkflowBuildTaskMetaList)
	if err == nil {
		jsonWorkflowBuildTaskMetaList, _ := json.Marshal(dst.WorkflowBuildTaskMetaList)
		if string(jsonWorkflowBuildTaskMetaList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(WorkflowBuildTaskMetaResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(WorkflowBuildTaskMetaResponse)")
	}
}
