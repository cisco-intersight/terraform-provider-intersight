...
func (dst *IaasDeviceStatusRelationship) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into IaasDeviceStatus
	err = json.Unmarshal(data, &dst.IaasDeviceStatus)
	if err == nil {
		jsonIaasDeviceStatus, _ := json.Marshal(dst.IaasDeviceStatus)
		if string(jsonIaasDeviceStatus) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IaasDeviceStatusRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IaasDeviceStatusRelationship)")
	}
}
