...
func MoAggregateTransformAsProcessorUnitResponse(v *MoAggregateTransform) ProcessorUnitResponse {
	return ProcessorUnitResponse{MoAggregateTransform: v}
}
...
func MoDocumentCountAsProcessorUnitResponse(v *MoDocumentCount) ProcessorUnitResponse {
	return ProcessorUnitResponse{MoDocumentCount: v}
}
...
func MoTagSummaryAsProcessorUnitResponse(v *MoTagSummary) ProcessorUnitResponse {
	return ProcessorUnitResponse{MoTagSummary: v}
}
...
func ProcessorUnitListAsProcessorUnitResponse(v *ProcessorUnitList) ProcessorUnitResponse {
	return ProcessorUnitResponse{ProcessorUnitList: v}
}
...
func (dst *ProcessorUnitResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "processor.Unit.List" {
		...
		if err == nil {
			jsonProcessorUnitList, _ := json.Marshal(dst.ProcessorUnitList)
			if string(jsonProcessorUnitList) == "{}" { // empty struct
	...
	// try to unmarshal data into ProcessorUnitList
	err = json.Unmarshal(data, &dst.ProcessorUnitList)
	if err == nil {
		jsonProcessorUnitList, _ := json.Marshal(dst.ProcessorUnitList)
		if string(jsonProcessorUnitList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(ProcessorUnitResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(ProcessorUnitResponse)")
	}
}
