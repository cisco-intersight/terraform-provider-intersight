...
func (dst *WorkflowCatalogResponse) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into WorkflowCatalogList
	err = json.Unmarshal(data, &dst.WorkflowCatalogList)
	if err == nil {
		jsonWorkflowCatalogList, _ := json.Marshal(dst.WorkflowCatalogList)
		if string(jsonWorkflowCatalogList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(WorkflowCatalogResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(WorkflowCatalogResponse)")
	}
}
