...
func (dst *WorkflowWorkflowMetaResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "workflow.WorkflowMeta.List" {
		...
		if err == nil {
			jsonWorkflowWorkflowMetaList, _ := json.Marshal(dst.WorkflowWorkflowMetaList)
			if string(jsonWorkflowWorkflowMetaList) == "{}" { // empty struct
	...
	// try to unmarshal data into WorkflowWorkflowMetaList
	err = json.Unmarshal(data, &dst.WorkflowWorkflowMetaList)
	if err == nil {
		jsonWorkflowWorkflowMetaList, _ := json.Marshal(dst.WorkflowWorkflowMetaList)
		if string(jsonWorkflowWorkflowMetaList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(WorkflowWorkflowMetaResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(WorkflowWorkflowMetaResponse)")
	}
}
