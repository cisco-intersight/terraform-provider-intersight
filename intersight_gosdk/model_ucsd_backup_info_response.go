...
func (dst *UcsdBackupInfoResponse) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into UcsdBackupInfoList
	err = json.Unmarshal(data, &dst.UcsdBackupInfoList)
	if err == nil {
		jsonUcsdBackupInfoList, _ := json.Marshal(dst.UcsdBackupInfoList)
		if string(jsonUcsdBackupInfoList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(UcsdBackupInfoResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(UcsdBackupInfoResponse)")
	}
}
