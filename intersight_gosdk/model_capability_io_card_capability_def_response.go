...
func (dst *CapabilityIoCardCapabilityDefResponse) UnmarshalJSON(data []byte) error {
	var err error
	match := 0
	// try to unmarshal data into CapabilityIoCardCapabilityDefList
	err = json.Unmarshal(data, &dst.CapabilityIoCardCapabilityDefList)
	if err == nil {
		jsonCapabilityIoCardCapabilityDefList, _ := json.Marshal(dst.CapabilityIoCardCapabilityDefList)
		if string(jsonCapabilityIoCardCapabilityDefList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(CapabilityIoCardCapabilityDefResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(CapabilityIoCardCapabilityDefResponse)")
	}
}
