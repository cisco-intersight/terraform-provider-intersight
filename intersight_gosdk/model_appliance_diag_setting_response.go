...
func (dst *ApplianceDiagSettingResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "appliance.DiagSetting.List" {
		...
		if err == nil {
			jsonApplianceDiagSettingList, _ := json.Marshal(dst.ApplianceDiagSettingList)
			if string(jsonApplianceDiagSettingList) == "{}" { // empty struct
	...
	// try to unmarshal data into ApplianceDiagSettingList
	err = json.Unmarshal(data, &dst.ApplianceDiagSettingList)
	if err == nil {
		jsonApplianceDiagSettingList, _ := json.Marshal(dst.ApplianceDiagSettingList)
		if string(jsonApplianceDiagSettingList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(ApplianceDiagSettingResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(ApplianceDiagSettingResponse)")
	}
}
