...
func (dst *WorkflowTaskDefinitionResponse) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into WorkflowTaskDefinitionList
	err = json.Unmarshal(data, &dst.WorkflowTaskDefinitionList)
	if err == nil {
		jsonWorkflowTaskDefinitionList, _ := json.Marshal(dst.WorkflowTaskDefinitionList)
		if string(jsonWorkflowTaskDefinitionList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(WorkflowTaskDefinitionResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(WorkflowTaskDefinitionResponse)")
	}
}
