...
func (dst *HyperflexServerModelRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "hyperflex.ServerModel" {
		...
		if err == nil {
			jsonHyperflexServerModel, _ := json.Marshal(dst.HyperflexServerModel)
			if string(jsonHyperflexServerModel) == "{}" { // empty struct
	...
	// try to unmarshal data into HyperflexServerModel
	err = json.Unmarshal(data, &dst.HyperflexServerModel)
	if err == nil {
		jsonHyperflexServerModel, _ := json.Marshal(dst.HyperflexServerModel)
		if string(jsonHyperflexServerModel) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(HyperflexServerModelRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(HyperflexServerModelRelationship)")
	}
}
