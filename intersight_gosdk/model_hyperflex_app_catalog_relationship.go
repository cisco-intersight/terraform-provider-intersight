...
func (dst *HyperflexAppCatalogRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "hyperflex.AppCatalog" {
		...
		if err == nil {
			jsonHyperflexAppCatalog, _ := json.Marshal(dst.HyperflexAppCatalog)
			if string(jsonHyperflexAppCatalog) == "{}" { // empty struct
	...
	// try to unmarshal data into HyperflexAppCatalog
	err = json.Unmarshal(data, &dst.HyperflexAppCatalog)
	if err == nil {
		jsonHyperflexAppCatalog, _ := json.Marshal(dst.HyperflexAppCatalog)
		if string(jsonHyperflexAppCatalog) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(HyperflexAppCatalogRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(HyperflexAppCatalogRelationship)")
	}
}
