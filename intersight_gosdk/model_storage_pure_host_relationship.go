...
func (dst *StoragePureHostRelationship) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into StoragePureHost
	err = json.Unmarshal(data, &dst.StoragePureHost)
	if err == nil {
		jsonStoragePureHost, _ := json.Marshal(dst.StoragePureHost)
		if string(jsonStoragePureHost) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(StoragePureHostRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(StoragePureHostRelationship)")
	}
}
