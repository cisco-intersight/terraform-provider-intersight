...
func (dst *IamApiKeyResponse) UnmarshalJSON(data []byte) error {
	var err error
	match := 0
	// try to unmarshal data into IamApiKeyList
	err = json.Unmarshal(data, &dst.IamApiKeyList)
	if err == nil {
		jsonIamApiKeyList, _ := json.Marshal(dst.IamApiKeyList)
		if string(jsonIamApiKeyList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IamApiKeyResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IamApiKeyResponse)")
	}
}
