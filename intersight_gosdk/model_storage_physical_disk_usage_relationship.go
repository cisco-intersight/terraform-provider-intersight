...
func MoMoRefAsStoragePhysicalDiskUsageRelationship(v *MoMoRef) StoragePhysicalDiskUsageRelationship {
	return StoragePhysicalDiskUsageRelationship{MoMoRef: v}
}
...
func StoragePhysicalDiskUsageAsStoragePhysicalDiskUsageRelationship(v *StoragePhysicalDiskUsage) StoragePhysicalDiskUsageRelationship {
	return StoragePhysicalDiskUsageRelationship{StoragePhysicalDiskUsage: v}
}
...
func (dst *StoragePhysicalDiskUsageRelationship) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into StoragePhysicalDiskUsage
	err = json.Unmarshal(data, &dst.StoragePhysicalDiskUsage)
	if err == nil {
		jsonStoragePhysicalDiskUsage, _ := json.Marshal(dst.StoragePhysicalDiskUsage)
		if string(jsonStoragePhysicalDiskUsage) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(StoragePhysicalDiskUsageRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(StoragePhysicalDiskUsageRelationship)")
	}
}
