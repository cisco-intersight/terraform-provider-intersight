...
func (dst *NiaapiVersionRegexResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "niaapi.VersionRegex.List" {
		...
		if err == nil {
			jsonNiaapiVersionRegexList, _ := json.Marshal(dst.NiaapiVersionRegexList)
			if string(jsonNiaapiVersionRegexList) == "{}" { // empty struct
	...
	// try to unmarshal data into NiaapiVersionRegexList
	err = json.Unmarshal(data, &dst.NiaapiVersionRegexList)
	if err == nil {
		jsonNiaapiVersionRegexList, _ := json.Marshal(dst.NiaapiVersionRegexList)
		if string(jsonNiaapiVersionRegexList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(NiaapiVersionRegexResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(NiaapiVersionRegexResponse)")
	}
}
