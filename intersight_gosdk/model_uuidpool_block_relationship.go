...
func (dst *UuidpoolBlockRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "uuidpool.Block" {
		...
		if err == nil {
			jsonUuidpoolBlock, _ := json.Marshal(dst.UuidpoolBlock)
			if string(jsonUuidpoolBlock) == "{}" { // empty struct
	...
	// try to unmarshal data into UuidpoolBlock
	err = json.Unmarshal(data, &dst.UuidpoolBlock)
	if err == nil {
		jsonUuidpoolBlock, _ := json.Marshal(dst.UuidpoolBlock)
		if string(jsonUuidpoolBlock) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(UuidpoolBlockRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(UuidpoolBlockRelationship)")
	}
}
