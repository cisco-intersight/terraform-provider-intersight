...
func (dst *EquipmentPsuControlRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "equipment.PsuControl" {
		...
		if err == nil {
			jsonEquipmentPsuControl, _ := json.Marshal(dst.EquipmentPsuControl)
			if string(jsonEquipmentPsuControl) == "{}" { // empty struct
	...
	// try to unmarshal data into EquipmentPsuControl
	err = json.Unmarshal(data, &dst.EquipmentPsuControl)
	if err == nil {
		jsonEquipmentPsuControl, _ := json.Marshal(dst.EquipmentPsuControl)
		if string(jsonEquipmentPsuControl) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(EquipmentPsuControlRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(EquipmentPsuControlRelationship)")
	}
}
