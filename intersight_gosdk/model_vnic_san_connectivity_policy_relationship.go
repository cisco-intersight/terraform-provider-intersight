...
func (dst *VnicSanConnectivityPolicyRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "vnic.SanConnectivityPolicy" {
		...
		if err == nil {
			jsonVnicSanConnectivityPolicy, _ := json.Marshal(dst.VnicSanConnectivityPolicy)
			if string(jsonVnicSanConnectivityPolicy) == "{}" { // empty struct
	...
	// try to unmarshal data into VnicSanConnectivityPolicy
	err = json.Unmarshal(data, &dst.VnicSanConnectivityPolicy)
	if err == nil {
		jsonVnicSanConnectivityPolicy, _ := json.Marshal(dst.VnicSanConnectivityPolicy)
		if string(jsonVnicSanConnectivityPolicy) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(VnicSanConnectivityPolicyRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(VnicSanConnectivityPolicyRelationship)")
	}
}
