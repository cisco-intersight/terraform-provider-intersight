...
func (dst *StorageFlexFlashControllerPropsResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "storage.FlexFlashControllerProps.List" {
		...
		if err == nil {
			jsonStorageFlexFlashControllerPropsList, _ := json.Marshal(dst.StorageFlexFlashControllerPropsList)
			if string(jsonStorageFlexFlashControllerPropsList) == "{}" { // empty struct
	...
	// try to unmarshal data into StorageFlexFlashControllerPropsList
	err = json.Unmarshal(data, &dst.StorageFlexFlashControllerPropsList)
	if err == nil {
		jsonStorageFlexFlashControllerPropsList, _ := json.Marshal(dst.StorageFlexFlashControllerPropsList)
		if string(jsonStorageFlexFlashControllerPropsList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(StorageFlexFlashControllerPropsResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(StorageFlexFlashControllerPropsResponse)")
	}
}
