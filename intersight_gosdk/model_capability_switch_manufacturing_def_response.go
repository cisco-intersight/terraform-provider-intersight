...
func (dst *CapabilitySwitchManufacturingDefResponse) UnmarshalJSON(data []byte) error {
	var err error
	match := 0
	// try to unmarshal data into CapabilitySwitchManufacturingDefList
	err = json.Unmarshal(data, &dst.CapabilitySwitchManufacturingDefList)
	if err == nil {
		jsonCapabilitySwitchManufacturingDefList, _ := json.Marshal(dst.CapabilitySwitchManufacturingDefList)
		if string(jsonCapabilitySwitchManufacturingDefList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(CapabilitySwitchManufacturingDefResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(CapabilitySwitchManufacturingDefResponse)")
	}
}
