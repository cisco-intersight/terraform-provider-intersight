...
func (dst *ComputeBladeRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "compute.Blade" {
		...
		if err == nil {
			jsonComputeBlade, _ := json.Marshal(dst.ComputeBlade)
			if string(jsonComputeBlade) == "{}" { // empty struct
	...
	// try to unmarshal data into ComputeBlade
	err = json.Unmarshal(data, &dst.ComputeBlade)
	if err == nil {
		jsonComputeBlade, _ := json.Marshal(dst.ComputeBlade)
		if string(jsonComputeBlade) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(ComputeBladeRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(ComputeBladeRelationship)")
	}
}
