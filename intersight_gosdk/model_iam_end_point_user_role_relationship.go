...
func (dst *IamEndPointUserRoleRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "iam.EndPointUserRole" {
		...
		if err == nil {
			jsonIamEndPointUserRole, _ := json.Marshal(dst.IamEndPointUserRole)
			if string(jsonIamEndPointUserRole) == "{}" { // empty struct
	...
	// try to unmarshal data into IamEndPointUserRole
	err = json.Unmarshal(data, &dst.IamEndPointUserRole)
	if err == nil {
		jsonIamEndPointUserRole, _ := json.Marshal(dst.IamEndPointUserRole)
		if string(jsonIamEndPointUserRole) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IamEndPointUserRoleRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IamEndPointUserRoleRelationship)")
	}
}
