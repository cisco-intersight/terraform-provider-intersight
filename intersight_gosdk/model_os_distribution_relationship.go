...
func (dst *OsDistributionRelationship) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into OsDistribution
	err = json.Unmarshal(data, &dst.OsDistribution)
	if err == nil {
		jsonOsDistribution, _ := json.Marshal(dst.OsDistribution)
		if string(jsonOsDistribution) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(OsDistributionRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(OsDistributionRelationship)")
	}
}
