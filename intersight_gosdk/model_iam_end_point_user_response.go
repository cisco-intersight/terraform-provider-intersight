...
func (dst *IamEndPointUserResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "iam.EndPointUser.List" {
		...
		if err == nil {
			jsonIamEndPointUserList, _ := json.Marshal(dst.IamEndPointUserList)
			if string(jsonIamEndPointUserList) == "{}" { // empty struct
	...
	// try to unmarshal data into IamEndPointUserList
	err = json.Unmarshal(data, &dst.IamEndPointUserList)
	if err == nil {
		jsonIamEndPointUserList, _ := json.Marshal(dst.IamEndPointUserList)
		if string(jsonIamEndPointUserList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IamEndPointUserResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IamEndPointUserResponse)")
	}
}
