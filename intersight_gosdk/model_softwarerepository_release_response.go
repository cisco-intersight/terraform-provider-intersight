...
func (dst *SoftwarerepositoryReleaseResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "softwarerepository.Release.List" {
		...
		if err == nil {
			jsonSoftwarerepositoryReleaseList, _ := json.Marshal(dst.SoftwarerepositoryReleaseList)
			if string(jsonSoftwarerepositoryReleaseList) == "{}" { // empty struct
	...
	// try to unmarshal data into SoftwarerepositoryReleaseList
	err = json.Unmarshal(data, &dst.SoftwarerepositoryReleaseList)
	if err == nil {
		jsonSoftwarerepositoryReleaseList, _ := json.Marshal(dst.SoftwarerepositoryReleaseList)
		if string(jsonSoftwarerepositoryReleaseList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(SoftwarerepositoryReleaseResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(SoftwarerepositoryReleaseResponse)")
	}
}
