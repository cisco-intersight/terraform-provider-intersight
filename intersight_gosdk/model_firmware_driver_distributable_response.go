...
func (dst *FirmwareDriverDistributableResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "firmware.DriverDistributable.List" {
		...
		if err == nil {
			jsonFirmwareDriverDistributableList, _ := json.Marshal(dst.FirmwareDriverDistributableList)
			if string(jsonFirmwareDriverDistributableList) == "{}" { // empty struct
	...
	// try to unmarshal data into FirmwareDriverDistributableList
	err = json.Unmarshal(data, &dst.FirmwareDriverDistributableList)
	if err == nil {
		jsonFirmwareDriverDistributableList, _ := json.Marshal(dst.FirmwareDriverDistributableList)
		if string(jsonFirmwareDriverDistributableList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(FirmwareDriverDistributableResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(FirmwareDriverDistributableResponse)")
	}
}
