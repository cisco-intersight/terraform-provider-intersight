...
func (dst *ServerConfigResultEntryResponse) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into ServerConfigResultEntryList
	err = json.Unmarshal(data, &dst.ServerConfigResultEntryList)
	if err == nil {
		jsonServerConfigResultEntryList, _ := json.Marshal(dst.ServerConfigResultEntryList)
		if string(jsonServerConfigResultEntryList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(ServerConfigResultEntryResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(ServerConfigResultEntryResponse)")
	}
}
