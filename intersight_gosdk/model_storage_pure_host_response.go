...
func (dst *StoragePureHostResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "storage.PureHost.List" {
		...
		if err == nil {
			jsonStoragePureHostList, _ := json.Marshal(dst.StoragePureHostList)
			if string(jsonStoragePureHostList) == "{}" { // empty struct
	...
	// try to unmarshal data into StoragePureHostList
	err = json.Unmarshal(data, &dst.StoragePureHostList)
	if err == nil {
		jsonStoragePureHostList, _ := json.Marshal(dst.StoragePureHostList)
		if string(jsonStoragePureHostList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(StoragePureHostResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(StoragePureHostResponse)")
	}
}
