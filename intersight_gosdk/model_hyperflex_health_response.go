...
func (dst *HyperflexHealthResponse) UnmarshalJSON(data []byte) error {
	var err error
	match := 0
	// try to unmarshal data into HyperflexHealthList
	err = json.Unmarshal(data, &dst.HyperflexHealthList)
	if err == nil {
		jsonHyperflexHealthList, _ := json.Marshal(dst.HyperflexHealthList)
		if string(jsonHyperflexHealthList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(HyperflexHealthResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(HyperflexHealthResponse)")
	}
}
