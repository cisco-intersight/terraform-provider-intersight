...
func (dst *StorageEnclosureDiskResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "storage.EnclosureDisk.List" {
		...
		if err == nil {
			jsonStorageEnclosureDiskList, _ := json.Marshal(dst.StorageEnclosureDiskList)
			if string(jsonStorageEnclosureDiskList) == "{}" { // empty struct
	...
	// try to unmarshal data into StorageEnclosureDiskList
	err = json.Unmarshal(data, &dst.StorageEnclosureDiskList)
	if err == nil {
		jsonStorageEnclosureDiskList, _ := json.Marshal(dst.StorageEnclosureDiskList)
		if string(jsonStorageEnclosureDiskList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(StorageEnclosureDiskResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(StorageEnclosureDiskResponse)")
	}
}
