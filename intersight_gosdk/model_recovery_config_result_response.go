...
func (dst *RecoveryConfigResultResponse) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into RecoveryConfigResultList
	err = json.Unmarshal(data, &dst.RecoveryConfigResultList)
	if err == nil {
		jsonRecoveryConfigResultList, _ := json.Marshal(dst.RecoveryConfigResultList)
		if string(jsonRecoveryConfigResultList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(RecoveryConfigResultResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(RecoveryConfigResultResponse)")
	}
}
