...
func (dst *StoragePureArrayRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "storage.PureArray" {
		...
		if err == nil {
			jsonStoragePureArray, _ := json.Marshal(dst.StoragePureArray)
			if string(jsonStoragePureArray) == "{}" { // empty struct
	...
	// try to unmarshal data into StoragePureArray
	err = json.Unmarshal(data, &dst.StoragePureArray)
	if err == nil {
		jsonStoragePureArray, _ := json.Marshal(dst.StoragePureArray)
		if string(jsonStoragePureArray) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(StoragePureArrayRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(StoragePureArrayRelationship)")
	}
}
