...
func (dst *VirtualizationVmwareHostRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "virtualization.VmwareHost" {
		...
		if err == nil {
			jsonVirtualizationVmwareHost, _ := json.Marshal(dst.VirtualizationVmwareHost)
			if string(jsonVirtualizationVmwareHost) == "{}" { // empty struct
	...
	// try to unmarshal data into VirtualizationVmwareHost
	err = json.Unmarshal(data, &dst.VirtualizationVmwareHost)
	if err == nil {
		jsonVirtualizationVmwareHost, _ := json.Marshal(dst.VirtualizationVmwareHost)
		if string(jsonVirtualizationVmwareHost) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(VirtualizationVmwareHostRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(VirtualizationVmwareHostRelationship)")
	}
}
