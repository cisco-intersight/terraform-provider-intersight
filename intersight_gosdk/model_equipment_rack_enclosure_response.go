...
func (dst *EquipmentRackEnclosureResponse) UnmarshalJSON(data []byte) error {
	var err error
	match := 0
	// try to unmarshal data into EquipmentRackEnclosureList
	err = json.Unmarshal(data, &dst.EquipmentRackEnclosureList)
	if err == nil {
		jsonEquipmentRackEnclosureList, _ := json.Marshal(dst.EquipmentRackEnclosureList)
		if string(jsonEquipmentRackEnclosureList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(EquipmentRackEnclosureResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(EquipmentRackEnclosureResponse)")
	}
}
