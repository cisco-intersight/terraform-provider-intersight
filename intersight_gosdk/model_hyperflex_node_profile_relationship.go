...
func (dst *HyperflexNodeProfileRelationship) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into HyperflexNodeProfile
	err = json.Unmarshal(data, &dst.HyperflexNodeProfile)
	if err == nil {
		jsonHyperflexNodeProfile, _ := json.Marshal(dst.HyperflexNodeProfile)
		if string(jsonHyperflexNodeProfile) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(HyperflexNodeProfileRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(HyperflexNodeProfileRelationship)")
	}
}
