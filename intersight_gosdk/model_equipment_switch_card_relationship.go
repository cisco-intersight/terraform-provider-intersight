...
func (dst *EquipmentSwitchCardRelationship) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into EquipmentSwitchCard
	err = json.Unmarshal(data, &dst.EquipmentSwitchCard)
	if err == nil {
		jsonEquipmentSwitchCard, _ := json.Marshal(dst.EquipmentSwitchCard)
		if string(jsonEquipmentSwitchCard) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(EquipmentSwitchCardRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(EquipmentSwitchCardRelationship)")
	}
}
