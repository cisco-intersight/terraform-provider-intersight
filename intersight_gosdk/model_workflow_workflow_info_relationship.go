...
func (dst *WorkflowWorkflowInfoRelationship) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into WorkflowWorkflowInfo
	err = json.Unmarshal(data, &dst.WorkflowWorkflowInfo)
	if err == nil {
		jsonWorkflowWorkflowInfo, _ := json.Marshal(dst.WorkflowWorkflowInfo)
		if string(jsonWorkflowWorkflowInfo) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(WorkflowWorkflowInfoRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(WorkflowWorkflowInfoRelationship)")
	}
}
