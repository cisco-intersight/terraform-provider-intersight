...
func (dst *FirmwareDimmDescriptorResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "firmware.DimmDescriptor.List" {
		...
		if err == nil {
			jsonFirmwareDimmDescriptorList, _ := json.Marshal(dst.FirmwareDimmDescriptorList)
			if string(jsonFirmwareDimmDescriptorList) == "{}" { // empty struct
	...
	// try to unmarshal data into FirmwareDimmDescriptorList
	err = json.Unmarshal(data, &dst.FirmwareDimmDescriptorList)
	if err == nil {
		jsonFirmwareDimmDescriptorList, _ := json.Marshal(dst.FirmwareDimmDescriptorList)
		if string(jsonFirmwareDimmDescriptorList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(FirmwareDimmDescriptorResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(FirmwareDimmDescriptorResponse)")
	}
}
