...
func (dst *UuidpoolUniverseResponse) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into UuidpoolUniverseList
	err = json.Unmarshal(data, &dst.UuidpoolUniverseList)
	if err == nil {
		jsonUuidpoolUniverseList, _ := json.Marshal(dst.UuidpoolUniverseList)
		if string(jsonUuidpoolUniverseList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(UuidpoolUniverseResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(UuidpoolUniverseResponse)")
	}
}
