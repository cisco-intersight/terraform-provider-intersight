...
func (dst *HyperflexConfigResultEntryRelationship) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into HyperflexConfigResultEntry
	err = json.Unmarshal(data, &dst.HyperflexConfigResultEntry)
	if err == nil {
		jsonHyperflexConfigResultEntry, _ := json.Marshal(dst.HyperflexConfigResultEntry)
		if string(jsonHyperflexConfigResultEntry) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(HyperflexConfigResultEntryRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(HyperflexConfigResultEntryRelationship)")
	}
}
