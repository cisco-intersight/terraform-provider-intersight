...
func (dst *IamCertificateRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "iam.Certificate" {
		...
		if err == nil {
			jsonIamCertificate, _ := json.Marshal(dst.IamCertificate)
			if string(jsonIamCertificate) == "{}" { // empty struct
	...
	// try to unmarshal data into IamCertificate
	err = json.Unmarshal(data, &dst.IamCertificate)
	if err == nil {
		jsonIamCertificate, _ := json.Marshal(dst.IamCertificate)
		if string(jsonIamCertificate) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IamCertificateRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IamCertificateRelationship)")
	}
}
