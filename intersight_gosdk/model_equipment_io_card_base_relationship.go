...
func (dst *EquipmentIoCardBaseRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "equipment.IoCardBase" {
		...
		if err == nil {
			jsonEquipmentIoCardBase, _ := json.Marshal(dst.EquipmentIoCardBase)
			if string(jsonEquipmentIoCardBase) == "{}" { // empty struct
	...
	// try to unmarshal data into EquipmentIoCardBase
	err = json.Unmarshal(data, &dst.EquipmentIoCardBase)
	if err == nil {
		jsonEquipmentIoCardBase, _ := json.Marshal(dst.EquipmentIoCardBase)
		if string(jsonEquipmentIoCardBase) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(EquipmentIoCardBaseRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(EquipmentIoCardBaseRelationship)")
	}
}
