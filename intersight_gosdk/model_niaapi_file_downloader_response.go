...
func (dst *NiaapiFileDownloaderResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "niaapi.FileDownloader.List" {
		...
		if err == nil {
			jsonNiaapiFileDownloaderList, _ := json.Marshal(dst.NiaapiFileDownloaderList)
			if string(jsonNiaapiFileDownloaderList) == "{}" { // empty struct
	...
	// try to unmarshal data into NiaapiFileDownloaderList
	err = json.Unmarshal(data, &dst.NiaapiFileDownloaderList)
	if err == nil {
		jsonNiaapiFileDownloaderList, _ := json.Marshal(dst.NiaapiFileDownloaderList)
		if string(jsonNiaapiFileDownloaderList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(NiaapiFileDownloaderResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(NiaapiFileDownloaderResponse)")
	}
}
