...
func (dst *EquipmentPsuResponse) UnmarshalJSON(data []byte) error {
	var err error
	match := 0
	// try to unmarshal data into EquipmentPsuList
	err = json.Unmarshal(data, &dst.EquipmentPsuList)
	if err == nil {
		jsonEquipmentPsuList, _ := json.Marshal(dst.EquipmentPsuList)
		if string(jsonEquipmentPsuList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(EquipmentPsuResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(EquipmentPsuResponse)")
	}
}
