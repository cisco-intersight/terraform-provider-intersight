...
func (dst *StoragePureVolumeRelationship) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into StoragePureVolume
	err = json.Unmarshal(data, &dst.StoragePureVolume)
	if err == nil {
		jsonStoragePureVolume, _ := json.Marshal(dst.StoragePureVolume)
		if string(jsonStoragePureVolume) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(StoragePureVolumeRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(StoragePureVolumeRelationship)")
	}
}
