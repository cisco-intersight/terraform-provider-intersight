...
func (dst *FcpoolPoolMemberResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "fcpool.PoolMember.List" {
		...
		if err == nil {
			jsonFcpoolPoolMemberList, _ := json.Marshal(dst.FcpoolPoolMemberList)
			if string(jsonFcpoolPoolMemberList) == "{}" { // empty struct
	...
	// try to unmarshal data into FcpoolPoolMemberList
	err = json.Unmarshal(data, &dst.FcpoolPoolMemberList)
	if err == nil {
		jsonFcpoolPoolMemberList, _ := json.Marshal(dst.FcpoolPoolMemberList)
		if string(jsonFcpoolPoolMemberList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(FcpoolPoolMemberResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(FcpoolPoolMemberResponse)")
	}
}
