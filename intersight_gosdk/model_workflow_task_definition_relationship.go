...
func (dst *WorkflowTaskDefinitionRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "workflow.TaskDefinition" {
		...
		if err == nil {
			jsonWorkflowTaskDefinition, _ := json.Marshal(dst.WorkflowTaskDefinition)
			if string(jsonWorkflowTaskDefinition) == "{}" { // empty struct
	...
	// try to unmarshal data into WorkflowTaskDefinition
	err = json.Unmarshal(data, &dst.WorkflowTaskDefinition)
	if err == nil {
		jsonWorkflowTaskDefinition, _ := json.Marshal(dst.WorkflowTaskDefinition)
		if string(jsonWorkflowTaskDefinition) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(WorkflowTaskDefinitionRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(WorkflowTaskDefinitionRelationship)")
	}
}
