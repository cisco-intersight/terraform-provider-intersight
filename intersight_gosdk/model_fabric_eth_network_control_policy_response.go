...
func (dst *FabricEthNetworkControlPolicyResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "fabric.EthNetworkControlPolicy.List" {
		...
		if err == nil {
			jsonFabricEthNetworkControlPolicyList, _ := json.Marshal(dst.FabricEthNetworkControlPolicyList)
			if string(jsonFabricEthNetworkControlPolicyList) == "{}" { // empty struct
	...
	// try to unmarshal data into FabricEthNetworkControlPolicyList
	err = json.Unmarshal(data, &dst.FabricEthNetworkControlPolicyList)
	if err == nil {
		jsonFabricEthNetworkControlPolicyList, _ := json.Marshal(dst.FabricEthNetworkControlPolicyList)
		if string(jsonFabricEthNetworkControlPolicyList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(FabricEthNetworkControlPolicyResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(FabricEthNetworkControlPolicyResponse)")
	}
}
