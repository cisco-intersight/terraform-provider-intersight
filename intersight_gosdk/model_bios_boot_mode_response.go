...
func (dst *BiosBootModeResponse) UnmarshalJSON(data []byte) error {
	var err error
	match := 0
	// try to unmarshal data into BiosBootModeList
	err = json.Unmarshal(data, &dst.BiosBootModeList)
	if err == nil {
		jsonBiosBootModeList, _ := json.Marshal(dst.BiosBootModeList)
		if string(jsonBiosBootModeList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(BiosBootModeResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(BiosBootModeResponse)")
	}
}
