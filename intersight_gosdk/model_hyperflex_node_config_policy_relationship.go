...
func (dst *HyperflexNodeConfigPolicyRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "hyperflex.NodeConfigPolicy" {
		...
		if err == nil {
			jsonHyperflexNodeConfigPolicy, _ := json.Marshal(dst.HyperflexNodeConfigPolicy)
			if string(jsonHyperflexNodeConfigPolicy) == "{}" { // empty struct
	...
	// try to unmarshal data into HyperflexNodeConfigPolicy
	err = json.Unmarshal(data, &dst.HyperflexNodeConfigPolicy)
	if err == nil {
		jsonHyperflexNodeConfigPolicy, _ := json.Marshal(dst.HyperflexNodeConfigPolicy)
		if string(jsonHyperflexNodeConfigPolicy) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(HyperflexNodeConfigPolicyRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(HyperflexNodeConfigPolicyRelationship)")
	}
}
