...
func (dst *StorageFlexUtilPhysicalDriveResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "storage.FlexUtilPhysicalDrive.List" {
		...
		if err == nil {
			jsonStorageFlexUtilPhysicalDriveList, _ := json.Marshal(dst.StorageFlexUtilPhysicalDriveList)
			if string(jsonStorageFlexUtilPhysicalDriveList) == "{}" { // empty struct
	...
	// try to unmarshal data into StorageFlexUtilPhysicalDriveList
	err = json.Unmarshal(data, &dst.StorageFlexUtilPhysicalDriveList)
	if err == nil {
		jsonStorageFlexUtilPhysicalDriveList, _ := json.Marshal(dst.StorageFlexUtilPhysicalDriveList)
		if string(jsonStorageFlexUtilPhysicalDriveList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(StorageFlexUtilPhysicalDriveResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(StorageFlexUtilPhysicalDriveResponse)")
	}
}
