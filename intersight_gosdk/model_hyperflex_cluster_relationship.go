...
func (dst *HyperflexClusterRelationship) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into HyperflexCluster
	err = json.Unmarshal(data, &dst.HyperflexCluster)
	if err == nil {
		jsonHyperflexCluster, _ := json.Marshal(dst.HyperflexCluster)
		if string(jsonHyperflexCluster) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(HyperflexClusterRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(HyperflexClusterRelationship)")
	}
}
