...
func (dst *IamSystemResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "iam.System.List" {
		...
		if err == nil {
			jsonIamSystemList, _ := json.Marshal(dst.IamSystemList)
			if string(jsonIamSystemList) == "{}" { // empty struct
	...
	// try to unmarshal data into IamSystemList
	err = json.Unmarshal(data, &dst.IamSystemList)
	if err == nil {
		jsonIamSystemList, _ := json.Marshal(dst.IamSystemList)
		if string(jsonIamSystemList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IamSystemResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IamSystemResponse)")
	}
}
