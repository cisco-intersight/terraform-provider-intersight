...
func (dst *FirmwareUpgradeImpactStatusRelationship) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into FirmwareUpgradeImpactStatus
	err = json.Unmarshal(data, &dst.FirmwareUpgradeImpactStatus)
	if err == nil {
		jsonFirmwareUpgradeImpactStatus, _ := json.Marshal(dst.FirmwareUpgradeImpactStatus)
		if string(jsonFirmwareUpgradeImpactStatus) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(FirmwareUpgradeImpactStatusRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(FirmwareUpgradeImpactStatusRelationship)")
	}
}
