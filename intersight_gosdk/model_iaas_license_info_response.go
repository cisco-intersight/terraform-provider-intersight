...
func (dst *IaasLicenseInfoResponse) UnmarshalJSON(data []byte) error {
	var err error
	match := 0
	// try to unmarshal data into IaasLicenseInfoList
	err = json.Unmarshal(data, &dst.IaasLicenseInfoList)
	if err == nil {
		jsonIaasLicenseInfoList, _ := json.Marshal(dst.IaasLicenseInfoList)
		if string(jsonIaasLicenseInfoList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IaasLicenseInfoResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IaasLicenseInfoResponse)")
	}
}
