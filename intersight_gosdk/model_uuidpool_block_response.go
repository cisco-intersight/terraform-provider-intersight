...
func (dst *UuidpoolBlockResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "uuidpool.Block.List" {
		...
		if err == nil {
			jsonUuidpoolBlockList, _ := json.Marshal(dst.UuidpoolBlockList)
			if string(jsonUuidpoolBlockList) == "{}" { // empty struct
	...
	// try to unmarshal data into UuidpoolBlockList
	err = json.Unmarshal(data, &dst.UuidpoolBlockList)
	if err == nil {
		jsonUuidpoolBlockList, _ := json.Marshal(dst.UuidpoolBlockList)
		if string(jsonUuidpoolBlockList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(UuidpoolBlockResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(UuidpoolBlockResponse)")
	}
}
