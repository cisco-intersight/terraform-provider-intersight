...
func (dst *FcPhysicalPortRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "fc.PhysicalPort" {
		...
		if err == nil {
			jsonFcPhysicalPort, _ := json.Marshal(dst.FcPhysicalPort)
			if string(jsonFcPhysicalPort) == "{}" { // empty struct
	...
	// try to unmarshal data into FcPhysicalPort
	err = json.Unmarshal(data, &dst.FcPhysicalPort)
	if err == nil {
		jsonFcPhysicalPort, _ := json.Marshal(dst.FcPhysicalPort)
		if string(jsonFcPhysicalPort) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(FcPhysicalPortRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(FcPhysicalPortRelationship)")
	}
}
