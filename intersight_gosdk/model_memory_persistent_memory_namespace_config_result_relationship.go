...
func (dst *MemoryPersistentMemoryNamespaceConfigResultRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "memory.PersistentMemoryNamespaceConfigResult" {
		...
		if err == nil {
			jsonMemoryPersistentMemoryNamespaceConfigResult, _ := json.Marshal(dst.MemoryPersistentMemoryNamespaceConfigResult)
			if string(jsonMemoryPersistentMemoryNamespaceConfigResult) == "{}" { // empty struct
	...
	// try to unmarshal data into MemoryPersistentMemoryNamespaceConfigResult
	err = json.Unmarshal(data, &dst.MemoryPersistentMemoryNamespaceConfigResult)
	if err == nil {
		jsonMemoryPersistentMemoryNamespaceConfigResult, _ := json.Marshal(dst.MemoryPersistentMemoryNamespaceConfigResult)
		if string(jsonMemoryPersistentMemoryNamespaceConfigResult) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(MemoryPersistentMemoryNamespaceConfigResultRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(MemoryPersistentMemoryNamespaceConfigResultRelationship)")
	}
}
