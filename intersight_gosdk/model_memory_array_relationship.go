...
func (dst *MemoryArrayRelationship) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into MemoryArray
	err = json.Unmarshal(data, &dst.MemoryArray)
	if err == nil {
		jsonMemoryArray, _ := json.Marshal(dst.MemoryArray)
		if string(jsonMemoryArray) == "{}" { // empty struct
			dst.MemoryArray = nil
		} else {
			match++
		}
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(MemoryArrayRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(MemoryArrayRelationship)")
	}
}
