...
func (dst *StoragePureHostGroupResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "storage.PureHostGroup.List" {
		...
		if err == nil {
			jsonStoragePureHostGroupList, _ := json.Marshal(dst.StoragePureHostGroupList)
			if string(jsonStoragePureHostGroupList) == "{}" { // empty struct
	...
	// try to unmarshal data into StoragePureHostGroupList
	err = json.Unmarshal(data, &dst.StoragePureHostGroupList)
	if err == nil {
		jsonStoragePureHostGroupList, _ := json.Marshal(dst.StoragePureHostGroupList)
		if string(jsonStoragePureHostGroupList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(StoragePureHostGroupResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(StoragePureHostGroupResponse)")
	}
}
