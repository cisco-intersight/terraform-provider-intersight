...
func (dst *HyperflexSoftwareVersionPolicyResponse) UnmarshalJSON(data []byte) error {
	var err error
	match := 0
	// try to unmarshal data into HyperflexSoftwareVersionPolicyList
	err = json.Unmarshal(data, &dst.HyperflexSoftwareVersionPolicyList)
	if err == nil {
		jsonHyperflexSoftwareVersionPolicyList, _ := json.Marshal(dst.HyperflexSoftwareVersionPolicyList)
		if string(jsonHyperflexSoftwareVersionPolicyList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(HyperflexSoftwareVersionPolicyResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(HyperflexSoftwareVersionPolicyResponse)")
	}
}
