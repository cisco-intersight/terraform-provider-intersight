...
func (dst *TelemetryDruidDimensionSpec) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["type"] == "default" {
		...
		if err == nil {
			jsonTelemetryDruidDefaultDimensionSpec, _ := json.Marshal(dst.TelemetryDruidDefaultDimensionSpec)
			if string(jsonTelemetryDruidDefaultDimensionSpec) == "{}" { // empty struct
	...
	if jsonDict["type"] == "extraction" {
		...
		if err == nil {
			jsonTelemetryDruidExtractionDimensionSpec, _ := json.Marshal(dst.TelemetryDruidExtractionDimensionSpec)
			if string(jsonTelemetryDruidExtractionDimensionSpec) == "{}" { // empty struct
	...
	// try to unmarshal data into TelemetryDruidDefaultDimensionSpec
	err = json.Unmarshal(data, &dst.TelemetryDruidDefaultDimensionSpec)
	if err == nil {
		jsonTelemetryDruidDefaultDimensionSpec, _ := json.Marshal(dst.TelemetryDruidDefaultDimensionSpec)
		if string(jsonTelemetryDruidDefaultDimensionSpec) == "{}" { // empty struct
	...
	// try to unmarshal data into TelemetryDruidExtractionDimensionSpec
	err = json.Unmarshal(data, &dst.TelemetryDruidExtractionDimensionSpec)
	if err == nil {
		jsonTelemetryDruidExtractionDimensionSpec, _ := json.Marshal(dst.TelemetryDruidExtractionDimensionSpec)
		if string(jsonTelemetryDruidExtractionDimensionSpec) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(TelemetryDruidDimensionSpec)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(TelemetryDruidDimensionSpec)")
	}
}
