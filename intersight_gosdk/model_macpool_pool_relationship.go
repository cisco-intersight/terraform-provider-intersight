...
func (dst *MacpoolPoolRelationship) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into MacpoolPool
	err = json.Unmarshal(data, &dst.MacpoolPool)
	if err == nil {
		jsonMacpoolPool, _ := json.Marshal(dst.MacpoolPool)
		if string(jsonMacpoolPool) == "{}" { // empty struct
			dst.MacpoolPool = nil
		} else {
			match++
		}
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(MacpoolPoolRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(MacpoolPoolRelationship)")
	}
}
