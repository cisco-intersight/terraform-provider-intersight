...
func (dst *EquipmentFanResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "equipment.Fan.List" {
		...
		if err == nil {
			jsonEquipmentFanList, _ := json.Marshal(dst.EquipmentFanList)
			if string(jsonEquipmentFanList) == "{}" { // empty struct
	...
	// try to unmarshal data into EquipmentFanList
	err = json.Unmarshal(data, &dst.EquipmentFanList)
	if err == nil {
		jsonEquipmentFanList, _ := json.Marshal(dst.EquipmentFanList)
		if string(jsonEquipmentFanList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(EquipmentFanResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(EquipmentFanResponse)")
	}
}
