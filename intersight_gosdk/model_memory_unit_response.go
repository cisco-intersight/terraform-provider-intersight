...
func (dst *MemoryUnitResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "memory.Unit.List" {
		...
		if err == nil {
			jsonMemoryUnitList, _ := json.Marshal(dst.MemoryUnitList)
			if string(jsonMemoryUnitList) == "{}" { // empty struct
	...
	// try to unmarshal data into MemoryUnitList
	err = json.Unmarshal(data, &dst.MemoryUnitList)
	if err == nil {
		jsonMemoryUnitList, _ := json.Marshal(dst.MemoryUnitList)
		if string(jsonMemoryUnitList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(MemoryUnitResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(MemoryUnitResponse)")
	}
}
