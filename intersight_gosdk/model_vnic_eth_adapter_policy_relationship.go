...
func (dst *VnicEthAdapterPolicyRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "vnic.EthAdapterPolicy" {
		...
		if err == nil {
			jsonVnicEthAdapterPolicy, _ := json.Marshal(dst.VnicEthAdapterPolicy)
			if string(jsonVnicEthAdapterPolicy) == "{}" { // empty struct
	...
	// try to unmarshal data into VnicEthAdapterPolicy
	err = json.Unmarshal(data, &dst.VnicEthAdapterPolicy)
	if err == nil {
		jsonVnicEthAdapterPolicy, _ := json.Marshal(dst.VnicEthAdapterPolicy)
		if string(jsonVnicEthAdapterPolicy) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(VnicEthAdapterPolicyRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(VnicEthAdapterPolicyRelationship)")
	}
}
