...
func (dst *VnicEthIfRelationship) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into VnicEthIf
	err = json.Unmarshal(data, &dst.VnicEthIf)
	if err == nil {
		jsonVnicEthIf, _ := json.Marshal(dst.VnicEthIf)
		if string(jsonVnicEthIf) == "{}" { // empty struct
			dst.VnicEthIf = nil
		} else {
			match++
		}
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(VnicEthIfRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(VnicEthIfRelationship)")
	}
}
