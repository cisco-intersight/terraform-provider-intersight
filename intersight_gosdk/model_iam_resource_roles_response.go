...
func (dst *IamResourceRolesResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "iam.ResourceRoles.List" {
		...
		if err == nil {
			jsonIamResourceRolesList, _ := json.Marshal(dst.IamResourceRolesList)
			if string(jsonIamResourceRolesList) == "{}" { // empty struct
	...
	// try to unmarshal data into IamResourceRolesList
	err = json.Unmarshal(data, &dst.IamResourceRolesList)
	if err == nil {
		jsonIamResourceRolesList, _ := json.Marshal(dst.IamResourceRolesList)
		if string(jsonIamResourceRolesList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IamResourceRolesResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IamResourceRolesResponse)")
	}
}
