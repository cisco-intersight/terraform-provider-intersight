...
func (dst *OsConfigurationFileResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "os.ConfigurationFile.List" {
		...
		if err == nil {
			jsonOsConfigurationFileList, _ := json.Marshal(dst.OsConfigurationFileList)
			if string(jsonOsConfigurationFileList) == "{}" { // empty struct
	...
	// try to unmarshal data into OsConfigurationFileList
	err = json.Unmarshal(data, &dst.OsConfigurationFileList)
	if err == nil {
		jsonOsConfigurationFileList, _ := json.Marshal(dst.OsConfigurationFileList)
		if string(jsonOsConfigurationFileList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(OsConfigurationFileResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(OsConfigurationFileResponse)")
	}
}
