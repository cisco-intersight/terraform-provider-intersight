...
func (dst *CapabilityCapabilityRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "capability.Capability" {
		...
		if err == nil {
			jsonCapabilityCapability, _ := json.Marshal(dst.CapabilityCapability)
			if string(jsonCapabilityCapability) == "{}" { // empty struct
	...
	// try to unmarshal data into CapabilityCapability
	err = json.Unmarshal(data, &dst.CapabilityCapability)
	if err == nil {
		jsonCapabilityCapability, _ := json.Marshal(dst.CapabilityCapability)
		if string(jsonCapabilityCapability) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(CapabilityCapabilityRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(CapabilityCapabilityRelationship)")
	}
}
