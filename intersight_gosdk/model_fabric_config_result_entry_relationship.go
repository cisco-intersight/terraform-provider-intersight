...
func (dst *FabricConfigResultEntryRelationship) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into FabricConfigResultEntry
	err = json.Unmarshal(data, &dst.FabricConfigResultEntry)
	if err == nil {
		jsonFabricConfigResultEntry, _ := json.Marshal(dst.FabricConfigResultEntry)
		if string(jsonFabricConfigResultEntry) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(FabricConfigResultEntryRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(FabricConfigResultEntryRelationship)")
	}
}
