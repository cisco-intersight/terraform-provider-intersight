...
func (dst *PortSubGroupResponse) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into PortSubGroupList
	err = json.Unmarshal(data, &dst.PortSubGroupList)
	if err == nil {
		jsonPortSubGroupList, _ := json.Marshal(dst.PortSubGroupList)
		if string(jsonPortSubGroupList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(PortSubGroupResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(PortSubGroupResponse)")
	}
}
