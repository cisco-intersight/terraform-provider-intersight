...
func (dst *AssetClusterMemberRelationship) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into AssetClusterMember
	err = json.Unmarshal(data, &dst.AssetClusterMember)
	if err == nil {
		jsonAssetClusterMember, _ := json.Marshal(dst.AssetClusterMember)
		if string(jsonAssetClusterMember) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(AssetClusterMemberRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(AssetClusterMemberRelationship)")
	}
}
