...
func (dst *NetworkVlanPortInfoRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "network.VlanPortInfo" {
		...
		if err == nil {
			jsonNetworkVlanPortInfo, _ := json.Marshal(dst.NetworkVlanPortInfo)
			if string(jsonNetworkVlanPortInfo) == "{}" { // empty struct
	...
	// try to unmarshal data into NetworkVlanPortInfo
	err = json.Unmarshal(data, &dst.NetworkVlanPortInfo)
	if err == nil {
		jsonNetworkVlanPortInfo, _ := json.Marshal(dst.NetworkVlanPortInfo)
		if string(jsonNetworkVlanPortInfo) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(NetworkVlanPortInfoRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(NetworkVlanPortInfoRelationship)")
	}
}
