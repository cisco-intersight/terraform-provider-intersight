...
func (dst *WorkflowErrorResponseHandlerResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "workflow.ErrorResponseHandler.List" {
		...
		if err == nil {
			jsonWorkflowErrorResponseHandlerList, _ := json.Marshal(dst.WorkflowErrorResponseHandlerList)
			if string(jsonWorkflowErrorResponseHandlerList) == "{}" { // empty struct
	...
	// try to unmarshal data into WorkflowErrorResponseHandlerList
	err = json.Unmarshal(data, &dst.WorkflowErrorResponseHandlerList)
	if err == nil {
		jsonWorkflowErrorResponseHandlerList, _ := json.Marshal(dst.WorkflowErrorResponseHandlerList)
		if string(jsonWorkflowErrorResponseHandlerList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(WorkflowErrorResponseHandlerResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(WorkflowErrorResponseHandlerResponse)")
	}
}
