...
func (dst *MemoryPersistentMemoryUnitRelationship) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into MemoryPersistentMemoryUnit
	err = json.Unmarshal(data, &dst.MemoryPersistentMemoryUnit)
	if err == nil {
		jsonMemoryPersistentMemoryUnit, _ := json.Marshal(dst.MemoryPersistentMemoryUnit)
		if string(jsonMemoryPersistentMemoryUnit) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(MemoryPersistentMemoryUnitRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(MemoryPersistentMemoryUnitRelationship)")
	}
}
