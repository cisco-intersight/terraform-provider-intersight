...
func (dst *IamLocalUserPasswordRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "iam.LocalUserPassword" {
		...
		if err == nil {
			jsonIamLocalUserPassword, _ := json.Marshal(dst.IamLocalUserPassword)
			if string(jsonIamLocalUserPassword) == "{}" { // empty struct
	...
	// try to unmarshal data into IamLocalUserPassword
	err = json.Unmarshal(data, &dst.IamLocalUserPassword)
	if err == nil {
		jsonIamLocalUserPassword, _ := json.Marshal(dst.IamLocalUserPassword)
		if string(jsonIamLocalUserPassword) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IamLocalUserPasswordRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IamLocalUserPasswordRelationship)")
	}
}
