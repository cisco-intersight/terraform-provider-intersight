...
func (dst *StorageFlexFlashPhysicalDriveRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "storage.FlexFlashPhysicalDrive" {
		...
		if err == nil {
			jsonStorageFlexFlashPhysicalDrive, _ := json.Marshal(dst.StorageFlexFlashPhysicalDrive)
			if string(jsonStorageFlexFlashPhysicalDrive) == "{}" { // empty struct
	...
	// try to unmarshal data into StorageFlexFlashPhysicalDrive
	err = json.Unmarshal(data, &dst.StorageFlexFlashPhysicalDrive)
	if err == nil {
		jsonStorageFlexFlashPhysicalDrive, _ := json.Marshal(dst.StorageFlexFlashPhysicalDrive)
		if string(jsonStorageFlexFlashPhysicalDrive) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(StorageFlexFlashPhysicalDriveRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(StorageFlexFlashPhysicalDriveRelationship)")
	}
}
