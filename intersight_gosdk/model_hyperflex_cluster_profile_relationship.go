...
func (dst *HyperflexClusterProfileRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "hyperflex.ClusterProfile" {
		...
		if err == nil {
			jsonHyperflexClusterProfile, _ := json.Marshal(dst.HyperflexClusterProfile)
			if string(jsonHyperflexClusterProfile) == "{}" { // empty struct
	...
	// try to unmarshal data into HyperflexClusterProfile
	err = json.Unmarshal(data, &dst.HyperflexClusterProfile)
	if err == nil {
		jsonHyperflexClusterProfile, _ := json.Marshal(dst.HyperflexClusterProfile)
		if string(jsonHyperflexClusterProfile) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(HyperflexClusterProfileRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(HyperflexClusterProfileRelationship)")
	}
}
