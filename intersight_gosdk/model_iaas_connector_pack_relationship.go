...
func (dst *IaasConnectorPackRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "iaas.ConnectorPack" {
		...
		if err == nil {
			jsonIaasConnectorPack, _ := json.Marshal(dst.IaasConnectorPack)
			if string(jsonIaasConnectorPack) == "{}" { // empty struct
	...
	// try to unmarshal data into IaasConnectorPack
	err = json.Unmarshal(data, &dst.IaasConnectorPack)
	if err == nil {
		jsonIaasConnectorPack, _ := json.Marshal(dst.IaasConnectorPack)
		if string(jsonIaasConnectorPack) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IaasConnectorPackRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IaasConnectorPackRelationship)")
	}
}
