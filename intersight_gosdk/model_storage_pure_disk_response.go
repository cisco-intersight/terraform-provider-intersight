...
func (dst *StoragePureDiskResponse) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into StoragePureDiskList
	err = json.Unmarshal(data, &dst.StoragePureDiskList)
	if err == nil {
		jsonStoragePureDiskList, _ := json.Marshal(dst.StoragePureDiskList)
		if string(jsonStoragePureDiskList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(StoragePureDiskResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(StoragePureDiskResponse)")
	}
}
