...
func (dst *BiosUnitResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "bios.Unit.List" {
		...
		if err == nil {
			jsonBiosUnitList, _ := json.Marshal(dst.BiosUnitList)
			if string(jsonBiosUnitList) == "{}" { // empty struct
	...
	// try to unmarshal data into BiosUnitList
	err = json.Unmarshal(data, &dst.BiosUnitList)
	if err == nil {
		jsonBiosUnitList, _ := json.Marshal(dst.BiosUnitList)
		if string(jsonBiosUnitList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(BiosUnitResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(BiosUnitResponse)")
	}
}
