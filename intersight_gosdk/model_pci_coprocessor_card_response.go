...
func (dst *PciCoprocessorCardResponse) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into PciCoprocessorCardList
	err = json.Unmarshal(data, &dst.PciCoprocessorCardList)
	if err == nil {
		jsonPciCoprocessorCardList, _ := json.Marshal(dst.PciCoprocessorCardList)
		if string(jsonPciCoprocessorCardList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(PciCoprocessorCardResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(PciCoprocessorCardResponse)")
	}
}
