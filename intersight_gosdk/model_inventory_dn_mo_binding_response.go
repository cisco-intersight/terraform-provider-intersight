...
func (dst *InventoryDnMoBindingResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "inventory.DnMoBinding.List" {
		...
		if err == nil {
			jsonInventoryDnMoBindingList, _ := json.Marshal(dst.InventoryDnMoBindingList)
			if string(jsonInventoryDnMoBindingList) == "{}" { // empty struct
	...
	// try to unmarshal data into InventoryDnMoBindingList
	err = json.Unmarshal(data, &dst.InventoryDnMoBindingList)
	if err == nil {
		jsonInventoryDnMoBindingList, _ := json.Marshal(dst.InventoryDnMoBindingList)
		if string(jsonInventoryDnMoBindingList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(InventoryDnMoBindingResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(InventoryDnMoBindingResponse)")
	}
}
