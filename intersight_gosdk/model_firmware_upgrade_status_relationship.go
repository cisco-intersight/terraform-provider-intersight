...
func (dst *FirmwareUpgradeStatusRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "firmware.UpgradeStatus" {
		...
		if err == nil {
			jsonFirmwareUpgradeStatus, _ := json.Marshal(dst.FirmwareUpgradeStatus)
			if string(jsonFirmwareUpgradeStatus) == "{}" { // empty struct
	...
	// try to unmarshal data into FirmwareUpgradeStatus
	err = json.Unmarshal(data, &dst.FirmwareUpgradeStatus)
	if err == nil {
		jsonFirmwareUpgradeStatus, _ := json.Marshal(dst.FirmwareUpgradeStatus)
		if string(jsonFirmwareUpgradeStatus) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(FirmwareUpgradeStatusRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(FirmwareUpgradeStatusRelationship)")
	}
}
