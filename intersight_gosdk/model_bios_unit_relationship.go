...
func (dst *BiosUnitRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "bios.Unit" {
		...
		if err == nil {
			jsonBiosUnit, _ := json.Marshal(dst.BiosUnit)
			if string(jsonBiosUnit) == "{}" { // empty struct
				dst.BiosUnit = nil
			} else {
	...
	// try to unmarshal data into BiosUnit
	err = json.Unmarshal(data, &dst.BiosUnit)
	if err == nil {
		jsonBiosUnit, _ := json.Marshal(dst.BiosUnit)
		if string(jsonBiosUnit) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(BiosUnitRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(BiosUnitRelationship)")
	}
}
