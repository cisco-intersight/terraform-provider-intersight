...
func (dst *NiatelemetryNiaLicenseStateRelationship) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into NiatelemetryNiaLicenseState
	err = json.Unmarshal(data, &dst.NiatelemetryNiaLicenseState)
	if err == nil {
		jsonNiatelemetryNiaLicenseState, _ := json.Marshal(dst.NiatelemetryNiaLicenseState)
		if string(jsonNiatelemetryNiaLicenseState) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(NiatelemetryNiaLicenseStateRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(NiatelemetryNiaLicenseStateRelationship)")
	}
}
