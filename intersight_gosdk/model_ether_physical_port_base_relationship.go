...
func (dst *EtherPhysicalPortBaseRelationship) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into EtherPhysicalPortBase
	err = json.Unmarshal(data, &dst.EtherPhysicalPortBase)
	if err == nil {
		jsonEtherPhysicalPortBase, _ := json.Marshal(dst.EtherPhysicalPortBase)
		if string(jsonEtherPhysicalPortBase) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(EtherPhysicalPortBaseRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(EtherPhysicalPortBaseRelationship)")
	}
}
