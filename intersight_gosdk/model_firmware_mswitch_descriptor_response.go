...
func (dst *FirmwareMswitchDescriptorResponse) UnmarshalJSON(data []byte) error {
	var err error
	match := 0
	// try to unmarshal data into FirmwareMswitchDescriptorList
	err = json.Unmarshal(data, &dst.FirmwareMswitchDescriptorList)
	if err == nil {
		jsonFirmwareMswitchDescriptorList, _ := json.Marshal(dst.FirmwareMswitchDescriptorList)
		if string(jsonFirmwareMswitchDescriptorList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(FirmwareMswitchDescriptorResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(FirmwareMswitchDescriptorResponse)")
	}
}
