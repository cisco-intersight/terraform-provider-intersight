...
func (dst *HyperflexNodeResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "hyperflex.Node.List" {
		...
		if err == nil {
			jsonHyperflexNodeList, _ := json.Marshal(dst.HyperflexNodeList)
			if string(jsonHyperflexNodeList) == "{}" { // empty struct
	...
	// try to unmarshal data into HyperflexNodeList
	err = json.Unmarshal(data, &dst.HyperflexNodeList)
	if err == nil {
		jsonHyperflexNodeList, _ := json.Marshal(dst.HyperflexNodeList)
		if string(jsonHyperflexNodeList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(HyperflexNodeResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(HyperflexNodeResponse)")
	}
}
