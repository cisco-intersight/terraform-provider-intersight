...
func (dst *WorkflowCustomDataTypeDefinitionResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "workflow.CustomDataTypeDefinition.List" {
		...
		if err == nil {
			jsonWorkflowCustomDataTypeDefinitionList, _ := json.Marshal(dst.WorkflowCustomDataTypeDefinitionList)
			if string(jsonWorkflowCustomDataTypeDefinitionList) == "{}" { // empty struct
	...
	// try to unmarshal data into WorkflowCustomDataTypeDefinitionList
	err = json.Unmarshal(data, &dst.WorkflowCustomDataTypeDefinitionList)
	if err == nil {
		jsonWorkflowCustomDataTypeDefinitionList, _ := json.Marshal(dst.WorkflowCustomDataTypeDefinitionList)
		if string(jsonWorkflowCustomDataTypeDefinitionList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(WorkflowCustomDataTypeDefinitionResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(WorkflowCustomDataTypeDefinitionResponse)")
	}
}
