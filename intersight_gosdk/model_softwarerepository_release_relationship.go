...
func (dst *SoftwarerepositoryReleaseRelationship) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into SoftwarerepositoryRelease
	err = json.Unmarshal(data, &dst.SoftwarerepositoryRelease)
	if err == nil {
		jsonSoftwarerepositoryRelease, _ := json.Marshal(dst.SoftwarerepositoryRelease)
		if string(jsonSoftwarerepositoryRelease) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(SoftwarerepositoryReleaseRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(SoftwarerepositoryReleaseRelationship)")
	}
}
