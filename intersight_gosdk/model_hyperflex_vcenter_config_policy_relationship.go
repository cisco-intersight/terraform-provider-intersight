...
func (dst *HyperflexVcenterConfigPolicyRelationship) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into HyperflexVcenterConfigPolicy
	err = json.Unmarshal(data, &dst.HyperflexVcenterConfigPolicy)
	if err == nil {
		jsonHyperflexVcenterConfigPolicy, _ := json.Marshal(dst.HyperflexVcenterConfigPolicy)
		if string(jsonHyperflexVcenterConfigPolicy) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(HyperflexVcenterConfigPolicyRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(HyperflexVcenterConfigPolicyRelationship)")
	}
}
