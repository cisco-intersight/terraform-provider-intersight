...
func (dst *IamUserGroupResponse) UnmarshalJSON(data []byte) error {
	var err error
	match := 0
	// try to unmarshal data into IamUserGroupList
	err = json.Unmarshal(data, &dst.IamUserGroupList)
	if err == nil {
		jsonIamUserGroupList, _ := json.Marshal(dst.IamUserGroupList)
		if string(jsonIamUserGroupList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IamUserGroupResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IamUserGroupResponse)")
	}
}
