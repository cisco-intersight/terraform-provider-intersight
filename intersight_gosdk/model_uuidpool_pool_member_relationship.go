...
func (dst *UuidpoolPoolMemberRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "uuidpool.PoolMember" {
		...
		if err == nil {
			jsonUuidpoolPoolMember, _ := json.Marshal(dst.UuidpoolPoolMember)
			if string(jsonUuidpoolPoolMember) == "{}" { // empty struct
	...
	// try to unmarshal data into UuidpoolPoolMember
	err = json.Unmarshal(data, &dst.UuidpoolPoolMember)
	if err == nil {
		jsonUuidpoolPoolMember, _ := json.Marshal(dst.UuidpoolPoolMember)
		if string(jsonUuidpoolPoolMember) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(UuidpoolPoolMemberRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(UuidpoolPoolMemberRelationship)")
	}
}
