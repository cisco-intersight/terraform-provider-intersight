...
func (dst *EquipmentTransceiverResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "equipment.Transceiver.List" {
		...
		if err == nil {
			jsonEquipmentTransceiverList, _ := json.Marshal(dst.EquipmentTransceiverList)
			if string(jsonEquipmentTransceiverList) == "{}" { // empty struct
	...
	// try to unmarshal data into EquipmentTransceiverList
	err = json.Unmarshal(data, &dst.EquipmentTransceiverList)
	if err == nil {
		jsonEquipmentTransceiverList, _ := json.Marshal(dst.EquipmentTransceiverList)
		if string(jsonEquipmentTransceiverList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(EquipmentTransceiverResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(EquipmentTransceiverResponse)")
	}
}
