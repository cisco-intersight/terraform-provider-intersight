...
func (dst *WorkflowTaskInfoRelationship) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into WorkflowTaskInfo
	err = json.Unmarshal(data, &dst.WorkflowTaskInfo)
	if err == nil {
		jsonWorkflowTaskInfo, _ := json.Marshal(dst.WorkflowTaskInfo)
		if string(jsonWorkflowTaskInfo) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(WorkflowTaskInfoRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(WorkflowTaskInfoRelationship)")
	}
}
