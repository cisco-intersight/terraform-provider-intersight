...
func (dst *ComputeRackUnitResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "compute.RackUnit.List" {
		...
		if err == nil {
			jsonComputeRackUnitList, _ := json.Marshal(dst.ComputeRackUnitList)
			if string(jsonComputeRackUnitList) == "{}" { // empty struct
	...
	// try to unmarshal data into ComputeRackUnitList
	err = json.Unmarshal(data, &dst.ComputeRackUnitList)
	if err == nil {
		jsonComputeRackUnitList, _ := json.Marshal(dst.ComputeRackUnitList)
		if string(jsonComputeRackUnitList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(ComputeRackUnitResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(ComputeRackUnitResponse)")
	}
}
