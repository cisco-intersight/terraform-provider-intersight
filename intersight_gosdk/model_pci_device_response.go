...
func (dst *PciDeviceResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "pci.Device.List" {
		...
		if err == nil {
			jsonPciDeviceList, _ := json.Marshal(dst.PciDeviceList)
			if string(jsonPciDeviceList) == "{}" { // empty struct
	...
	// try to unmarshal data into PciDeviceList
	err = json.Unmarshal(data, &dst.PciDeviceList)
	if err == nil {
		jsonPciDeviceList, _ := json.Marshal(dst.PciDeviceList)
		if string(jsonPciDeviceList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(PciDeviceResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(PciDeviceResponse)")
	}
}
