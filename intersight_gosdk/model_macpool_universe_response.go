...
func (dst *MacpoolUniverseResponse) UnmarshalJSON(data []byte) error {
	var err error
	match := 0
	// try to unmarshal data into MacpoolUniverseList
	err = json.Unmarshal(data, &dst.MacpoolUniverseList)
	if err == nil {
		jsonMacpoolUniverseList, _ := json.Marshal(dst.MacpoolUniverseList)
		if string(jsonMacpoolUniverseList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(MacpoolUniverseResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(MacpoolUniverseResponse)")
	}
}
