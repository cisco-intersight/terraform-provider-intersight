...
func (dst *FaultInstanceResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "fault.Instance.List" {
		...
		if err == nil {
			jsonFaultInstanceList, _ := json.Marshal(dst.FaultInstanceList)
			if string(jsonFaultInstanceList) == "{}" { // empty struct
	...
	// try to unmarshal data into FaultInstanceList
	err = json.Unmarshal(data, &dst.FaultInstanceList)
	if err == nil {
		jsonFaultInstanceList, _ := json.Marshal(dst.FaultInstanceList)
		if string(jsonFaultInstanceList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(FaultInstanceResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(FaultInstanceResponse)")
	}
}
