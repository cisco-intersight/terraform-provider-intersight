...
func (dst *IamResourceLimitsResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "iam.ResourceLimits.List" {
		...
		if err == nil {
			jsonIamResourceLimitsList, _ := json.Marshal(dst.IamResourceLimitsList)
			if string(jsonIamResourceLimitsList) == "{}" { // empty struct
	...
	// try to unmarshal data into IamResourceLimitsList
	err = json.Unmarshal(data, &dst.IamResourceLimitsList)
	if err == nil {
		jsonIamResourceLimitsList, _ := json.Marshal(dst.IamResourceLimitsList)
		if string(jsonIamResourceLimitsList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IamResourceLimitsResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IamResourceLimitsResponse)")
	}
}
