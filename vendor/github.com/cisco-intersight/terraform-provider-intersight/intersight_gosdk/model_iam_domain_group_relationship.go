...
func (dst *IamDomainGroupRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "iam.DomainGroup" {
		...
		if err == nil {
			jsonIamDomainGroup, _ := json.Marshal(dst.IamDomainGroup)
			if string(jsonIamDomainGroup) == "{}" { // empty struct
	...
	// try to unmarshal data into IamDomainGroup
	err = json.Unmarshal(data, &dst.IamDomainGroup)
	if err == nil {
		jsonIamDomainGroup, _ := json.Marshal(dst.IamDomainGroup)
		if string(jsonIamDomainGroup) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IamDomainGroupRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IamDomainGroupRelationship)")
	}
}
