...
func (dst *StorageFlexUtilControllerRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "storage.FlexUtilController" {
		...
		if err == nil {
			jsonStorageFlexUtilController, _ := json.Marshal(dst.StorageFlexUtilController)
			if string(jsonStorageFlexUtilController) == "{}" { // empty struct
	...
	// try to unmarshal data into StorageFlexUtilController
	err = json.Unmarshal(data, &dst.StorageFlexUtilController)
	if err == nil {
		jsonStorageFlexUtilController, _ := json.Marshal(dst.StorageFlexUtilController)
		if string(jsonStorageFlexUtilController) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(StorageFlexUtilControllerRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(StorageFlexUtilControllerRelationship)")
	}
}
