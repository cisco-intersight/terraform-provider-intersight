...
func (dst *BootPrecisionPolicyResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "boot.PrecisionPolicy.List" {
		...
		if err == nil {
			jsonBootPrecisionPolicyList, _ := json.Marshal(dst.BootPrecisionPolicyList)
			if string(jsonBootPrecisionPolicyList) == "{}" { // empty struct
	...
	// try to unmarshal data into BootPrecisionPolicyList
	err = json.Unmarshal(data, &dst.BootPrecisionPolicyList)
	if err == nil {
		jsonBootPrecisionPolicyList, _ := json.Marshal(dst.BootPrecisionPolicyList)
		if string(jsonBootPrecisionPolicyList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(BootPrecisionPolicyResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(BootPrecisionPolicyResponse)")
	}
}
