...
func (dst *ManagementInterfaceResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "management.Interface.List" {
		...
		if err == nil {
			jsonManagementInterfaceList, _ := json.Marshal(dst.ManagementInterfaceList)
			if string(jsonManagementInterfaceList) == "{}" { // empty struct
	...
	// try to unmarshal data into ManagementInterfaceList
	err = json.Unmarshal(data, &dst.ManagementInterfaceList)
	if err == nil {
		jsonManagementInterfaceList, _ := json.Marshal(dst.ManagementInterfaceList)
		if string(jsonManagementInterfaceList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(ManagementInterfaceResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(ManagementInterfaceResponse)")
	}
}
