...
func (dst *StorageControllerRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "storage.Controller" {
		...
		if err == nil {
			jsonStorageController, _ := json.Marshal(dst.StorageController)
			if string(jsonStorageController) == "{}" { // empty struct
	...
	// try to unmarshal data into StorageController
	err = json.Unmarshal(data, &dst.StorageController)
	if err == nil {
		jsonStorageController, _ := json.Marshal(dst.StorageController)
		if string(jsonStorageController) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(StorageControllerRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(StorageControllerRelationship)")
	}
}
