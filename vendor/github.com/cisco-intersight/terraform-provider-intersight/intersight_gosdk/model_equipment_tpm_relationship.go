...
func (dst *EquipmentTpmRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "equipment.Tpm" {
		...
		if err == nil {
			jsonEquipmentTpm, _ := json.Marshal(dst.EquipmentTpm)
			if string(jsonEquipmentTpm) == "{}" { // empty struct
	...
	// try to unmarshal data into EquipmentTpm
	err = json.Unmarshal(data, &dst.EquipmentTpm)
	if err == nil {
		jsonEquipmentTpm, _ := json.Marshal(dst.EquipmentTpm)
		if string(jsonEquipmentTpm) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(EquipmentTpmRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(EquipmentTpmRelationship)")
	}
}
