...
func (dst *FirmwareUpgradeBaseRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "firmware.UpgradeBase" {
		...
		if err == nil {
			jsonFirmwareUpgradeBase, _ := json.Marshal(dst.FirmwareUpgradeBase)
			if string(jsonFirmwareUpgradeBase) == "{}" { // empty struct
	...
	// try to unmarshal data into FirmwareUpgradeBase
	err = json.Unmarshal(data, &dst.FirmwareUpgradeBase)
	if err == nil {
		jsonFirmwareUpgradeBase, _ := json.Marshal(dst.FirmwareUpgradeBase)
		if string(jsonFirmwareUpgradeBase) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(FirmwareUpgradeBaseRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(FirmwareUpgradeBaseRelationship)")
	}
}
