...
func (dst *SshPolicyResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "ssh.Policy.List" {
		...
		if err == nil {
			jsonSshPolicyList, _ := json.Marshal(dst.SshPolicyList)
			if string(jsonSshPolicyList) == "{}" { // empty struct
	...
	// try to unmarshal data into SshPolicyList
	err = json.Unmarshal(data, &dst.SshPolicyList)
	if err == nil {
		jsonSshPolicyList, _ := json.Marshal(dst.SshPolicyList)
		if string(jsonSshPolicyList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(SshPolicyResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(SshPolicyResponse)")
	}
}
