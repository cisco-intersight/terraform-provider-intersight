...
func (dst *HyperflexLocalCredentialPolicyRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "hyperflex.LocalCredentialPolicy" {
		...
		if err == nil {
			jsonHyperflexLocalCredentialPolicy, _ := json.Marshal(dst.HyperflexLocalCredentialPolicy)
			if string(jsonHyperflexLocalCredentialPolicy) == "{}" { // empty struct
	...
	// try to unmarshal data into HyperflexLocalCredentialPolicy
	err = json.Unmarshal(data, &dst.HyperflexLocalCredentialPolicy)
	if err == nil {
		jsonHyperflexLocalCredentialPolicy, _ := json.Marshal(dst.HyperflexLocalCredentialPolicy)
		if string(jsonHyperflexLocalCredentialPolicy) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(HyperflexLocalCredentialPolicyRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(HyperflexLocalCredentialPolicyRelationship)")
	}
}
