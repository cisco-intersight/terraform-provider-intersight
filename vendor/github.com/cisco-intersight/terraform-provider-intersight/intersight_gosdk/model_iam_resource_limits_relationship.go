...
func (dst *IamResourceLimitsRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "iam.ResourceLimits" {
		...
		if err == nil {
			jsonIamResourceLimits, _ := json.Marshal(dst.IamResourceLimits)
			if string(jsonIamResourceLimits) == "{}" { // empty struct
	...
	// try to unmarshal data into IamResourceLimits
	err = json.Unmarshal(data, &dst.IamResourceLimits)
	if err == nil {
		jsonIamResourceLimits, _ := json.Marshal(dst.IamResourceLimits)
		if string(jsonIamResourceLimits) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IamResourceLimitsRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IamResourceLimitsRelationship)")
	}
}
