...
func (dst *HyperflexClusterNetworkPolicyRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "hyperflex.ClusterNetworkPolicy" {
		...
		if err == nil {
			jsonHyperflexClusterNetworkPolicy, _ := json.Marshal(dst.HyperflexClusterNetworkPolicy)
			if string(jsonHyperflexClusterNetworkPolicy) == "{}" { // empty struct
	...
	// try to unmarshal data into HyperflexClusterNetworkPolicy
	err = json.Unmarshal(data, &dst.HyperflexClusterNetworkPolicy)
	if err == nil {
		jsonHyperflexClusterNetworkPolicy, _ := json.Marshal(dst.HyperflexClusterNetworkPolicy)
		if string(jsonHyperflexClusterNetworkPolicy) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(HyperflexClusterNetworkPolicyRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(HyperflexClusterNetworkPolicyRelationship)")
	}
}
