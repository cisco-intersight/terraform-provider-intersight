...
func (dst *SoftwarerepositoryOperatingSystemFileRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "softwarerepository.OperatingSystemFile" {
		...
		if err == nil {
			jsonSoftwarerepositoryOperatingSystemFile, _ := json.Marshal(dst.SoftwarerepositoryOperatingSystemFile)
			if string(jsonSoftwarerepositoryOperatingSystemFile) == "{}" { // empty struct
	...
	// try to unmarshal data into SoftwarerepositoryOperatingSystemFile
	err = json.Unmarshal(data, &dst.SoftwarerepositoryOperatingSystemFile)
	if err == nil {
		jsonSoftwarerepositoryOperatingSystemFile, _ := json.Marshal(dst.SoftwarerepositoryOperatingSystemFile)
		if string(jsonSoftwarerepositoryOperatingSystemFile) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(SoftwarerepositoryOperatingSystemFileRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(SoftwarerepositoryOperatingSystemFileRelationship)")
	}
}
