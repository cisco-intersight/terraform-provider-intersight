...
func (dst *FabricPortPolicyResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "fabric.PortPolicy.List" {
		...
		if err == nil {
			jsonFabricPortPolicyList, _ := json.Marshal(dst.FabricPortPolicyList)
			if string(jsonFabricPortPolicyList) == "{}" { // empty struct
	...
	// try to unmarshal data into FabricPortPolicyList
	err = json.Unmarshal(data, &dst.FabricPortPolicyList)
	if err == nil {
		jsonFabricPortPolicyList, _ := json.Marshal(dst.FabricPortPolicyList)
		if string(jsonFabricPortPolicyList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(FabricPortPolicyResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(FabricPortPolicyResponse)")
	}
}
