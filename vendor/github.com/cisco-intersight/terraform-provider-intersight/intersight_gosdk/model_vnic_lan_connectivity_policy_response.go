...
func (dst *VnicLanConnectivityPolicyResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "vnic.LanConnectivityPolicy.List" {
		...
		if err == nil {
			jsonVnicLanConnectivityPolicyList, _ := json.Marshal(dst.VnicLanConnectivityPolicyList)
			if string(jsonVnicLanConnectivityPolicyList) == "{}" { // empty struct
	...
	// try to unmarshal data into VnicLanConnectivityPolicyList
	err = json.Unmarshal(data, &dst.VnicLanConnectivityPolicyList)
	if err == nil {
		jsonVnicLanConnectivityPolicyList, _ := json.Marshal(dst.VnicLanConnectivityPolicyList)
		if string(jsonVnicLanConnectivityPolicyList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(VnicLanConnectivityPolicyResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(VnicLanConnectivityPolicyResponse)")
	}
}
