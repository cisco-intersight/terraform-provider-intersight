...
func (dst *IamSecurityHolderRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "iam.SecurityHolder" {
		...
		if err == nil {
			jsonIamSecurityHolder, _ := json.Marshal(dst.IamSecurityHolder)
			if string(jsonIamSecurityHolder) == "{}" { // empty struct
	...
	// try to unmarshal data into IamSecurityHolder
	err = json.Unmarshal(data, &dst.IamSecurityHolder)
	if err == nil {
		jsonIamSecurityHolder, _ := json.Marshal(dst.IamSecurityHolder)
		if string(jsonIamSecurityHolder) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IamSecurityHolderRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IamSecurityHolderRelationship)")
	}
}
