...
func (dst *StoragePureReplicationScheduleResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "storage.PureReplicationSchedule.List" {
		...
		if err == nil {
			jsonStoragePureReplicationScheduleList, _ := json.Marshal(dst.StoragePureReplicationScheduleList)
			if string(jsonStoragePureReplicationScheduleList) == "{}" { // empty struct
	...
	// try to unmarshal data into StoragePureReplicationScheduleList
	err = json.Unmarshal(data, &dst.StoragePureReplicationScheduleList)
	if err == nil {
		jsonStoragePureReplicationScheduleList, _ := json.Marshal(dst.StoragePureReplicationScheduleList)
		if string(jsonStoragePureReplicationScheduleList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(StoragePureReplicationScheduleResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(StoragePureReplicationScheduleResponse)")
	}
}
