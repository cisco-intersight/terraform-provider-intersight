...
func (dst *StorageEnclosureRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "storage.Enclosure" {
		...
		if err == nil {
			jsonStorageEnclosure, _ := json.Marshal(dst.StorageEnclosure)
			if string(jsonStorageEnclosure) == "{}" { // empty struct
	...
	// try to unmarshal data into StorageEnclosure
	err = json.Unmarshal(data, &dst.StorageEnclosure)
	if err == nil {
		jsonStorageEnclosure, _ := json.Marshal(dst.StorageEnclosure)
		if string(jsonStorageEnclosure) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(StorageEnclosureRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(StorageEnclosureRelationship)")
	}
}
