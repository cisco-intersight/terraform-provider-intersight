...
func (dst *IamSessionLimitsResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "iam.SessionLimits.List" {
		...
		if err == nil {
			jsonIamSessionLimitsList, _ := json.Marshal(dst.IamSessionLimitsList)
			if string(jsonIamSessionLimitsList) == "{}" { // empty struct
	...
	// try to unmarshal data into IamSessionLimitsList
	err = json.Unmarshal(data, &dst.IamSessionLimitsList)
	if err == nil {
		jsonIamSessionLimitsList, _ := json.Marshal(dst.IamSessionLimitsList)
		if string(jsonIamSessionLimitsList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IamSessionLimitsResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IamSessionLimitsResponse)")
	}
}
