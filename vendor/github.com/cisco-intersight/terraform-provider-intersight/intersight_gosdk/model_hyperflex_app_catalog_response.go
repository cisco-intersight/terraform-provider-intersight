...
func (dst *HyperflexAppCatalogResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "hyperflex.AppCatalog.List" {
		...
		if err == nil {
			jsonHyperflexAppCatalogList, _ := json.Marshal(dst.HyperflexAppCatalogList)
			if string(jsonHyperflexAppCatalogList) == "{}" { // empty struct
	...
	// try to unmarshal data into HyperflexAppCatalogList
	err = json.Unmarshal(data, &dst.HyperflexAppCatalogList)
	if err == nil {
		jsonHyperflexAppCatalogList, _ := json.Marshal(dst.HyperflexAppCatalogList)
		if string(jsonHyperflexAppCatalogList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(HyperflexAppCatalogResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(HyperflexAppCatalogResponse)")
	}
}
