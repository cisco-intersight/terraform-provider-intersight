...
func (dst *EtherPhysicalPortRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "ether.PhysicalPort" {
		...
		if err == nil {
			jsonEtherPhysicalPort, _ := json.Marshal(dst.EtherPhysicalPort)
			if string(jsonEtherPhysicalPort) == "{}" { // empty struct
	...
	// try to unmarshal data into EtherPhysicalPort
	err = json.Unmarshal(data, &dst.EtherPhysicalPort)
	if err == nil {
		jsonEtherPhysicalPort, _ := json.Marshal(dst.EtherPhysicalPort)
		if string(jsonEtherPhysicalPort) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(EtherPhysicalPortRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(EtherPhysicalPortRelationship)")
	}
}
