...
func (dst *IamEndPointPrivilegeRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "iam.EndPointPrivilege" {
		...
		if err == nil {
			jsonIamEndPointPrivilege, _ := json.Marshal(dst.IamEndPointPrivilege)
			if string(jsonIamEndPointPrivilege) == "{}" { // empty struct
	...
	// try to unmarshal data into IamEndPointPrivilege
	err = json.Unmarshal(data, &dst.IamEndPointPrivilege)
	if err == nil {
		jsonIamEndPointPrivilege, _ := json.Marshal(dst.IamEndPointPrivilege)
		if string(jsonIamEndPointPrivilege) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IamEndPointPrivilegeRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IamEndPointPrivilegeRelationship)")
	}
}
