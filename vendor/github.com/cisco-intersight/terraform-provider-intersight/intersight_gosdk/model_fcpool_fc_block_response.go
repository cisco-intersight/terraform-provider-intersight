...
func (dst *FcpoolFcBlockResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "fcpool.FcBlock.List" {
		...
		if err == nil {
			jsonFcpoolFcBlockList, _ := json.Marshal(dst.FcpoolFcBlockList)
			if string(jsonFcpoolFcBlockList) == "{}" { // empty struct
	...
	// try to unmarshal data into FcpoolFcBlockList
	err = json.Unmarshal(data, &dst.FcpoolFcBlockList)
	if err == nil {
		jsonFcpoolFcBlockList, _ := json.Marshal(dst.FcpoolFcBlockList)
		if string(jsonFcpoolFcBlockList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(FcpoolFcBlockResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(FcpoolFcBlockResponse)")
	}
}
