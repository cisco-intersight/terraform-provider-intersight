...
func (dst *EquipmentIoExpanderRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "equipment.IoExpander" {
		...
		if err == nil {
			jsonEquipmentIoExpander, _ := json.Marshal(dst.EquipmentIoExpander)
			if string(jsonEquipmentIoExpander) == "{}" { // empty struct
	...
	// try to unmarshal data into EquipmentIoExpander
	err = json.Unmarshal(data, &dst.EquipmentIoExpander)
	if err == nil {
		jsonEquipmentIoExpander, _ := json.Marshal(dst.EquipmentIoExpander)
		if string(jsonEquipmentIoExpander) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(EquipmentIoExpanderRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(EquipmentIoExpanderRelationship)")
	}
}
