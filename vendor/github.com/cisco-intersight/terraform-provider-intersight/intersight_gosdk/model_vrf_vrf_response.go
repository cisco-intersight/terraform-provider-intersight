...
func (dst *VrfVrfResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "vrf.Vrf.List" {
		...
		if err == nil {
			jsonVrfVrfList, _ := json.Marshal(dst.VrfVrfList)
			if string(jsonVrfVrfList) == "{}" { // empty struct
				dst.VrfVrfList = nil
			} else {
	...
	// try to unmarshal data into VrfVrfList
	err = json.Unmarshal(data, &dst.VrfVrfList)
	if err == nil {
		jsonVrfVrfList, _ := json.Marshal(dst.VrfVrfList)
		if string(jsonVrfVrfList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(VrfVrfResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(VrfVrfResponse)")
	}
}
