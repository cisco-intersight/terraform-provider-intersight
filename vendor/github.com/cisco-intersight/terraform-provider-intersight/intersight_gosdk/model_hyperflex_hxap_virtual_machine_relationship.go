...
func (dst *HyperflexHxapVirtualMachineRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "hyperflex.HxapVirtualMachine" {
		...
		if err == nil {
			jsonHyperflexHxapVirtualMachine, _ := json.Marshal(dst.HyperflexHxapVirtualMachine)
			if string(jsonHyperflexHxapVirtualMachine) == "{}" { // empty struct
	...
	// try to unmarshal data into HyperflexHxapVirtualMachine
	err = json.Unmarshal(data, &dst.HyperflexHxapVirtualMachine)
	if err == nil {
		jsonHyperflexHxapVirtualMachine, _ := json.Marshal(dst.HyperflexHxapVirtualMachine)
		if string(jsonHyperflexHxapVirtualMachine) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(HyperflexHxapVirtualMachineRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(HyperflexHxapVirtualMachineRelationship)")
	}
}
