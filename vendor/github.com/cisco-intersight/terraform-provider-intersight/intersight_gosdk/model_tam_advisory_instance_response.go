...
func (dst *TamAdvisoryInstanceResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "tam.AdvisoryInstance.List" {
		...
		if err == nil {
			jsonTamAdvisoryInstanceList, _ := json.Marshal(dst.TamAdvisoryInstanceList)
			if string(jsonTamAdvisoryInstanceList) == "{}" { // empty struct
	...
	// try to unmarshal data into TamAdvisoryInstanceList
	err = json.Unmarshal(data, &dst.TamAdvisoryInstanceList)
	if err == nil {
		jsonTamAdvisoryInstanceList, _ := json.Marshal(dst.TamAdvisoryInstanceList)
		if string(jsonTamAdvisoryInstanceList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(TamAdvisoryInstanceResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(TamAdvisoryInstanceResponse)")
	}
}
