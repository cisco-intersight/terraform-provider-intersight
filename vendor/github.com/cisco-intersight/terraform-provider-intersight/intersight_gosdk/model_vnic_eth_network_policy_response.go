...
func (dst *VnicEthNetworkPolicyResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "vnic.EthNetworkPolicy.List" {
		...
		if err == nil {
			jsonVnicEthNetworkPolicyList, _ := json.Marshal(dst.VnicEthNetworkPolicyList)
			if string(jsonVnicEthNetworkPolicyList) == "{}" { // empty struct
	...
	// try to unmarshal data into VnicEthNetworkPolicyList
	err = json.Unmarshal(data, &dst.VnicEthNetworkPolicyList)
	if err == nil {
		jsonVnicEthNetworkPolicyList, _ := json.Marshal(dst.VnicEthNetworkPolicyList)
		if string(jsonVnicEthNetworkPolicyList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(VnicEthNetworkPolicyResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(VnicEthNetworkPolicyResponse)")
	}
}
