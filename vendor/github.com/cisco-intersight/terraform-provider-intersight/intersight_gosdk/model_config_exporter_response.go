...
func (dst *ConfigExporterResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "config.Exporter.List" {
		...
		if err == nil {
			jsonConfigExporterList, _ := json.Marshal(dst.ConfigExporterList)
			if string(jsonConfigExporterList) == "{}" { // empty struct
	...
	// try to unmarshal data into ConfigExporterList
	err = json.Unmarshal(data, &dst.ConfigExporterList)
	if err == nil {
		jsonConfigExporterList, _ := json.Marshal(dst.ConfigExporterList)
		if string(jsonConfigExporterList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(ConfigExporterResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(ConfigExporterResponse)")
	}
}
