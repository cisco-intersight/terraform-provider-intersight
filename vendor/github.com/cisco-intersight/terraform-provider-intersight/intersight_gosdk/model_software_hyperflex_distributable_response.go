...
func (dst *SoftwareHyperflexDistributableResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "software.HyperflexDistributable.List" {
		...
		if err == nil {
			jsonSoftwareHyperflexDistributableList, _ := json.Marshal(dst.SoftwareHyperflexDistributableList)
			if string(jsonSoftwareHyperflexDistributableList) == "{}" { // empty struct
	...
	// try to unmarshal data into SoftwareHyperflexDistributableList
	err = json.Unmarshal(data, &dst.SoftwareHyperflexDistributableList)
	if err == nil {
		jsonSoftwareHyperflexDistributableList, _ := json.Marshal(dst.SoftwareHyperflexDistributableList)
		if string(jsonSoftwareHyperflexDistributableList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(SoftwareHyperflexDistributableResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(SoftwareHyperflexDistributableResponse)")
	}
}
