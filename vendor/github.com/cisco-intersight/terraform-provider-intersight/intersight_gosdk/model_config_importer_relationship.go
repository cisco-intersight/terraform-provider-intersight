...
func (dst *ConfigImporterRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "config.Importer" {
		...
		if err == nil {
			jsonConfigImporter, _ := json.Marshal(dst.ConfigImporter)
			if string(jsonConfigImporter) == "{}" { // empty struct
	...
	// try to unmarshal data into ConfigImporter
	err = json.Unmarshal(data, &dst.ConfigImporter)
	if err == nil {
		jsonConfigImporter, _ := json.Marshal(dst.ConfigImporter)
		if string(jsonConfigImporter) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(ConfigImporterRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(ConfigImporterRelationship)")
	}
}
