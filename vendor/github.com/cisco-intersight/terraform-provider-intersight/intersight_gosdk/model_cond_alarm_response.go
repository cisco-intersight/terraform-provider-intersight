...
func (dst *CondAlarmResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "cond.Alarm.List" {
		...
		if err == nil {
			jsonCondAlarmList, _ := json.Marshal(dst.CondAlarmList)
			if string(jsonCondAlarmList) == "{}" { // empty struct
	...
	// try to unmarshal data into CondAlarmList
	err = json.Unmarshal(data, &dst.CondAlarmList)
	if err == nil {
		jsonCondAlarmList, _ := json.Marshal(dst.CondAlarmList)
		if string(jsonCondAlarmList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(CondAlarmResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(CondAlarmResponse)")
	}
}
