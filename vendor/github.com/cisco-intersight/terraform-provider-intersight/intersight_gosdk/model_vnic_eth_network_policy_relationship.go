...
func (dst *VnicEthNetworkPolicyRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "vnic.EthNetworkPolicy" {
		...
		if err == nil {
			jsonVnicEthNetworkPolicy, _ := json.Marshal(dst.VnicEthNetworkPolicy)
			if string(jsonVnicEthNetworkPolicy) == "{}" { // empty struct
	...
	// try to unmarshal data into VnicEthNetworkPolicy
	err = json.Unmarshal(data, &dst.VnicEthNetworkPolicy)
	if err == nil {
		jsonVnicEthNetworkPolicy, _ := json.Marshal(dst.VnicEthNetworkPolicy)
		if string(jsonVnicEthNetworkPolicy) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(VnicEthNetworkPolicyRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(VnicEthNetworkPolicyRelationship)")
	}
}
