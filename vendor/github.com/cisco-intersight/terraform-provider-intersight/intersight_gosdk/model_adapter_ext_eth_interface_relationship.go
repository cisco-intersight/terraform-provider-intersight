...
func (dst *AdapterExtEthInterfaceRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "adapter.ExtEthInterface" {
		...
		if err == nil {
			jsonAdapterExtEthInterface, _ := json.Marshal(dst.AdapterExtEthInterface)
			if string(jsonAdapterExtEthInterface) == "{}" { // empty struct
	...
	// try to unmarshal data into AdapterExtEthInterface
	err = json.Unmarshal(data, &dst.AdapterExtEthInterface)
	if err == nil {
		jsonAdapterExtEthInterface, _ := json.Marshal(dst.AdapterExtEthInterface)
		if string(jsonAdapterExtEthInterface) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(AdapterExtEthInterfaceRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(AdapterExtEthInterfaceRelationship)")
	}
}
