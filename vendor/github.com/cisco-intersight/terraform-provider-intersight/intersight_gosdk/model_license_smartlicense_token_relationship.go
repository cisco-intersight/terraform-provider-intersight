...
func (dst *LicenseSmartlicenseTokenRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "license.SmartlicenseToken" {
		...
		if err == nil {
			jsonLicenseSmartlicenseToken, _ := json.Marshal(dst.LicenseSmartlicenseToken)
			if string(jsonLicenseSmartlicenseToken) == "{}" { // empty struct
	...
	// try to unmarshal data into LicenseSmartlicenseToken
	err = json.Unmarshal(data, &dst.LicenseSmartlicenseToken)
	if err == nil {
		jsonLicenseSmartlicenseToken, _ := json.Marshal(dst.LicenseSmartlicenseToken)
		if string(jsonLicenseSmartlicenseToken) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(LicenseSmartlicenseTokenRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(LicenseSmartlicenseTokenRelationship)")
	}
}
