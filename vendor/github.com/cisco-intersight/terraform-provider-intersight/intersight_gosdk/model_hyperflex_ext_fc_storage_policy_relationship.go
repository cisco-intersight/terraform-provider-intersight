...
func (dst *HyperflexExtFcStoragePolicyRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "hyperflex.ExtFcStoragePolicy" {
		...
		if err == nil {
			jsonHyperflexExtFcStoragePolicy, _ := json.Marshal(dst.HyperflexExtFcStoragePolicy)
			if string(jsonHyperflexExtFcStoragePolicy) == "{}" { // empty struct
	...
	// try to unmarshal data into HyperflexExtFcStoragePolicy
	err = json.Unmarshal(data, &dst.HyperflexExtFcStoragePolicy)
	if err == nil {
		jsonHyperflexExtFcStoragePolicy, _ := json.Marshal(dst.HyperflexExtFcStoragePolicy)
		if string(jsonHyperflexExtFcStoragePolicy) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(HyperflexExtFcStoragePolicyRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(HyperflexExtFcStoragePolicyRelationship)")
	}
}
