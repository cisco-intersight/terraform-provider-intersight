...
func (dst *IamApiKeyRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "iam.ApiKey" {
		...
		if err == nil {
			jsonIamApiKey, _ := json.Marshal(dst.IamApiKey)
			if string(jsonIamApiKey) == "{}" { // empty struct
				dst.IamApiKey = nil
			} else {
	...
	// try to unmarshal data into IamApiKey
	err = json.Unmarshal(data, &dst.IamApiKey)
	if err == nil {
		jsonIamApiKey, _ := json.Marshal(dst.IamApiKey)
		if string(jsonIamApiKey) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IamApiKeyRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IamApiKeyRelationship)")
	}
}
