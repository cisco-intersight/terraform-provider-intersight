...
func (dst *IpmioverlanPolicyResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "ipmioverlan.Policy.List" {
		...
		if err == nil {
			jsonIpmioverlanPolicyList, _ := json.Marshal(dst.IpmioverlanPolicyList)
			if string(jsonIpmioverlanPolicyList) == "{}" { // empty struct
	...
	// try to unmarshal data into IpmioverlanPolicyList
	err = json.Unmarshal(data, &dst.IpmioverlanPolicyList)
	if err == nil {
		jsonIpmioverlanPolicyList, _ := json.Marshal(dst.IpmioverlanPolicyList)
		if string(jsonIpmioverlanPolicyList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IpmioverlanPolicyResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IpmioverlanPolicyResponse)")
	}
}
