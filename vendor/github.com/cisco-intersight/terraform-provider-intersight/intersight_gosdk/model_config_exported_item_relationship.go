...
func (dst *ConfigExportedItemRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "config.ExportedItem" {
		...
		if err == nil {
			jsonConfigExportedItem, _ := json.Marshal(dst.ConfigExportedItem)
			if string(jsonConfigExportedItem) == "{}" { // empty struct
	...
	// try to unmarshal data into ConfigExportedItem
	err = json.Unmarshal(data, &dst.ConfigExportedItem)
	if err == nil {
		jsonConfigExportedItem, _ := json.Marshal(dst.ConfigExportedItem)
		if string(jsonConfigExportedItem) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(ConfigExportedItemRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(ConfigExportedItemRelationship)")
	}
}
