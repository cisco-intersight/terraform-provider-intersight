...
func (dst *VirtualizationVmwareVcenterResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "virtualization.VmwareVcenter.List" {
		...
		if err == nil {
			jsonVirtualizationVmwareVcenterList, _ := json.Marshal(dst.VirtualizationVmwareVcenterList)
			if string(jsonVirtualizationVmwareVcenterList) == "{}" { // empty struct
	...
	// try to unmarshal data into VirtualizationVmwareVcenterList
	err = json.Unmarshal(data, &dst.VirtualizationVmwareVcenterList)
	if err == nil {
		jsonVirtualizationVmwareVcenterList, _ := json.Marshal(dst.VirtualizationVmwareVcenterList)
		if string(jsonVirtualizationVmwareVcenterList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(VirtualizationVmwareVcenterResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(VirtualizationVmwareVcenterResponse)")
	}
}
