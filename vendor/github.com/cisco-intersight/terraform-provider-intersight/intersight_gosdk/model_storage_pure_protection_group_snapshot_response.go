...
func (dst *StoragePureProtectionGroupSnapshotResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "storage.PureProtectionGroupSnapshot.List" {
		...
		if err == nil {
			jsonStoragePureProtectionGroupSnapshotList, _ := json.Marshal(dst.StoragePureProtectionGroupSnapshotList)
			if string(jsonStoragePureProtectionGroupSnapshotList) == "{}" { // empty struct
	...
	// try to unmarshal data into StoragePureProtectionGroupSnapshotList
	err = json.Unmarshal(data, &dst.StoragePureProtectionGroupSnapshotList)
	if err == nil {
		jsonStoragePureProtectionGroupSnapshotList, _ := json.Marshal(dst.StoragePureProtectionGroupSnapshotList)
		if string(jsonStoragePureProtectionGroupSnapshotList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(StoragePureProtectionGroupSnapshotResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(StoragePureProtectionGroupSnapshotResponse)")
	}
}
