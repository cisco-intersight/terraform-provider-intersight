...
func (dst *IamPrivilegeResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "iam.Privilege.List" {
		...
		if err == nil {
			jsonIamPrivilegeList, _ := json.Marshal(dst.IamPrivilegeList)
			if string(jsonIamPrivilegeList) == "{}" { // empty struct
	...
	// try to unmarshal data into IamPrivilegeList
	err = json.Unmarshal(data, &dst.IamPrivilegeList)
	if err == nil {
		jsonIamPrivilegeList, _ := json.Marshal(dst.IamPrivilegeList)
		if string(jsonIamPrivilegeList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IamPrivilegeResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IamPrivilegeResponse)")
	}
}
