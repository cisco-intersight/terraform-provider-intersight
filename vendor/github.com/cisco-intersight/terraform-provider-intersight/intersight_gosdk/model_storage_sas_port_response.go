...
func (dst *StorageSasPortResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "storage.SasPort.List" {
		...
		if err == nil {
			jsonStorageSasPortList, _ := json.Marshal(dst.StorageSasPortList)
			if string(jsonStorageSasPortList) == "{}" { // empty struct
	...
	// try to unmarshal data into StorageSasPortList
	err = json.Unmarshal(data, &dst.StorageSasPortList)
	if err == nil {
		jsonStorageSasPortList, _ := json.Marshal(dst.StorageSasPortList)
		if string(jsonStorageSasPortList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(StorageSasPortResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(StorageSasPortResponse)")
	}
}
