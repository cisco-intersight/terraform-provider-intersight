...
func (dst *FirmwareUpgradeImpactStatusResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "firmware.UpgradeImpactStatus.List" {
		...
		if err == nil {
			jsonFirmwareUpgradeImpactStatusList, _ := json.Marshal(dst.FirmwareUpgradeImpactStatusList)
			if string(jsonFirmwareUpgradeImpactStatusList) == "{}" { // empty struct
	...
	// try to unmarshal data into FirmwareUpgradeImpactStatusList
	err = json.Unmarshal(data, &dst.FirmwareUpgradeImpactStatusList)
	if err == nil {
		jsonFirmwareUpgradeImpactStatusList, _ := json.Marshal(dst.FirmwareUpgradeImpactStatusList)
		if string(jsonFirmwareUpgradeImpactStatusList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(FirmwareUpgradeImpactStatusResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(FirmwareUpgradeImpactStatusResponse)")
	}
}
