...
func (dst *IamLdapPolicyRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "iam.LdapPolicy" {
		...
		if err == nil {
			jsonIamLdapPolicy, _ := json.Marshal(dst.IamLdapPolicy)
			if string(jsonIamLdapPolicy) == "{}" { // empty struct
	...
	// try to unmarshal data into IamLdapPolicy
	err = json.Unmarshal(data, &dst.IamLdapPolicy)
	if err == nil {
		jsonIamLdapPolicy, _ := json.Marshal(dst.IamLdapPolicy)
		if string(jsonIamLdapPolicy) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IamLdapPolicyRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IamLdapPolicyRelationship)")
	}
}
