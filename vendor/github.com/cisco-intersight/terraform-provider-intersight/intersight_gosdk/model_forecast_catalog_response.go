...
func (dst *ForecastCatalogResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "forecast.Catalog.List" {
		...
		if err == nil {
			jsonForecastCatalogList, _ := json.Marshal(dst.ForecastCatalogList)
			if string(jsonForecastCatalogList) == "{}" { // empty struct
	...
	// try to unmarshal data into ForecastCatalogList
	err = json.Unmarshal(data, &dst.ForecastCatalogList)
	if err == nil {
		jsonForecastCatalogList, _ := json.Marshal(dst.ForecastCatalogList)
		if string(jsonForecastCatalogList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(ForecastCatalogResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(ForecastCatalogResponse)")
	}
}
