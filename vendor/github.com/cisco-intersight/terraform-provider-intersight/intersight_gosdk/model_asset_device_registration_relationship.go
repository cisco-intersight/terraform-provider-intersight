...
func (dst *AssetDeviceRegistrationRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "asset.DeviceRegistration" {
		...
		if err == nil {
			jsonAssetDeviceRegistration, _ := json.Marshal(dst.AssetDeviceRegistration)
			if string(jsonAssetDeviceRegistration) == "{}" { // empty struct
	...
	// try to unmarshal data into AssetDeviceRegistration
	err = json.Unmarshal(data, &dst.AssetDeviceRegistration)
	if err == nil {
		jsonAssetDeviceRegistration, _ := json.Marshal(dst.AssetDeviceRegistration)
		if string(jsonAssetDeviceRegistration) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(AssetDeviceRegistrationRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(AssetDeviceRegistrationRelationship)")
	}
}
