...
func (dst *EquipmentIoCardRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "equipment.IoCard" {
		...
		if err == nil {
			jsonEquipmentIoCard, _ := json.Marshal(dst.EquipmentIoCard)
			if string(jsonEquipmentIoCard) == "{}" { // empty struct
	...
	// try to unmarshal data into EquipmentIoCard
	err = json.Unmarshal(data, &dst.EquipmentIoCard)
	if err == nil {
		jsonEquipmentIoCard, _ := json.Marshal(dst.EquipmentIoCard)
		if string(jsonEquipmentIoCard) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(EquipmentIoCardRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(EquipmentIoCardRelationship)")
	}
}
