...
func (dst *FabricUplinkRoleResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "fabric.UplinkRole.List" {
		...
		if err == nil {
			jsonFabricUplinkRoleList, _ := json.Marshal(dst.FabricUplinkRoleList)
			if string(jsonFabricUplinkRoleList) == "{}" { // empty struct
	...
	// try to unmarshal data into FabricUplinkRoleList
	err = json.Unmarshal(data, &dst.FabricUplinkRoleList)
	if err == nil {
		jsonFabricUplinkRoleList, _ := json.Marshal(dst.FabricUplinkRoleList)
		if string(jsonFabricUplinkRoleList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(FabricUplinkRoleResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(FabricUplinkRoleResponse)")
	}
}
