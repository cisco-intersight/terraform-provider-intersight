...
func (dst *IamPrivilegeRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "iam.Privilege" {
		...
		if err == nil {
			jsonIamPrivilege, _ := json.Marshal(dst.IamPrivilege)
			if string(jsonIamPrivilege) == "{}" { // empty struct
	...
	// try to unmarshal data into IamPrivilege
	err = json.Unmarshal(data, &dst.IamPrivilege)
	if err == nil {
		jsonIamPrivilege, _ := json.Marshal(dst.IamPrivilege)
		if string(jsonIamPrivilege) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IamPrivilegeRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IamPrivilegeRelationship)")
	}
}
