...
func (dst *IamAccountRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "iam.Account" {
		...
		if err == nil {
			jsonIamAccount, _ := json.Marshal(dst.IamAccount)
			if string(jsonIamAccount) == "{}" { // empty struct
				dst.IamAccount = nil
			} else {
	...
	// try to unmarshal data into IamAccount
	err = json.Unmarshal(data, &dst.IamAccount)
	if err == nil {
		jsonIamAccount, _ := json.Marshal(dst.IamAccount)
		if string(jsonIamAccount) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IamAccountRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IamAccountRelationship)")
	}
}
