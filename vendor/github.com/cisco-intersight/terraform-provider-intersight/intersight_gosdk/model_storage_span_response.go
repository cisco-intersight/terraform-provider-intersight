...
func (dst *StorageSpanResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "storage.Span.List" {
		...
		if err == nil {
			jsonStorageSpanList, _ := json.Marshal(dst.StorageSpanList)
			if string(jsonStorageSpanList) == "{}" { // empty struct
	...
	// try to unmarshal data into StorageSpanList
	err = json.Unmarshal(data, &dst.StorageSpanList)
	if err == nil {
		jsonStorageSpanList, _ := json.Marshal(dst.StorageSpanList)
		if string(jsonStorageSpanList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(StorageSpanResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(StorageSpanResponse)")
	}
}
