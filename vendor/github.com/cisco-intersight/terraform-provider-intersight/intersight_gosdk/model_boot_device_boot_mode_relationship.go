...
func (dst *BootDeviceBootModeRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "boot.DeviceBootMode" {
		...
		if err == nil {
			jsonBootDeviceBootMode, _ := json.Marshal(dst.BootDeviceBootMode)
			if string(jsonBootDeviceBootMode) == "{}" { // empty struct
	...
	// try to unmarshal data into BootDeviceBootMode
	err = json.Unmarshal(data, &dst.BootDeviceBootMode)
	if err == nil {
		jsonBootDeviceBootMode, _ := json.Marshal(dst.BootDeviceBootMode)
		if string(jsonBootDeviceBootMode) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(BootDeviceBootModeRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(BootDeviceBootModeRelationship)")
	}
}
