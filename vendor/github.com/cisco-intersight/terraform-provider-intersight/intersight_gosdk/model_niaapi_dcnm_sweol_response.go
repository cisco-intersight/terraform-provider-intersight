...
func (dst *NiaapiDcnmSweolResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "niaapi.DcnmSweol.List" {
		...
		if err == nil {
			jsonNiaapiDcnmSweolList, _ := json.Marshal(dst.NiaapiDcnmSweolList)
			if string(jsonNiaapiDcnmSweolList) == "{}" { // empty struct
	...
	// try to unmarshal data into NiaapiDcnmSweolList
	err = json.Unmarshal(data, &dst.NiaapiDcnmSweolList)
	if err == nil {
		jsonNiaapiDcnmSweolList, _ := json.Marshal(dst.NiaapiDcnmSweolList)
		if string(jsonNiaapiDcnmSweolList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(NiaapiDcnmSweolResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(NiaapiDcnmSweolResponse)")
	}
}
