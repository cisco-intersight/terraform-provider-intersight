...
func (dst *TamAdvisoryInfoResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "tam.AdvisoryInfo.List" {
		...
		if err == nil {
			jsonTamAdvisoryInfoList, _ := json.Marshal(dst.TamAdvisoryInfoList)
			if string(jsonTamAdvisoryInfoList) == "{}" { // empty struct
	...
	// try to unmarshal data into TamAdvisoryInfoList
	err = json.Unmarshal(data, &dst.TamAdvisoryInfoList)
	if err == nil {
		jsonTamAdvisoryInfoList, _ := json.Marshal(dst.TamAdvisoryInfoList)
		if string(jsonTamAdvisoryInfoList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(TamAdvisoryInfoResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(TamAdvisoryInfoResponse)")
	}
}
