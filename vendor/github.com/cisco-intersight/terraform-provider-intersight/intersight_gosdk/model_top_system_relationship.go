...
func (dst *TopSystemRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "top.System" {
		...
		if err == nil {
			jsonTopSystem, _ := json.Marshal(dst.TopSystem)
			if string(jsonTopSystem) == "{}" { // empty struct
				dst.TopSystem = nil
			} else {
	...
	// try to unmarshal data into TopSystem
	err = json.Unmarshal(data, &dst.TopSystem)
	if err == nil {
		jsonTopSystem, _ := json.Marshal(dst.TopSystem)
		if string(jsonTopSystem) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(TopSystemRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(TopSystemRelationship)")
	}
}
