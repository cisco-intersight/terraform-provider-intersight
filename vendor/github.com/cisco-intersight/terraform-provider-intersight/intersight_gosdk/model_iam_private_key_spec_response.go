...
func (dst *IamPrivateKeySpecResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "iam.PrivateKeySpec.List" {
		...
		if err == nil {
			jsonIamPrivateKeySpecList, _ := json.Marshal(dst.IamPrivateKeySpecList)
			if string(jsonIamPrivateKeySpecList) == "{}" { // empty struct
	...
	// try to unmarshal data into IamPrivateKeySpecList
	err = json.Unmarshal(data, &dst.IamPrivateKeySpecList)
	if err == nil {
		jsonIamPrivateKeySpecList, _ := json.Marshal(dst.IamPrivateKeySpecList)
		if string(jsonIamPrivateKeySpecList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IamPrivateKeySpecResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IamPrivateKeySpecResponse)")
	}
}
