...
func (dst *HyperflexClusterResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "hyperflex.Cluster.List" {
		...
		if err == nil {
			jsonHyperflexClusterList, _ := json.Marshal(dst.HyperflexClusterList)
			if string(jsonHyperflexClusterList) == "{}" { // empty struct
	...
	// try to unmarshal data into HyperflexClusterList
	err = json.Unmarshal(data, &dst.HyperflexClusterList)
	if err == nil {
		jsonHyperflexClusterList, _ := json.Marshal(dst.HyperflexClusterList)
		if string(jsonHyperflexClusterList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(HyperflexClusterResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(HyperflexClusterResponse)")
	}
}
