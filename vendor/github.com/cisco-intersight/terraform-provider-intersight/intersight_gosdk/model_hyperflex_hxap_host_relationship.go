...
func (dst *HyperflexHxapHostRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "hyperflex.HxapHost" {
		...
		if err == nil {
			jsonHyperflexHxapHost, _ := json.Marshal(dst.HyperflexHxapHost)
			if string(jsonHyperflexHxapHost) == "{}" { // empty struct
	...
	// try to unmarshal data into HyperflexHxapHost
	err = json.Unmarshal(data, &dst.HyperflexHxapHost)
	if err == nil {
		jsonHyperflexHxapHost, _ := json.Marshal(dst.HyperflexHxapHost)
		if string(jsonHyperflexHxapHost) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(HyperflexHxapHostRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(HyperflexHxapHostRelationship)")
	}
}
