...
func (dst *StorageSasPortRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "storage.SasPort" {
		...
		if err == nil {
			jsonStorageSasPort, _ := json.Marshal(dst.StorageSasPort)
			if string(jsonStorageSasPort) == "{}" { // empty struct
	...
	// try to unmarshal data into StorageSasPort
	err = json.Unmarshal(data, &dst.StorageSasPort)
	if err == nil {
		jsonStorageSasPort, _ := json.Marshal(dst.StorageSasPort)
		if string(jsonStorageSasPort) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(StorageSasPortRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(StorageSasPortRelationship)")
	}
}
