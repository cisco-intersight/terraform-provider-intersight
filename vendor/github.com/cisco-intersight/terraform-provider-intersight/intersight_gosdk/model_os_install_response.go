...
func (dst *OsInstallResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "os.Install.List" {
		...
		if err == nil {
			jsonOsInstallList, _ := json.Marshal(dst.OsInstallList)
			if string(jsonOsInstallList) == "{}" { // empty struct
	...
	// try to unmarshal data into OsInstallList
	err = json.Unmarshal(data, &dst.OsInstallList)
	if err == nil {
		jsonOsInstallList, _ := json.Marshal(dst.OsInstallList)
		if string(jsonOsInstallList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(OsInstallResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(OsInstallResponse)")
	}
}
