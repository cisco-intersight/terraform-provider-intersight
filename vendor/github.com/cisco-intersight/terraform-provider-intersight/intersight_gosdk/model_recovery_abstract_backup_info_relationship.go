...
func (dst *RecoveryAbstractBackupInfoRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "recovery.AbstractBackupInfo" {
		...
		if err == nil {
			jsonRecoveryAbstractBackupInfo, _ := json.Marshal(dst.RecoveryAbstractBackupInfo)
			if string(jsonRecoveryAbstractBackupInfo) == "{}" { // empty struct
	...
	// try to unmarshal data into RecoveryAbstractBackupInfo
	err = json.Unmarshal(data, &dst.RecoveryAbstractBackupInfo)
	if err == nil {
		jsonRecoveryAbstractBackupInfo, _ := json.Marshal(dst.RecoveryAbstractBackupInfo)
		if string(jsonRecoveryAbstractBackupInfo) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(RecoveryAbstractBackupInfoRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(RecoveryAbstractBackupInfoRelationship)")
	}
}
