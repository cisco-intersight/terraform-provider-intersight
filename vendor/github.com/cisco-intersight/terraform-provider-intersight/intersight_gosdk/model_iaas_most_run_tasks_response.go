...
func (dst *IaasMostRunTasksResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "iaas.MostRunTasks.List" {
		...
		if err == nil {
			jsonIaasMostRunTasksList, _ := json.Marshal(dst.IaasMostRunTasksList)
			if string(jsonIaasMostRunTasksList) == "{}" { // empty struct
	...
	// try to unmarshal data into IaasMostRunTasksList
	err = json.Unmarshal(data, &dst.IaasMostRunTasksList)
	if err == nil {
		jsonIaasMostRunTasksList, _ := json.Marshal(dst.IaasMostRunTasksList)
		if string(jsonIaasMostRunTasksList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IaasMostRunTasksResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IaasMostRunTasksResponse)")
	}
}
