...
func (dst *ComputeRackUnitIdentityResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "compute.RackUnitIdentity.List" {
		...
		if err == nil {
			jsonComputeRackUnitIdentityList, _ := json.Marshal(dst.ComputeRackUnitIdentityList)
			if string(jsonComputeRackUnitIdentityList) == "{}" { // empty struct
	...
	// try to unmarshal data into ComputeRackUnitIdentityList
	err = json.Unmarshal(data, &dst.ComputeRackUnitIdentityList)
	if err == nil {
		jsonComputeRackUnitIdentityList, _ := json.Marshal(dst.ComputeRackUnitIdentityList)
		if string(jsonComputeRackUnitIdentityList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(ComputeRackUnitIdentityResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(ComputeRackUnitIdentityResponse)")
	}
}
