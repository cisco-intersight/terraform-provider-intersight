...
func (dst *StoragePureProtectionGroupResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "storage.PureProtectionGroup.List" {
		...
		if err == nil {
			jsonStoragePureProtectionGroupList, _ := json.Marshal(dst.StoragePureProtectionGroupList)
			if string(jsonStoragePureProtectionGroupList) == "{}" { // empty struct
	...
	// try to unmarshal data into StoragePureProtectionGroupList
	err = json.Unmarshal(data, &dst.StoragePureProtectionGroupList)
	if err == nil {
		jsonStoragePureProtectionGroupList, _ := json.Marshal(dst.StoragePureProtectionGroupList)
		if string(jsonStoragePureProtectionGroupList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(StoragePureProtectionGroupResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(StoragePureProtectionGroupResponse)")
	}
}
