...
func (dst *ComputeRackUnitRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "compute.RackUnit" {
		...
		if err == nil {
			jsonComputeRackUnit, _ := json.Marshal(dst.ComputeRackUnit)
			if string(jsonComputeRackUnit) == "{}" { // empty struct
	...
	// try to unmarshal data into ComputeRackUnit
	err = json.Unmarshal(data, &dst.ComputeRackUnit)
	if err == nil {
		jsonComputeRackUnit, _ := json.Marshal(dst.ComputeRackUnit)
		if string(jsonComputeRackUnit) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(ComputeRackUnitRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(ComputeRackUnitRelationship)")
	}
}
