...
func (dst *IamAccountResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "iam.Account.List" {
		...
		if err == nil {
			jsonIamAccountList, _ := json.Marshal(dst.IamAccountList)
			if string(jsonIamAccountList) == "{}" { // empty struct
	...
	// try to unmarshal data into IamAccountList
	err = json.Unmarshal(data, &dst.IamAccountList)
	if err == nil {
		jsonIamAccountList, _ := json.Marshal(dst.IamAccountList)
		if string(jsonIamAccountList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IamAccountResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IamAccountResponse)")
	}
}
