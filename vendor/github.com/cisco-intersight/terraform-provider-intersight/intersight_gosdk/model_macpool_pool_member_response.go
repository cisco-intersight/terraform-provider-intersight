...
func (dst *MacpoolPoolMemberResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "macpool.PoolMember.List" {
		...
		if err == nil {
			jsonMacpoolPoolMemberList, _ := json.Marshal(dst.MacpoolPoolMemberList)
			if string(jsonMacpoolPoolMemberList) == "{}" { // empty struct
	...
	// try to unmarshal data into MacpoolPoolMemberList
	err = json.Unmarshal(data, &dst.MacpoolPoolMemberList)
	if err == nil {
		jsonMacpoolPoolMemberList, _ := json.Marshal(dst.MacpoolPoolMemberList)
		if string(jsonMacpoolPoolMemberList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(MacpoolPoolMemberResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(MacpoolPoolMemberResponse)")
	}
}
