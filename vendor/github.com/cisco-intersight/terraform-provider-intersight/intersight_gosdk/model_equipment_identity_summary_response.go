...
func (dst *EquipmentIdentitySummaryResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "equipment.IdentitySummary.List" {
		...
		if err == nil {
			jsonEquipmentIdentitySummaryList, _ := json.Marshal(dst.EquipmentIdentitySummaryList)
			if string(jsonEquipmentIdentitySummaryList) == "{}" { // empty struct
	...
	// try to unmarshal data into EquipmentIdentitySummaryList
	err = json.Unmarshal(data, &dst.EquipmentIdentitySummaryList)
	if err == nil {
		jsonEquipmentIdentitySummaryList, _ := json.Marshal(dst.EquipmentIdentitySummaryList)
		if string(jsonEquipmentIdentitySummaryList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(EquipmentIdentitySummaryResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(EquipmentIdentitySummaryResponse)")
	}
}
