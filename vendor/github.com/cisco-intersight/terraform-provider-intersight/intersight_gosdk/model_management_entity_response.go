...
func (dst *ManagementEntityResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "management.Entity.List" {
		...
		if err == nil {
			jsonManagementEntityList, _ := json.Marshal(dst.ManagementEntityList)
			if string(jsonManagementEntityList) == "{}" { // empty struct
	...
	// try to unmarshal data into ManagementEntityList
	err = json.Unmarshal(data, &dst.ManagementEntityList)
	if err == nil {
		jsonManagementEntityList, _ := json.Marshal(dst.ManagementEntityList)
		if string(jsonManagementEntityList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(ManagementEntityResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(ManagementEntityResponse)")
	}
}
