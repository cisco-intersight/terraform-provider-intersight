...
func (dst *AdapterHostIscsiInterfaceRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "adapter.HostIscsiInterface" {
		...
		if err == nil {
			jsonAdapterHostIscsiInterface, _ := json.Marshal(dst.AdapterHostIscsiInterface)
			if string(jsonAdapterHostIscsiInterface) == "{}" { // empty struct
	...
	// try to unmarshal data into AdapterHostIscsiInterface
	err = json.Unmarshal(data, &dst.AdapterHostIscsiInterface)
	if err == nil {
		jsonAdapterHostIscsiInterface, _ := json.Marshal(dst.AdapterHostIscsiInterface)
		if string(jsonAdapterHostIscsiInterface) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(AdapterHostIscsiInterfaceRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(AdapterHostIscsiInterfaceRelationship)")
	}
}
