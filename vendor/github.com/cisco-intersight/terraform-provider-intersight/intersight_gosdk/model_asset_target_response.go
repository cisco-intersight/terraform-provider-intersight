...
func (dst *AssetTargetResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "asset.Target.List" {
		...
		if err == nil {
			jsonAssetTargetList, _ := json.Marshal(dst.AssetTargetList)
			if string(jsonAssetTargetList) == "{}" { // empty struct
	...
	// try to unmarshal data into AssetTargetList
	err = json.Unmarshal(data, &dst.AssetTargetList)
	if err == nil {
		jsonAssetTargetList, _ := json.Marshal(dst.AssetTargetList)
		if string(jsonAssetTargetList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(AssetTargetResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(AssetTargetResponse)")
	}
}
