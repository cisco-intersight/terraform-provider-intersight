...
func (dst *VirtualizationBaseHostRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "virtualization.BaseHost" {
		...
		if err == nil {
			jsonVirtualizationBaseHost, _ := json.Marshal(dst.VirtualizationBaseHost)
			if string(jsonVirtualizationBaseHost) == "{}" { // empty struct
	...
	// try to unmarshal data into VirtualizationBaseHost
	err = json.Unmarshal(data, &dst.VirtualizationBaseHost)
	if err == nil {
		jsonVirtualizationBaseHost, _ := json.Marshal(dst.VirtualizationBaseHost)
		if string(jsonVirtualizationBaseHost) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(VirtualizationBaseHostRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(VirtualizationBaseHostRelationship)")
	}
}
