...
func (dst *HyperflexLocalCredentialPolicyResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "hyperflex.LocalCredentialPolicy.List" {
		...
		if err == nil {
			jsonHyperflexLocalCredentialPolicyList, _ := json.Marshal(dst.HyperflexLocalCredentialPolicyList)
			if string(jsonHyperflexLocalCredentialPolicyList) == "{}" { // empty struct
	...
	// try to unmarshal data into HyperflexLocalCredentialPolicyList
	err = json.Unmarshal(data, &dst.HyperflexLocalCredentialPolicyList)
	if err == nil {
		jsonHyperflexLocalCredentialPolicyList, _ := json.Marshal(dst.HyperflexLocalCredentialPolicyList)
		if string(jsonHyperflexLocalCredentialPolicyList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(HyperflexLocalCredentialPolicyResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(HyperflexLocalCredentialPolicyResponse)")
	}
}
