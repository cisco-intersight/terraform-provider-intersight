...
func (dst *VirtualizationVmwareDatacenterRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "virtualization.VmwareDatacenter" {
		...
		if err == nil {
			jsonVirtualizationVmwareDatacenter, _ := json.Marshal(dst.VirtualizationVmwareDatacenter)
			if string(jsonVirtualizationVmwareDatacenter) == "{}" { // empty struct
	...
	// try to unmarshal data into VirtualizationVmwareDatacenter
	err = json.Unmarshal(data, &dst.VirtualizationVmwareDatacenter)
	if err == nil {
		jsonVirtualizationVmwareDatacenter, _ := json.Marshal(dst.VirtualizationVmwareDatacenter)
		if string(jsonVirtualizationVmwareDatacenter) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(VirtualizationVmwareDatacenterRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(VirtualizationVmwareDatacenterRelationship)")
	}
}
