...
func (dst *InventoryGenericInventoryHolderRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "inventory.GenericInventoryHolder" {
		...
		if err == nil {
			jsonInventoryGenericInventoryHolder, _ := json.Marshal(dst.InventoryGenericInventoryHolder)
			if string(jsonInventoryGenericInventoryHolder) == "{}" { // empty struct
	...
	// try to unmarshal data into InventoryGenericInventoryHolder
	err = json.Unmarshal(data, &dst.InventoryGenericInventoryHolder)
	if err == nil {
		jsonInventoryGenericInventoryHolder, _ := json.Marshal(dst.InventoryGenericInventoryHolder)
		if string(jsonInventoryGenericInventoryHolder) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(InventoryGenericInventoryHolderRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(InventoryGenericInventoryHolderRelationship)")
	}
}
