...
func (dst *IamOAuthTokenResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "iam.OAuthToken.List" {
		...
		if err == nil {
			jsonIamOAuthTokenList, _ := json.Marshal(dst.IamOAuthTokenList)
			if string(jsonIamOAuthTokenList) == "{}" { // empty struct
	...
	// try to unmarshal data into IamOAuthTokenList
	err = json.Unmarshal(data, &dst.IamOAuthTokenList)
	if err == nil {
		jsonIamOAuthTokenList, _ := json.Marshal(dst.IamOAuthTokenList)
		if string(jsonIamOAuthTokenList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(IamOAuthTokenResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(IamOAuthTokenResponse)")
	}
}
