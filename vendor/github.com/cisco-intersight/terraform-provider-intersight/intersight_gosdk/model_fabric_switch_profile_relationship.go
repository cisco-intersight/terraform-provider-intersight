...
func (dst *FabricSwitchProfileRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "fabric.SwitchProfile" {
		...
		if err == nil {
			jsonFabricSwitchProfile, _ := json.Marshal(dst.FabricSwitchProfile)
			if string(jsonFabricSwitchProfile) == "{}" { // empty struct
	...
	// try to unmarshal data into FabricSwitchProfile
	err = json.Unmarshal(data, &dst.FabricSwitchProfile)
	if err == nil {
		jsonFabricSwitchProfile, _ := json.Marshal(dst.FabricSwitchProfile)
		if string(jsonFabricSwitchProfile) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(FabricSwitchProfileRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(FabricSwitchProfileRelationship)")
	}
}
