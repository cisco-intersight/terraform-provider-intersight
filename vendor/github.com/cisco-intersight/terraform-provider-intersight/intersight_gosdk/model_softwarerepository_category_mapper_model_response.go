...
func (dst *SoftwarerepositoryCategoryMapperModelResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "softwarerepository.CategoryMapperModel.List" {
		...
		if err == nil {
			jsonSoftwarerepositoryCategoryMapperModelList, _ := json.Marshal(dst.SoftwarerepositoryCategoryMapperModelList)
			if string(jsonSoftwarerepositoryCategoryMapperModelList) == "{}" { // empty struct
	...
	// try to unmarshal data into SoftwarerepositoryCategoryMapperModelList
	err = json.Unmarshal(data, &dst.SoftwarerepositoryCategoryMapperModelList)
	if err == nil {
		jsonSoftwarerepositoryCategoryMapperModelList, _ := json.Marshal(dst.SoftwarerepositoryCategoryMapperModelList)
		if string(jsonSoftwarerepositoryCategoryMapperModelList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(SoftwarerepositoryCategoryMapperModelResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(SoftwarerepositoryCategoryMapperModelResponse)")
	}
}
