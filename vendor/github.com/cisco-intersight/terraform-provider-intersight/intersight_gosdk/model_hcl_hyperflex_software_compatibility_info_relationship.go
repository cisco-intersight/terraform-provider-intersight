...
func (dst *HclHyperflexSoftwareCompatibilityInfoRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "hcl.HyperflexSoftwareCompatibilityInfo" {
		...
		if err == nil {
			jsonHclHyperflexSoftwareCompatibilityInfo, _ := json.Marshal(dst.HclHyperflexSoftwareCompatibilityInfo)
			if string(jsonHclHyperflexSoftwareCompatibilityInfo) == "{}" { // empty struct
	...
	// try to unmarshal data into HclHyperflexSoftwareCompatibilityInfo
	err = json.Unmarshal(data, &dst.HclHyperflexSoftwareCompatibilityInfo)
	if err == nil {
		jsonHclHyperflexSoftwareCompatibilityInfo, _ := json.Marshal(dst.HclHyperflexSoftwareCompatibilityInfo)
		if string(jsonHclHyperflexSoftwareCompatibilityInfo) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(HclHyperflexSoftwareCompatibilityInfoRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(HclHyperflexSoftwareCompatibilityInfoRelationship)")
	}
}
