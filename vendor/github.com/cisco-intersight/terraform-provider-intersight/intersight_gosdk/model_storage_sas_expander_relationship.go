...
func (dst *StorageSasExpanderRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "storage.SasExpander" {
		...
		if err == nil {
			jsonStorageSasExpander, _ := json.Marshal(dst.StorageSasExpander)
			if string(jsonStorageSasExpander) == "{}" { // empty struct
	...
	// try to unmarshal data into StorageSasExpander
	err = json.Unmarshal(data, &dst.StorageSasExpander)
	if err == nil {
		jsonStorageSasExpander, _ := json.Marshal(dst.StorageSasExpander)
		if string(jsonStorageSasExpander) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(StorageSasExpanderRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(StorageSasExpanderRelationship)")
	}
}
