...
func (dst *OsConfigurationFileRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "os.ConfigurationFile" {
		...
		if err == nil {
			jsonOsConfigurationFile, _ := json.Marshal(dst.OsConfigurationFile)
			if string(jsonOsConfigurationFile) == "{}" { // empty struct
	...
	// try to unmarshal data into OsConfigurationFile
	err = json.Unmarshal(data, &dst.OsConfigurationFile)
	if err == nil {
		jsonOsConfigurationFile, _ := json.Marshal(dst.OsConfigurationFile)
		if string(jsonOsConfigurationFile) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(OsConfigurationFileRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(OsConfigurationFileRelationship)")
	}
}
