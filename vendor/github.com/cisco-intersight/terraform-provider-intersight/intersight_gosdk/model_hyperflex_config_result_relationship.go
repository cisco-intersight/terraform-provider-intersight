...
func (dst *HyperflexConfigResultRelationship) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ClassId"] == "hyperflex.ConfigResult" {
		...
		if err == nil {
			jsonHyperflexConfigResult, _ := json.Marshal(dst.HyperflexConfigResult)
			if string(jsonHyperflexConfigResult) == "{}" { // empty struct
	...
	// try to unmarshal data into HyperflexConfigResult
	err = json.Unmarshal(data, &dst.HyperflexConfigResult)
	if err == nil {
		jsonHyperflexConfigResult, _ := json.Marshal(dst.HyperflexConfigResult)
		if string(jsonHyperflexConfigResult) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(HyperflexConfigResultRelationship)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(HyperflexConfigResultRelationship)")
	}
}
