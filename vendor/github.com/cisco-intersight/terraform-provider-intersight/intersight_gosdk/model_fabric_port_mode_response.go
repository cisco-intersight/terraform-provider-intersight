...
func (dst *FabricPortModeResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "fabric.PortMode.List" {
		...
		if err == nil {
			jsonFabricPortModeList, _ := json.Marshal(dst.FabricPortModeList)
			if string(jsonFabricPortModeList) == "{}" { // empty struct
	...
	// try to unmarshal data into FabricPortModeList
	err = json.Unmarshal(data, &dst.FabricPortModeList)
	if err == nil {
		jsonFabricPortModeList, _ := json.Marshal(dst.FabricPortModeList)
		if string(jsonFabricPortModeList) == "{}" { // empty struct
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("Data matches more than one schema in oneOf(FabricPortModeResponse)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("Data failed to match schemas in oneOf(FabricPortModeResponse)")
	}
}
